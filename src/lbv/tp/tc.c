 ...
 *
 * @return	 LbvPose   returns a position (\ref LbvPose = datatype carrying XYZABC information
 */

int tcGetPos(TC_STRUCT const * const tc, LbvPose * const out) {
    tcGetPosReal(tc, TC_GET_PROGRESS, out);
    return 0;
}

int tcGetStartpoint(TC_STRUCT const * const tc, LbvPose * const out) {
    tcGetPosReal(tc, TC_GET_STARTPOINT, out);
    return 0;
}

int tcGetEndpoint(TC_STRUCT const * const tc, LbvPose * const out) {
    tcGetPosReal(tc, TC_GET_ENDPOINT, out);
    return 0;
}

int tcGetPosReal(TC_STRUCT const * const tc, int of_point, LbvPose * const pos)
{
    PmCartesian xyz;
    PmCartesian abc;
    PmCartesian uvw;
    ...
    if (res_fit == TP_ERR_OK) {
        ...
        pmCartesianToLbvPose(&xyz, &abc, &uvw, pos);
    }
    return res_fit;
}
...
int pmLine9Init(PmLine9 * const line9,
        LbvPose const * const start,
        LbvPose const * const end)
{
    // Scratch variables
    ...
    lbvPoseToPmCartesian(start, &start_xyz, &start_abc, &start_uvw);
    lbvPoseToPmCartesian(end, &end_xyz, &end_abc, &end_uvw);
        ...
        double reversal_scale)
{
    ...
    lbvPoseToPmCartesian(start, &start_xyz, &abc, &uvw);
    lbvPoseGetXYZ(end, &end_xyz);
