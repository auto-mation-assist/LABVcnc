...
#include "blendmath.h"
//KLUDGE Don't include all of lbv.hh here, just hand-copy the TERM COND
//definitions until we can break the lbv constants out into a separate file.
//#include "lbv.hh"
...
extern lbvmot_status_t *lbvmotStatus;
extern lbvmot_debug_t *lbvmotDebug;
extern lbvmot_config_t *lbvmotConfig;
...
STATIC double tpGetTangentKinkRatio(void) {
    ...
    return fmax(fmin(lbvmotConfig->arcBlendTangentKinkRatio,max_ratio),min_ratio);
}
...
STATIC int tpGetMachineAccelBounds(PmCartesian  * const acc_bound) {
    ...
    acc_bound->x = lbvmotDebug->axes[0].acc_limit; //0==>x
    acc_bound->y = lbvmotDebug->axes[1].acc_limit; //1==>y
    acc_bound->z = lbvmotDebug->axes[2].acc_limit; //2==>z
    return TP_ERR_OK;
}
...
STATIC int tpGetMachineVelBounds(PmCartesian  * const vel_bound) {
    ...
    vel_bound->x = lbvmotDebug->axes[0].vel_limit; //0==>x
    vel_bound->y = lbvmotDebug->axes[1].vel_limit; //1==>y
    vel_bound->z = lbvmotDebug->axes[2].vel_limit; //2==>z
    return TP_ERR_OK;
}
...
/**
 * Get a segment's feed scale based on the current planner state and lbvmotStatus.
 * @note depends on lbvmotStatus for system information.
 */
STATIC double tpGetFeedScale(TP_STRUCT const * const tp,
    ...
    if (pausing)  {
        ...
        return fmin(lbvmotStatus->net_feed_scale, 1.0);
    } else {
        return lbvmotStatus->net_feed_scale;
    }
}
...
STATIC inline double getMaxFeedScale(TC_STRUCT const * tc)
{
    ...
    if (tc && tc->synchronized == TC_SYNC_POSITION ) {
        ...
        return lbvmotConfig->maxFeedScale;
    }
}
...
STATIC inline double tpGetMaxTargetVel(TP_STRUCT const * const tp, TC_STRUCT const * const tc)
{
    double max_scale = lbvmotConfig->maxFeedScale;
    if (tc->is_blending) {
        //KLUDGE: Don't allow feed override to keep blending from overruning max velocity
...
STATIC inline double tpGetRealFinalVel(TP_STRUCT const * const tp,
     ...
     */
    if (lbvmotDebug->stepping || tc->term_cond != TC_TERM_COND_TANGENT || tp->reverse_run) {
        return 0.0;
    } 
...
int tpClearDIOs(TP_STRUCT * const tp) {
    ...
    for (i = 0; i < lbvmotConfig->numDIO; i++) {
        tp->syncdio.dios[i] = 0;
    }
    for (i = 0; i < lbvmotConfig->numAIO; i++) {
        tp->syncdio.aios[i] = 0;
    }
...
int tpClear(TP_STRUCT * const tp)
{
    ...
    tp->synchronized = 0;
    tp->uu_per_rev = 0.0;
    lbvmotStatus->current_vel = 0.0;
    lbvmotStatus->requested_vel = 0.0;
    lbvmotStatus->distance_to_go = 0.0;
    ZERO_LBV_POSE(lbvmotStatus->dtg);
 ...
 */
int tpSetPos(TP_STRUCT * const tp, LbvPose const * const pos)
{
    if (0 == tp) {
        return TP_ERR_FAIL;
    }
 ...
 * only at TP initialization and when switching modes.
 */
int tpSetCurrentPos(TP_STRUCT * const tp, LbvPose const * const pos)
{
    if (0 == tp) {
        return TP_ERR_FAIL;
    }
...
int tpAddCurrentPos(TP_STRUCT * const tp, LbvPose const * const disp)
{
    if (!tp || !disp) {
        return TP_ERR_MISSING_INPUT;
    }

    if (lbvPoseValid(disp)) {
        lbvPoseSelfAdd(&tp->currentPos, disp);
        return TP_ERR_OK;
    } else {
...
STATIC int tpInitBlendArcFromPrev(TP_STRUCT const * const tp,
    ...
    int canon_motion_type = LBV_MOTION_TYPE_ARC;
#else
...
STATIC tp_err_t tpCreateLineArcBlend(TP_STRUCT * const tp, TC_STRUCT * const prev_tc, TC_STRUCT * const tc, TC_STRUCT * const blend_tc)
{
    ...
    blendCheckConsume(&param, &points_exact, prev_tc, lbvmotConfig->arcBlendGapCycles);
        ...
        TC_STRUCT * const tc, TC_STRUCT * const blend_tc)
{
    ...
    blendCheckConsume(&param, &points, prev_tc, lbvmotConfig->arcBlendGapCycles);
...
STATIC int tpCheckCanonType(TC_STRUCT * prev_tc, TC_STRUCT * tc)
{
    ...
    if ((prev_tc->canon_motion_type == LBV_MOTION_TYPE_TRAVERSE) ^
            (tc->canon_motion_type == LBV_MOTION_TYPE_TRAVERSE)) {
 ...
 */
int tpAddRigidTap(TP_STRUCT * const tp, LbvPose end, double vel, double ini_maxvel,
...
STATIC int tpRunOptimization(TP_STRUCT * const tp) {
    ...
    for (x = 1; x < lbvmotConfig->arcBlendOptDepth + 2; ++x) {
 ...
 */
int tpAddLine(TP_STRUCT * const tp, LbvPose end, int canon_motion_type, double vel, double
    ...
    if (lbvmotConfig->arcBlendEnable){
...
void tpToggleDIOs(TC_STRUCT * const tc) {
    ...
    if (tc->syncdio.anychanged != 0) { // we have DIO's to turn on or off
        for (i=0; i < lbvmotConfig->numDIO; i++) {
            ...
            if (tc->syncdio.dios[i] > 0) lbvmotDioWrite(i, 1); // turn DIO[i] on
            if (tc->syncdio.dios[i] < 0) lbvmotDioWrite(i, 0); // turn DIO[i] off
        }
        for (i=0; i < lbvmotConfig->numAIO; i++) {
            if (!(tc->syncdio.aio_mask & (1 << i))) continue;
            lbvmotAioWrite(i, tc->syncdio.aios[i]); // set AIO[i]
        }
...
STATIC void tpUpdateRigidTapState(TP_STRUCT const * const tp,
    ...
    double new_spindlepos = lbvmotStatus->spindle_status[tp->spindle.spindle_num].spindleRevs;
    if (lbvmotStatus->spindle_status[tp->spindle.spindle_num].direction < 0)
    ...
    switch (tc->coords.rigidtap.state) {
        case TAPPING:
            ...
            if (tc->progress >= tc->coords.rigidtap.reversal_target) {
                // command reversal
            	lbvmotStatus->spindle_status[tp->spindle.spindle_num].speed *= -1.0 * tc->coords.rigidtap.reversal_scale;
        ...
        case RETRACTION:
            ...
            if (tc->progress >= tc->coords.rigidtap.reversal_target) {
            	lbvmotStatus->spindle_status[tp->spindle.spindle_num].speed *= -1 / tc->coords.rigidtap.reversal_scale;
...
/**
 * Update lbvMotStatus with information about trajectory motion.
 * Based on the specified trajectory segment tc, read its progress and status
 * flags. Then, update the lbvmotStatus structure with this information.
 */
STATIC int tpUpdateMovementStatus(TP_STRUCT * const tp, TC_STRUCT const * const tc ) {
    ...
    if (!tc) {
        ...
        lbvmotStatus->distance_to_go = 0;
        lbvmotStatus->enables_queued = lbvmotStatus->enables_new;
        lbvmotStatus->requested_vel = 0;
        lbvmotStatus->current_vel = 0;
        lbvPoseZero(&lbvmotStatus->dtg);
    ...
    lbvmotStatus->distance_to_go = tc->target - tc->progress;
    lbvmotStatus->enables_queued = tc->enables;
    // report our line number to the guis
    tp->execId = tc->id;
    lbvmotStatus->requested_vel = tc->reqvel;
    lbvmotStatus->current_vel = tc->currentvel;

    lbvPoseSub(&tc_pos, &tp->currentPos, &lbvmotStatus->dtg);
    return TP_ERR_OK;
}
...
STATIC tp_err_t tpCheckAtSpeed(TP_STRUCT * const tp, TC_STRUCT * const tc)
{
    ...
    if (MOTION_ID_VALID(tp->spindle.waiting_for_atspeed)) {
        for (s = 0; s < lbvmotConfig->numSpindles; s++){
            if(!lbvmotStatus->spindle_status[s].at_speed) {
    ...
    if (MOTION_ID_VALID(tp->spindle.waiting_for_index)) {
        if (lbvmotStatus->spindle_status[tp->spindle.spindle_num].spindle_index_enable) {
...
STATIC tp_err_t tpActivateSegment(TP_STRUCT * const tp, TC_STRUCT * const tc) {
     ...
     * performance cost.
     * */
    double cutoff_time = 1.0 / (fmax(lbvmotConfig->arcBlendRampFreq, TP_TIME_EPSILON));
    ...
    int needs_atspeed = tc->atspeed ||
        (tc->synchronized == TC_SYNC_POSITION && !(lbvmotStatus->spindleSync));
    ...
    if (TC_SYNC_POSITION == tc->synchronized && !(lbvmotStatus->spindleSync)) {
        tp_debug_print("Setting up position sync\n");
        // if we aren't already synced, wait
        tp->spindle.waiting_for_index = tc->id;
        // ask for an index reset
        lbvmotStatus->spindle_status[tp->spindle.spindle_num].spindle_index_enable = 1;
        tp->spindle.offset = 0.0;
...
STATIC void tpSyncVelocityMode(TP_STRUCT * const tp, TC_STRUCT * const tc, TC_STRUCT * const nexttc) {
    double speed = lbvmotStatus->spindle_status[tp->spindle.spindle_num].spindleSpeedIn;
...
STATIC void tpSyncPositionMode(TP_STRUCT * const tp, TC_STRUCT * const tc,
    ...
    double spindle_pos = tpGetSignedSpindlePosition(&lbvmotStatus->spindle_status[tp->spindle.spindle_num]);
...
STATIC int tpDoParabolicBlending(TP_STRUCT * const tp, TC_STRUCT * const tc,
    ...
    lbvmotStatus->current_vel = tc->currentvel + nexttc->currentvel;
...
STATIC int tpUpdateCycle(TP_STRUCT * const tp,
    ...
    LbvPose displacement;

    // Calculate displacement
    tcGetPos(tc, &displacement);
    lbvPoseSelfSub(&displacement, &before);
...
#ifdef TC_DEBUG
    double mag;
    lbvPoseMagnitude(&displacement, &mag);
...
STATIC int tpUpdateInitialStatus(TP_STRUCT const * const tp) {
    // Update queue length
    lbvmotStatus->tcqlen = tcqLen(&tp->queue);
...
int tpRunCycle(TP_STRUCT * const tp, long period)
{
...
#ifdef TC_DEBUG
    LbvPose pos_before = tp->currentPos;
#endif
    ...
    double mag;
    LbvPose disp;
    lbvPoseSub(&tp->currentPos, &pos_before, &disp);
    lbvPoseMagnitude(&disp, &mag);
    tc_debug_print("time: %.12e total movement = %.12e vel = %.12e\n",
            time_elapsed,
            mag, lbvmotStatus->current_vel);
...
int tpGetPos(TP_STRUCT const * const tp, LbvPose * const pos)
{
...
int tpIsMoving(TP_STRUCT const * const tp)
{
    ...
    if (lbvmotStatus->current_vel >= TP_VEL_EPSILON ) {
        tp_debug_print("TP moving, current_vel = %.16g\n", lbvmotStatus->current_vel);
        return true;
