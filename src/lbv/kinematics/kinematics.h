...
/*
  ...
  same, as for slideway machines (XYZ milling machines). The LBV will allow
  changing from joint to world mode and vice versa. Also, the LBV will set
  ...
  Since the LBV requires at least the inverse kinematics, this should simply
  terminate the LBV.

  KINEMATICS_INVERSE_ONLY means that only the inverse kinematics exist.
  The forwards won't be called, and the LBV will only allow changing from
  joint to world mode at the home position.

  KINEMATICS_BOTH means that both the forward and inverse kins are defined.
  Like KINEMATICS_IDENTITY, the LBV will allow changing between world and
...
/* the inverse kinematics take world coordinates and determine joint values,
   ...
   passed in. */
extern int kinematicsInverse(const struct LbvPose * world,
			     double *joint,
...
/* the home kinematics function sets all its arguments to their proper
   ...
   should be used.
*/
extern int kinematicsHome(struct LbvPose * world,
			  double *joint,
			  KINEMATICS_FORWARD_FLAGS * fflags,
			  KINEMATICS_INVERSE_FLAGS * iflags);
