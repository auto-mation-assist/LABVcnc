...
InterpBase *interp_from_shlib(const char *shlib) {
    ...
    if(!interp_lib) {
	fprintf(stderr, "lbvTaskInit: could not open interpreter '%s': %s\n", shlib, dlerror());
	char relative_interp[PATH_MAX];
	snprintf(relative_interp, sizeof(relative_interp), "%s/%s",
	    LBV2_HOME "/lib/lbv2", shlib);
    ...
    if(!constructor) {
	fprintf(stderr, "lbvTaskInit: could not get symbol makeInterp from interpreter '%s': %s\n", shlib, dlerror());
	return 0;
    }
    InterpBase *pinterp = constructor();
    if(!pinterp) {
	fprintf(stderr, "lbvTaskInit: makeInterp() returned NULL from interpreter '%s'\n", shlib);
	return 0;
    }
    return pinterp;
}
