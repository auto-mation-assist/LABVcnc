...
int Interp::init()
{
  ...
  if(iniFileName != NULL) {
      ...
      if (inifile.Open(iniFileName) == false) {
          ...
          inifile.Find(&_setup.tool_change_at_g30, "TOOL_CHANGE_AT_G30", "LBVIO");
          inifile.Find(&_setup.tool_change_quill_up, "TOOL_CHANGE_QUILL_UP", "LBVIO");
          inifile.Find(&_setup.tool_change_with_spindle_on, "TOOL_CHANGE_WITH_SPINDLE_ON", "LBVIO");
          ...
          inifile.Find(&_setup.random_toolchanger, "RANDOM_TOOLCHANGER", "LBVIO");
          ...
          inifile.Find(&_setup.debugmask, "DEBUG", "LBV");

	  _setup.debugmask |= LBV_DEBUG_UNCONDITIONAL;
...
//_setup.stack_index does not need initialization
   ZERO_LBV_POSE(_setup.tool_offset);
...
int Interp::load_tool_table()
{
  ...
  for (; n < CANON_POCKETS_MAX; n++) {
    ...
    ZERO_LBV_POSE(_setup.tool_table[n].offset);
...
Called By:
   lbvtask before calling Interp::init()
