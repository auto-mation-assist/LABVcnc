...
static inline LbvPose get_tool_offset (Interp &interp)  {
    return interp._setup.tool_offset;
}
static inline void set_tool_offset(Interp &interp, LbvPose value)  {
...
BOOST_PYTHON_MODULE(interpreter) {
    ...
    def("equal", &__equal);  // LBV's perception of equality of doubles
    def("is_near_int", &is_near_int);  // LBV's perception of closeness to an int
