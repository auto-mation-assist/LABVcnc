...
#define EXECUTING_BLOCK(s)   ((s).blocks[0])
  ...
  double speed[LBVMOT_MAX_SPINDLES];// array of spindle speeds
  SPINDLE_MODE spindle_mode[LBVMOT_MAX_SPINDLES];// CONSTANT_RPM or CONSTANT_SURFACE
  CANON_SPEED_FEED_MODE speed_feed_mode;        // independent or synched
  bool speed_override[LBVMOT_MAX_SPINDLES];        // whether speed override is enabled
  CANON_DIRECTION spindle_turning[LBVMOT_MAX_SPINDLES];  // direction spindle is turning
  char stack[STACK_LEN][STACK_ENTRY_LEN];      // stack of calls for error reporting
  int stack_index;              // index into the stack
  LbvPose tool_offset;          // tool length offset
  ...
  int debugmask;                     // from ini  LBV/DEBUG
