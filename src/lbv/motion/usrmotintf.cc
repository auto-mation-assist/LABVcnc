...
#include "motion.h"		/* lbvmot_status_t,CMD */
#include "motion_debug.h"       /* lbvmot_debug_t */
#include "motion_struct.h"      /* lbvmot_struct_t */
#include "lbvmotcfg.h"		/* LBVMOT_ERROR_NUM,LEN */
#include "lbvmotglb.h"		/* SHMEM_KEY */
...
static lbvmot_command_t *lbvmotCommand = 0;
static lbvmot_status_t *lbvmotStatus = 0;
static lbvmot_config_t *lbvmotConfig = 0;
static lbvmot_debug_t *lbvmotDebug = 0;
static lbvmot_error_t *lbvmotError = 0;
static lbvmot_struct_t *lbvmotStruct = 0;
...
int usrmotIniLoad(const char *filename)
{
	...
	rtapi_print("can't find lbvmot ini file %s\n", filename);
	return -1;
    }

    try {
        inifile.Find((int *)&SHMEM_KEY, "SHMEM_KEY", "LBVMOT");
        inifile.Find(&LBVMOT_COMM_TIMEOUT, "COMM_TIMEOUT", "LBVMOT");
    }
...
int usrmotWriteLbvmotCommand(lbvmot_command_t * c)
{
    lbvmot_status_t s;
	...
	return LBVMOT_COMM_INVALID_MOTION_ID;
    }
    c->head = ++headCount;
    c->tail = c->head;
	...
	return LBVMOT_COMM_ERROR_CONNECT;
    }
    /* copy entire command structure to shared memory */
    *lbvmotCommand = *c;
    /* poll for receipt of command */
    /* set timeout for comm failure, now + timeout */
    end = etime() + LBVMOT_COMM_TIMEOUT;
    /* now check to see if it got it */
    while (etime() < end) {
	/* update status */
	if (( usrmotReadLbvmotStatus(&s) == 0 ) && ( s.commandNumEcho == commandNum )) {
	    /* now check lbvmot status flag */
	    if (s.commandStatus == LBVMOT_COMMAND_OK) {
		return LBVMOT_COMM_OK;
	    } else {
                rcs_print("USRMOT: ERROR: invalid command\n");
		return LBVMOT_COMM_ERROR_COMMAND;
	    }
	}
	esleep(25e-6);
    }
    rcs_print("USRMOT: ERROR: command timeout\n");
    return LBVMOT_COMM_ERROR_TIMEOUT;
}

/* copies status to s */
int usrmotReadLbvmotStatus(lbvmot_status_t * s)
{
	...
	memcpy(s, lbvmotStatus, sizeof(lbvmot_status_t));
    ...
    return LBVMOT_COMM_SPLIT_READ_TIMEOUT;
}

/* copies config to s */
int usrmotReadLbvmotConfig(lbvmot_config_t * s)
{
    int split_read_count;
	...
	memcpy(s, lbvmotConfig, sizeof(lbvmot_config_t));
...
printf("ReadLbvmotConfig COMM_SPLIT_READ_TIMEOUT\n" );
    return LBVMOT_COMM_SPLIT_READ_TIMEOUT;
}

/* copies debug to s */
int usrmotReadLbvmotDebug(lbvmot_debug_t * s)
{
    int split_read_count;
	...
	memcpy(s, lbvmotDebug, sizeof(lbvmot_debug_t));
	/* got it, now check head-tail matches */
	if (s->head == s->tail) {
	    /* head and tail match, done */
	    return LBVMOT_COMM_OK;
...
printf("ReadLbvmotDebug COMM_SPLIT_READ_TIMEOUT\n" );
    return LBVMOT_COMM_SPLIT_READ_TIMEOUT;
}

/* copies error to s */
int usrmotReadLbvmotError(char *e)
{
    ...
    char data[LBVMOT_ERROR_LEN];
    struct dbuf d;
    dbuf_init(&d, (unsigned char *)data, LBVMOT_ERROR_LEN);

    /* returns 0 if something, -1 if not */
    int result = lbvmotErrorGet(lbvmotError, data);
    if(result < 0) return result;

    struct dbuf_iter di;
    dbuf_iter_init(&di, &d);

    result =  snprintdbuf(e, LBVMOT_ERROR_LEN, &di);
    if(result < 0) return result;
    return 0;
}
...
void printLbvPose(LbvPose * pose)
{
...
void printTPstruct(TP_STRUCT * tp)
{
    ...
    printf("termCond=%d\n", tp->termCond);
    printf("currentPos :");
    printLbvPose(&tp->currentPos);
    printf("\n");
    printf("goalPos :");
    printLbvPose(&tp->goalPos);
    printf("\n");
    printf("done=%d\n", tp->done);
    printf("depth=%d\n", tp->depth);
...
void usrmotPrintLbvmotDebug(lbvmot_debug_t *d, int which)
{
//    int t;
...
void usrmotPrintLbvmotConfig(lbvmot_config_t c, int which)
{
//    int t;
//    char m[32];
...
void usrmotPrintLbvmotStatus(lbvmot_status_t *s, int which)
{
//    int t;
//    char m[32];
    ...
    case 0:
	printf("mode:         \t%s\n",
	    s->motionFlag & LBVMOT_MOTION_TELEOP_BIT ? "teleop" :
	    (s->motionFlag & LBVMOT_MOTION_COORD_BIT ? "coord" : "free")
	    );
	printf("cmd:          \t%d\n", s->commandEcho);
	printf("cmd num:      \t%d\n", s->commandNumEcho);
	printf("heartbeat:    \t%u\n", s->heartbeat);
/*! \todo Another #if 0 */
	...
	printf("axes enabled: \t");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d", s->axisFlag[t] & LBVMOT_JOINT_ENABLE_BIT ? 1 : 0);
	}
	printf("\n");
#endif
...
/*! \todo Another #if 0 */
	...
	printf("inpos:        \t%d\n",
	    s->motionFlag & LBVMOT_MOTION_INPOS_BIT ? 1 : 0);
/*! \todo Another #if 0 */
#if 0				/*! \todo FIXME - change to work with joint
				   structures */
	printf("homing:       \t");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d", s->axisFlag[0] & LBVMOT_JOINT_HOMING_BIT ? 1 : 0);
	}
	printf("\n");
#endif
	printf("enabled:     \t%s\n",
	    s->motionFlag & LBVMOT_MOTION_ENABLE_BIT ? "ENABLED" : "DISABLED");
...
/*! \todo Another #if 0 */
	...
	printf("\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_ENABLE_BIT) != 0));
	}
	printf("enable\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_ACTIVE_BIT) != 0));
	}
	printf("active\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_INPOS_BIT) != 0));
	}
	printf("inpos\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_ERROR_BIT) != 0));
	}
	printf("error\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t",
		((s->axisFlag[t] & LBVMOT_JOINT_MAX_SOFT_LIMIT_BIT) != 0));
	}
	printf("max_soft_limit\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t",
		((s->axisFlag[t] & LBVMOT_JOINT_MIN_SOFT_LIMIT_BIT) != 0));
	}
	printf("min_soft_limit\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t",
		((s->axisFlag[t] & LBVMOT_JOINT_MAX_HARD_LIMIT_BIT) != 0));
	}
	printf("max_hard_limit\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t",
		((s->axisFlag[t] & LBVMOT_JOINT_MIN_HARD_LIMIT_BIT) != 0));
	}
	printf("min_hard_limit\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t",
		((s->axisFlag[t] & LBVMOT_JOINT_HOME_SWITCH_BIT) != 0));
	}
	printf("home_switch\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_HOMING_BIT) != 0));
	}
	printf("homing\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_HOMED_BIT) != 0));
	}
	printf("homed\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_FERROR_BIT) != 0));
	}
	printf("ferror\n");
	for (t = 0; t < LBVMOT_MAX_JOINTS; t++) {
	    printf("%d\t", ((s->axisFlag[t] & LBVMOT_JOINT_FAULT_BIT) != 0));
	}
#endif
	printf("fault\n");
	printf("\npolarity: ");
...
int usrmotInit(const char *modname)
{
    ...
    shmem_id = rtapi_shmem_new(SHMEM_KEY, module_id, sizeof(lbvmot_struct_t));
    if (shmem_id < 0) {
	fprintf(stderr,
...
/*! \todo FIXME - comp structs no longer in shmem */
    for (axis = 0; axis < LBVMOT_MAX_JOINTS; axis++) {
	lbvmotComp[axis] = 0;
    }
#endif
...
int usrmotLoadComp(int joint, const char *file, int type)
{
    ...
    double nom, fwd, rev;
    int ret = 0;
    lbvmot_command_t lbvmotCommand;
	...
	if (3 != sscanf(buffer, "%lf %lf %lf", &nom, &fwd, &rev)) {
		...
		/* expecting nominal-forward-reverse triplets, e.g., 
    		...
    		lbvmotCommand.comp_nominal = nom;
    		lbvmotCommand.comp_forward = nom - fwd; //convert to diffs
    		lbvmotCommand.comp_reverse = nom - rev; //convert to diffs
	    } else {
		/* expecting nominal-forw_trim-rev_trim triplets */
    		lbvmotCommand.comp_nominal = nom;
    		lbvmotCommand.comp_forward = fwd;
    		lbvmotCommand.comp_reverse = rev;		
	    }
	    lbvmotCommand.joint = joint;
	    lbvmotCommand.command = LBVMOT_SET_JOINT_COMP;
	    ret |= usrmotWriteLbvmotCommand(&lbvmotCommand);
	}
    }
    fclose(fp);
...
/* currently disabled */
    ...
    if (lbvmotComp[joint] == 0) {
	fprintf(stderr, "compensation data structure not present\n");
	return -1;
    }

    printf("total:  %d\n", lbvmotComp[joint]->total);
    printf("avgint: %f\n", lbvmotComp[joint]->avgint);
    for (t = 0; t < lbvmotComp[joint]->total; t++) {
	printf("%f\t%f\t%f\n",
	    lbvmotComp[joint]->nominal[t],
	    lbvmotComp[joint]->forward[t], lbvmotComp[joint]->reverse[t]);
    }
