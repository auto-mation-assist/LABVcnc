...
MODULE_DESCRIPTION("Motion Controller for LBV");
MODULE_LICENSE("GPL");
...
static int num_joints = LBVMOT_MAX_JOINTS;	/* default number of joints present */
...
/* pointer to lbvmot_hal_data_t struct in HAL shmem, with all HAL data */
lbvmot_hal_data_t *lbvmot_hal_data = 0;
...
/*
  ...
  lbvmotStruct is ptr to this memory.

  lbvmotCommand points to lbvmotStruct->command,
  lbvmotStatus points to lbvmotStruct->status,
  lbvmotError points to lbvmotStruct->error, and
 */
lbvmot_struct_t *lbvmotStruct = 0;
/* ptrs to either buffered copies or direct memory for command and status */
struct lbvmot_command_t *lbvmotCommand = 0;
struct lbvmot_status_t *lbvmotStatus = 0;
struct lbvmot_config_t *lbvmotConfig = 0;
struct lbvmot_debug_t *lbvmotDebug = 0;
struct lbvmot_error_t *lbvmotError = 0;	/* unused for RT_FIFO */
...
static int lbv_shmem_id;	/* the shared memory ID */
...
void reportError(const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    lbvmotErrorPutfv(lbvmotError, fmt, args);
    va_end(args);
}
...
static void lbv_message_handler(msg_level_t level, const char *fmt, va_list ap)
{
    va_list apc;
    va_copy(apc, ap);
    if(level == RTAPI_MSG_ERR) lbvmotErrorPutfv(lbvmotError, fmt, apc);
...
int rtapi_app_main(void)
{
    ...
    if (( num_joints < 1 ) || ( num_joints > LBVMOT_MAX_JOINTS )) {
	rtapi_print_msg(RTAPI_MSG_ERR,
	    _("MOTION: num_joints is %d, must be between 1 and %d\n"), num_joints, LBVMOT_MAX_JOINTS);
    ...
    if (( num_spindles < 0 ) || ( num_spindles > LBVMOT_MAX_SPINDLES )) {
	rtapi_print_msg(RTAPI_MSG_ERR,
	    _("MOTION: num_spindles is %d, must be between 0 and %d\n"), num_spindles, LBVMOT_MAX_SPINDLES);
	hal_exit(mot_comp_id);
	return -1;
    }
    motion_num_spindles = num_spindles;

    if (( num_dio < 1 ) || ( num_dio > LBVMOT_MAX_DIO )) {
	rtapi_print_msg(RTAPI_MSG_ERR,
	    _("MOTION: num_dio is %d, must be between 1 and %d\n"), num_dio, LBVMOT_MAX_DIO);
	hal_exit(mot_comp_id);
	return -1;
    }
    
    if (( num_aio < 1 ) || ( num_aio > LBVMOT_MAX_AIO )) {
	rtapi_print_msg(RTAPI_MSG_ERR,
	    _("MOTION: num_aio is %d, must be between 1 and %d\n"), num_aio, LBVMOT_MAX_AIO);
	hal_exit(mot_comp_id);
	return -1;
    }
    ...
    rtapi_set_msg_handler(lbv_message_handler);
    return 0;
}
...
void rtapi_app_exit(void)
{
    ...
    /* free shared memory */
    retval = rtapi_shmem_delete(lbv_shmem_id, mot_comp_id);
    if (retval < 0) {
	rtapi_print_msg(RTAPI_MSG_ERR,
...
static int init_hal_io(void)
{
    ...
    lbvmot_hal_data = hal_malloc(sizeof(lbvmot_hal_data_t));
    if (lbvmot_hal_data == 0) {
	rtapi_print_msg(RTAPI_MSG_ERR, _("MOTION: lbvmot_hal_data malloc failed\n"));
	return -1;
    }

    /* export machine wide hal pins */
    if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->probe_input), mot_comp_id, "motion.probe-input")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_IN, &(lbvmot_hal_data->adaptive_feed), mot_comp_id, "motion.adaptive-feed")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->feed_hold), mot_comp_id, "motion.feed-hold")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->feed_inhibit), mot_comp_id, "motion.feed-inhibit")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->homing_inhibit), mot_comp_id, "motion.homing-inhibit")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->tp_reverse), mot_comp_id, "motion.tp-reverse")) < 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->enable), mot_comp_id, "motion.enable")) != 0) goto error;

    /* export motion-synched digital output pins */
    /* export motion digital input pins */
    for (n = 0; n < num_dio; n++) {
	if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->synch_do[n]), mot_comp_id, "motion.digital-out-%02d", n)) != 0) goto error;
	if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->synch_di[n]), mot_comp_id, "motion.digital-in-%02d", n)) != 0) goto error;
    }

    /* export motion-synched analog output pins */
    /* export motion analog input pins */
    for (n = 0; n < num_aio; n++) {
	if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->analog_output[n]), mot_comp_id, "motion.analog-out-%02d", n)) != 0) goto error;
	if ((retval = hal_pin_float_newf(HAL_IN, &(lbvmot_hal_data->analog_input[n]), mot_comp_id, "motion.analog-in-%02d", n)) != 0) goto error;
    }

    /* export machine wide hal pins */
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->motion_enabled), mot_comp_id, "motion.motion-enabled")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->in_position), mot_comp_id, "motion.in-position")) != 0) goto error;
    if ((retval = hal_pin_s32_newf(HAL_OUT, &(lbvmot_hal_data->motion_type), mot_comp_id, "motion.motion-type")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->coord_mode), mot_comp_id, "motion.coord-mode")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->teleop_mode), mot_comp_id, "motion.teleop-mode")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->coord_error), mot_comp_id, "motion.coord-error")) != 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->on_soft_limit), mot_comp_id, "motion.on-soft-limit")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->current_vel), mot_comp_id, "motion.current-vel")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->requested_vel), mot_comp_id, "motion.requested-vel")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->distance_to_go), mot_comp_id, "motion.distance-to-go")) != 0) goto error;
    if ((retval = hal_pin_s32_newf(HAL_OUT, &(lbvmot_hal_data->program_line), mot_comp_id, "motion.program-line")) != 0) goto error;
    ...
    if ((retval = hal_param_bit_newf(HAL_RO, &(lbvmot_hal_data->debug_bit_0), mot_comp_id, "motion.debug-bit-0")) != 0) goto error;
    if ((retval = hal_param_bit_newf(HAL_RO, &(lbvmot_hal_data->debug_bit_1), mot_comp_id, "motion.debug-bit-1")) != 0) goto error;
    if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->debug_float_0), mot_comp_id, "motion.debug-float-0")) != 0) goto error;
    if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->debug_float_1), mot_comp_id, "motion.debug-float-1")) != 0) goto error;
    if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->debug_float_2), mot_comp_id, "motion.debug-float-2")) != 0) goto error;
    if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->debug_float_3), mot_comp_id, "motion.debug-float-3")) != 0) goto error;
    if ((retval = hal_param_s32_newf(HAL_RO, &(lbvmot_hal_data->debug_s32_0), mot_comp_id, "motion.debug-s32-0")) != 0) goto error;
    if ((retval = hal_param_s32_newf(HAL_RO, &(lbvmot_hal_data->debug_s32_1), mot_comp_id, "motion.debug-s32-1")) != 0) goto error;

    // FIXME - debug only, remove later
    // export HAL parameters for some trajectory planner internal variables
    // so they can be scoped
    if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->traj_pos_out), mot_comp_id, "traj.pos_out")) != 0) goto error;
    if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->traj_vel_out), mot_comp_id, "traj.vel_out")) != 0) goto error;
    if ((retval = hal_param_u32_newf(HAL_RO, &(lbvmot_hal_data->traj_active_tc), mot_comp_id, "traj.active_tc")) != 0) goto error;

    for ( n = 0 ; n < 4 ; n++ ) {
        if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->tc_pos[n]), mot_comp_id, "tc.%d.pos", n)) != 0) goto error;
        if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->tc_vel[n]), mot_comp_id, "tc.%d.vel", n)) != 0) goto error;
        if ((retval = hal_param_float_newf(HAL_RO, &(lbvmot_hal_data->tc_acc[n]), mot_comp_id, "tc.%d.acc", n)) != 0) goto error;
    }
    // end of exporting trajectory planner internals

    // export timing related HAL pins so they can be scoped and/or connected
    if ((retval = hal_pin_u32_newf(HAL_OUT, &(lbvmot_hal_data->last_period), mot_comp_id, "motion.servo.last-period")) != 0) goto error;
#ifdef HAVE_CPU_KHZ
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->last_period_ns), mot_comp_id, "motion.servo.last-period-ns")) != 0) goto error;
#endif

    // export timing related HAL pins so they can be scoped
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_x), mot_comp_id, "motion.tooloffset.x")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_y), mot_comp_id, "motion.tooloffset.y")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_z), mot_comp_id, "motion.tooloffset.z")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_a), mot_comp_id, "motion.tooloffset.a")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_b), mot_comp_id, "motion.tooloffset.b")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_c), mot_comp_id, "motion.tooloffset.c")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_u), mot_comp_id, "motion.tooloffset.u")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_v), mot_comp_id, "motion.tooloffset.v")) != 0) goto error;
    if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->tooloffset_w), mot_comp_id, "motion.tooloffset.w")) != 0) goto error;

    /* initialize machine wide pins and parameters */
    *(lbvmot_hal_data->adaptive_feed) = 1.0;
    *(lbvmot_hal_data->feed_hold) = 0;
    *(lbvmot_hal_data->feed_inhibit) = 0;
    *(lbvmot_hal_data->homing_inhibit) = 0;

    *(lbvmot_hal_data->probe_input) = 0;
    /* default value of enable is TRUE, so simple machines
       can leave it disconnected */
    *(lbvmot_hal_data->enable) = 1;
    
    /* motion synched dio, init to not enabled */
    for (n = 0; n < num_dio; n++) {
	 *(lbvmot_hal_data->synch_do[n]) = 0;
	 *(lbvmot_hal_data->synch_di[n]) = 0;
    }

    for (n = 0; n < num_aio; n++) {
	 *(lbvmot_hal_data->analog_output[n]) = 0.0;
	 *(lbvmot_hal_data->analog_input[n]) = 0.0;
    }
    
    /*! \todo FIXME - these don't really need initialized, since they are written
       with data from the lbvmotStatus struct */
    *(lbvmot_hal_data->motion_enabled) = 0;
    *(lbvmot_hal_data->in_position) = 0;
    *(lbvmot_hal_data->motion_type) = 0;
    *(lbvmot_hal_data->coord_mode) = 0;
    *(lbvmot_hal_data->teleop_mode) = 0;
    *(lbvmot_hal_data->coord_error) = 0;
    *(lbvmot_hal_data->on_soft_limit) = 0;

    /* init debug parameters */
    lbvmot_hal_data->debug_bit_0 = 0;
    lbvmot_hal_data->debug_bit_1 = 0;
    lbvmot_hal_data->debug_float_0 = 0.0;
    lbvmot_hal_data->debug_float_1 = 0.0;
    lbvmot_hal_data->debug_float_2 = 0.0;
    lbvmot_hal_data->debug_float_3 = 0.0;

    *(lbvmot_hal_data->last_period) = 0;

    /* export spindle pins and params */
    for (n=0; n < num_spindles; n++) {
        retval = export_spindle(n, &(lbvmot_hal_data->spindle[n]));
        if (retval != 0){
	...
	joint_data = &(lbvmot_hal_data->joint[n]);
	/* export all vars */
	...
	*(joint_data->amp_enable) = 0;
    ...
    for (n = 0; n < num_extrajoints; n++) {
        ejoint_data = &(lbvmot_hal_data->ejoint[n]);
    ...
    for (n = 0; n < LBVMOT_MAX_AXIS; n++) {
        char c = "xyzabcuvw"[n];
        axis_data = &(lbvmot_hal_data->axis[n]);
        if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->axis[n].pos_cmd),
           mot_comp_id, "axis.%c.pos-cmd",         c)) != 0) goto error;
        if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->axis[n].teleop_vel_cmd),
           mot_comp_id, "axis.%c.teleop-vel-cmd",         c)) != 0) goto error;
        if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->axis[n].teleop_pos_cmd),
           mot_comp_id, "axis.%c.teleop-pos-cmd",  c)) != 0) goto error;
        if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->axis[n].teleop_vel_lim),
           mot_comp_id, "axis.%c.teleop-vel-lim",  c)) != 0) goto error;
        if ((retval = hal_pin_bit_newf(HAL_OUT,   &(lbvmot_hal_data->axis[n].teleop_tp_enable),
           mot_comp_id, "axis.%c.teleop-tp-enable",c)) != 0) goto error;
        if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->axis[n].eoffset_enable),
           mot_comp_id, "axis.%c.eoffset-enable", c)) != 0) return retval;
        if ((retval = hal_pin_bit_newf(HAL_IN, &(lbvmot_hal_data->axis[n].eoffset_clear),
           mot_comp_id, "axis.%c.eoffset-clear", c)) != 0) return retval;

        if ((retval = hal_pin_s32_newf(HAL_IN, &(lbvmot_hal_data->axis[n].eoffset_counts),
           mot_comp_id, "axis.%c.eoffset-counts", c)) != 0) return retval;
        if ((retval = hal_pin_float_newf(HAL_IN, &(lbvmot_hal_data->axis[n].eoffset_scale),
           mot_comp_id, "axis.%c.eoffset-scale", c)) != 0) return retval;

        if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->axis[n].external_offset),
           mot_comp_id, "axis.%c.eoffset", c)) != 0) return retval;
        if ((retval = hal_pin_float_newf(HAL_OUT, &(lbvmot_hal_data->axis[n].external_offset_requested),
    ...
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->eoffset_limited), mot_comp_id,
                  "motion.eoffset-limited")) < 0) goto error;
    if ((retval = hal_pin_bit_newf(HAL_OUT, &(lbvmot_hal_data->eoffset_active), mot_comp_id,
...
static int init_comm_buffers(void)
{
    ...
    lbv_shmem_id = rtapi_shmem_new(key, mot_comp_id, sizeof(lbvmot_struct_t));
    if (lbv_shmem_id < 0) {
	rtapi_print_msg(RTAPI_MSG_ERR,
	    "MOTION: rtapi_shmem_new failed, returned %d\n", lbv_shmem_id);
	return -1;
    }
    retval = rtapi_shmem_getptr(lbv_shmem_id, (void **) &lbvmotStruct);
    if (retval < 0) {
	rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    lbvmotCommand = &lbvmotStruct->command;
    ...
    /* init error struct */
    lbvmotErrorInit(lbvmotError);

    /* init command struct */
    lbvmotCommand->head = 0;
    lbvmotCommand->command = 0;
    lbvmotCommand->commandNum = 0;
    lbvmotCommand->tail = 0;

    /* init status struct */
    lbvmotStatus->head = 0;
    lbvmotStatus->commandEcho = 0;
    lbvmotStatus->commandNumEcho = 0;
    lbvmotStatus->commandStatus = 0;

    /* init more stuff */
    lbvmotDebug->head = 0;
    lbvmotConfig->head = 0;

    lbvmotStatus->motionFlag = 0;
    SET_MOTION_ERROR_FLAG(0);
    SET_MOTION_COORD_FLAG(0);
    SET_MOTION_TELEOP_FLAG(0);
    lbvmotDebug->split = 0;
    lbvmotStatus->heartbeat = 0;

    ALL_JOINTS                 = num_joints;      // lbvmotConfig->numJoints from [KINS]JOINTS
    lbvmotConfig->numExtraJoints = num_extrajoints; // from motmod num_extrajoints=
    lbvmotStatus->numExtraJoints = num_extrajoints;

    lbvmotConfig->numSpindles = num_spindles;
    lbvmotConfig->numDIO = num_dio;
    lbvmotConfig->numAIO = num_aio;

    ZERO_LBV_POSE(lbvmotStatus->carte_pos_cmd);
    ZERO_LBV_POSE(lbvmotStatus->carte_pos_fb);
    lbvmotStatus->vel = 0.0;
    lbvmotConfig->limitVel = 0.0;
    lbvmotStatus->acc = 0.0;
    ...
    lbvmotStatus->id = 0;
    lbvmotStatus->depth = 0;
    lbvmotStatus->activeDepth = 0;
    lbvmotStatus->paused = 0;
    lbvmotStatus->overrideLimitMask = 0;
    SET_MOTION_INPOS_FLAG(1);
    SET_MOTION_ENABLE_FLAG(0);
    /* record the kinematics type of the machine */
    lbvmotConfig->kinType = kinematicsType();
    lbvmot_config_change();

    /* init pointer to joint structs */
#ifdef STRUCTS_IN_SHMEM
    joints = &(lbvmotDebug->joints[0]);
    axes = &(lbvmotDebug->axes[0]);
#else
    joints = &(joint_array[0]);
    axes = &(axis_array[0]);
#endif

    for (spindle_num = 0; spindle_num < LBVMOT_MAX_SPINDLES; spindle_num++){
        lbvmotStatus->spindle_status[spindle_num].scale = 1.0;
        lbvmotStatus->spindle_status[spindle_num].speed = 0.0;
    }
   ...
   for (axis_num = 0; axis_num < LBVMOT_MAX_AXIS; axis_num++) {
    ...
    /*! \todo FIXME-- add lbvmotError */

    lbvmotDebug->cur_time = lbvmotDebug->last_time = 0.0;
    lbvmotDebug->start_time = etime();
    lbvmotDebug->running_time = 0.0;

    /* init motion lbvmotDebug->coord_tp */
    if (-1 == tpCreate(&lbvmotDebug->coord_tp, DEFAULT_TC_QUEUE_SIZE,
	    lbvmotDebug->queueTcSpace)) {
	rtapi_print_msg(RTAPI_MSG_ERR,
	    "MOTION: failed to create motion lbvmotDebug->coord_tp\n");
	return -1;
    }
//    tpInit(&lbvmotDebug->coord_tp); // tpInit called from tpCreate
    tpSetCycleTime(&lbvmotDebug->coord_tp, lbvmotConfig->trajCycleTime);
    tpSetPos(&lbvmotDebug->coord_tp, &lbvmotStatus->carte_pos_cmd);
    tpSetVmax(&lbvmotDebug->coord_tp, lbvmotStatus->vel, lbvmotStatus->vel);
...
static int init_threads(void)
{
    ...
    retval = hal_export_funct("motion-controller", lbvmotController, 0	/* arg 
    ...
    retval = hal_export_funct("motion-command-handler", lbvmotCommandHandler, 0	/* arg 
	 */ , 1 /* uses_fp */ , 0 /* reentrant */ , mot_comp_id);
    if (retval < 0) {
	rtapi_print_msg(RTAPI_MSG_ERR,
...
/*! \todo Another #if 0 */
    ...
    retval = hal_export_funct("motion-traj-planner", lbvmotTrajPlanner, 0	/* arg 
	 */ , 1 /* uses_fp */ ,
    ...
    // if we don't set cycle times based on these guesses, lbv doesn't
    // start up right
...
void lbvmotSetCycleTime(unsigned long nsec )
{
    int servo_mult;
...
static int setTrajCycleTime(double secs)
{
    ...
    if(lbvmotConfig->servoCycleTime)
        lbvmotConfig->interpolationRate =
            (int) (secs / lbvmotConfig->servoCycleTime + 0.5);
    else
        lbvmotConfig->interpolationRate = 1;

    /* set traj planner */
    tpSetCycleTime(&lbvmotDebug->coord_tp, secs);
    ...
    lbvmotConfig->trajCycleTime = secs;
...
static int setServoCycleTime(double secs)
{
    ...
    /* compute the interpolation rate as nearest integer to traj/servo */
    lbvmotConfig->interpolationRate =
	(int) (lbvmotConfig->trajCycleTime / secs + 0.5);
    ...
    /* copy into status out */
    lbvmotConfig->servoCycleTime = secs;
