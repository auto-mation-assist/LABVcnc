...
#define MAX_HOME_SEQUENCES LBVMOT_MAX_JOINTS
...
static  home_local_data H[LBVMOT_MAX_JOINTS];
...
typedef struct {
    one_joint_home_data_t jhd[LBVMOT_MAX_JOINTS];
...
static void home_start_move(lbvmot_joint_t * joint, double vel)
{
    double joint_range;
...
static void home_do_moving_checks(lbvmot_joint_t * joint,int jno)
{
...
void do_homing(void)
{
	...
	do {
		...
		/* set flags that communicate with the rest of LBV */
	    ...
	    case HOME_UNLOCK:
		// unlock now
		lbvmotSetRotaryUnlock(joint_num, 1);
		H[joint_num].home_state = HOME_UNLOCK_WAIT;
		break;
		...
		if ((H[joint_num].home_flags & HOME_UNLOCK_FIRST) &&
		    !lbvmotGetRotaryIsUnlocked(joint_num)) break;
	    ...
	    case HOME_LOCK:
		if (H[joint_num].home_flags & HOME_UNLOCK_FIRST) {
		    lbvmotSetRotaryUnlock(joint_num, 0);
		} else {
		    immediate_state = 1;
		}
