...
typedef struct {
    ...
    hal_bit_t *synch_do[LBVMOT_MAX_DIO]; /* WPI array: output pins for motion synched IO */
    hal_bit_t *synch_di[LBVMOT_MAX_DIO]; /* RPI array: input pins for motion synched IO */
    hal_float_t *analog_input[LBVMOT_MAX_AIO]; /* RPI array: input pins for analog Inputs */
    hal_float_t *analog_output[LBVMOT_MAX_AIO]; /* RPI array: output pins for analog Inputs */
    ...
    spindle_hal_t spindle[LBVMOT_MAX_SPINDLES];     /*spindle data */
    joint_hal_t joint[LBVMOT_MAX_JOINTS];	/* data for each joint */
    extrajoint_hal_t ejoint[LBVMOT_MAX_EXTRAJOINTS]; /* data for each extrajoint */
    axis_hal_t axis[LBVMOT_MAX_AXIS];	        /* data for each axis */
...
extern lbvmot_hal_data_t *lbvmot_hal_data;
...
extern lbvmot_joint_t *joints;

/* pointer to array of axis structs with all axis data */
extern lbvmot_axis_t *axes;
...
/* Struct pointers */
extern struct lbvmot_struct_t *lbvmotStruct;
extern struct lbvmot_command_t *lbvmotCommand;
extern struct lbvmot_status_t *lbvmotStatus;
extern struct lbvmot_config_t *lbvmotConfig;
extern struct lbvmot_debug_t *lbvmotDebug;
extern struct lbvmot_error_t *lbvmotError;


// total number of joints (typically set with [KINS]JOINTS)
#define ALL_JOINTS lbvmotConfig->numJoints

// number of kinematics-only joints:
#define NO_OF_KINS_JOINTS (ALL_JOINTS - lbvmotConfig->numExtraJoints)
...
extern void lbvmotCommandHandler(void *arg, long period);
extern void lbvmotController(void *arg, long period);
extern void lbvmotSetCycleTime(unsigned long nsec);

/* these are related to synchronized I/O */
extern void lbvmotDioWrite(int index, char value);
extern void lbvmotAioWrite(int index, double value);

extern void lbvmotSetRotaryUnlock(int axis, int unlock);
extern int lbvmotGetRotaryIsUnlocked(int axis);
...
// The real transition to Teleop mode is done in lbvmotController().
//
...
extern void refresh_jog_limits(lbvmot_joint_t *joint,int joint_num);
/* handles 'homed' flags, see command.c for details */
extern void clearHomes(int joint_num);

extern void lbvmot_config_change(void);
...
#define GET_MOTION_ERROR_FLAG() (lbvmotStatus->motionFlag & LBVMOT_MOTION_ERROR_BIT ? 1 : 0)

#define SET_MOTION_ERROR_FLAG(fl) if (fl) lbvmotStatus->motionFlag |= LBVMOT_MOTION_ERROR_BIT; else lbvmotStatus->motionFlag &= ~LBVMOT_MOTION_ERROR_BIT;

#define GET_MOTION_COORD_FLAG() (lbvmotStatus->motionFlag & LBVMOT_MOTION_COORD_BIT ? 1 : 0)

#define SET_MOTION_COORD_FLAG(fl) if (fl) lbvmotStatus->motionFlag |= LBVMOT_MOTION_COORD_BIT; else lbvmotStatus->motionFlag &= ~LBVMOT_MOTION_COORD_BIT;

#define GET_MOTION_TELEOP_FLAG() (lbvmotStatus->motionFlag & LBVMOT_MOTION_TELEOP_BIT ? 1 : 0)

#define SET_MOTION_TELEOP_FLAG(fl) if (fl) lbvmotStatus->motionFlag |= LBVMOT_MOTION_TELEOP_BIT; else lbvmotStatus->motionFlag &= ~LBVMOT_MOTION_TELEOP_BIT;

#define GET_MOTION_INPOS_FLAG() (lbvmotStatus->motionFlag & LBVMOT_MOTION_INPOS_BIT ? 1 : 0)

#define SET_MOTION_INPOS_FLAG(fl) if (fl) lbvmotStatus->motionFlag |= LBVMOT_MOTION_INPOS_BIT; else lbvmotStatus->motionFlag &= ~LBVMOT_MOTION_INPOS_BIT;

#define GET_MOTION_ENABLE_FLAG() (lbvmotStatus->motionFlag & LBVMOT_MOTION_ENABLE_BIT ? 1 : 0)

#define SET_MOTION_ENABLE_FLAG(fl) if (fl) lbvmotStatus->motionFlag |= LBVMOT_MOTION_ENABLE_BIT; else lbvmotStatus->motionFlag &= ~LBVMOT_MOTION_ENABLE_BIT;

/* joint flags */

#define GET_JOINT_ENABLE_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_ENABLE_BIT ? 1 : 0)

#define SET_JOINT_ENABLE_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_ENABLE_BIT; else (joint)->flag &= ~LBVMOT_JOINT_ENABLE_BIT;

#define GET_JOINT_ACTIVE_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_ACTIVE_BIT ? 1 : 0)

#define SET_JOINT_ACTIVE_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_ACTIVE_BIT; else (joint)->flag &= ~LBVMOT_JOINT_ACTIVE_BIT;

#define GET_JOINT_INPOS_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_INPOS_BIT ? 1 : 0)

#define SET_JOINT_INPOS_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_INPOS_BIT; else (joint)->flag &= ~LBVMOT_JOINT_INPOS_BIT;

#define GET_JOINT_ERROR_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_ERROR_BIT ? 1 : 0)

#define SET_JOINT_ERROR_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_ERROR_BIT; else (joint)->flag &= ~LBVMOT_JOINT_ERROR_BIT;

#define GET_JOINT_PHL_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_MAX_HARD_LIMIT_BIT ? 1 : 0)

#define SET_JOINT_PHL_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_MAX_HARD_LIMIT_BIT; else (joint)->flag &= ~LBVMOT_JOINT_MAX_HARD_LIMIT_BIT;

#define GET_JOINT_NHL_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_MIN_HARD_LIMIT_BIT ? 1 : 0)

#define SET_JOINT_NHL_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_MIN_HARD_LIMIT_BIT; else (joint)->flag &= ~LBVMOT_JOINT_MIN_HARD_LIMIT_BIT;


#define GET_JOINT_FERROR_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_FERROR_BIT ? 1 : 0)

#define SET_JOINT_FERROR_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_FERROR_BIT; else (joint)->flag &= ~LBVMOT_JOINT_FERROR_BIT;

#define GET_JOINT_FAULT_FLAG(joint) ((joint)->flag & LBVMOT_JOINT_FAULT_BIT ? 1 : 0)

#define SET_JOINT_FAULT_FLAG(joint,fl) if (fl) (joint)->flag |= LBVMOT_JOINT_FAULT_BIT; else (joint)->flag &= ~LBVMOT_JOINT_FAULT_BIT;
