...
*   lbvmotController() is the main loop running at the servo cycle
...
extern struct lbvmot_status_t *lbvmotStatus;

// *pcmd_p[0] is shorthand for lbvmotStatus->carte_pos_cmd.tran.x
// *pcmd_p[1] is shorthand for lbvmotStatus->carte_pos_cmd.tran.y
//  etc.
static double *pcmd_p[LBVMOT_MAX_AXIS];
...
/* 'set_operating_mode()' handles transitions between the operating
   ...
   it consists of code copied exactly from lbv1.
*/
...
/* 'compute_screw_comp()' is responsible for calculating backlash and
   ...
   the velocity in lbvmotStatus->joint_vel_cmd to determine which way
   each joint is moving, and the position in lbvmotStatus->joint_pos_cmd
...
/* 'update_status()' copies assorted status information to shared
   memory (the lbvmotStatus structure) so that it is available to
   higher level code.
*/
...
/*
  lbvmotController() runs the trajectory and interpolation calculations
  each control cycle
  ...
  */
void lbvmotController(void *arg, long period)
{
    static int do_once = 1;
    if (do_once) {
        pcmd_p[0] = &(lbvmotStatus->carte_pos_cmd.tran.x);
        pcmd_p[1] = &(lbvmotStatus->carte_pos_cmd.tran.y);
        pcmd_p[2] = &(lbvmotStatus->carte_pos_cmd.tran.z);
        pcmd_p[3] = &(lbvmotStatus->carte_pos_cmd.a);
        pcmd_p[4] = &(lbvmotStatus->carte_pos_cmd.b);
        pcmd_p[5] = &(lbvmotStatus->carte_pos_cmd.c);
        pcmd_p[6] = &(lbvmotStatus->carte_pos_cmd.u);
        pcmd_p[7] = &(lbvmotStatus->carte_pos_cmd.v);
        pcmd_p[8] = &(lbvmotStatus->carte_pos_cmd.w);
        do_once = 0;
    }
    ...
    *(lbvmot_hal_data->last_period) = this_run;
#ifdef HAVE_CPU_KHZ
    *(lbvmot_hal_data->last_period_ns) = this_run * 1e6 / cpu_khz;
#endif
...
static void process_inputs(void)
{
    ...
    for (spindle_num = 0; spindle_num < lbvmotConfig->numSpindles; spindle_num++){
		lbvmotStatus->spindle_status[spindle_num].spindleRevs =
				*lbvmot_hal_data->spindle[spindle_num].spindle_revs;
		lbvmotStatus->spindle_status[spindle_num].spindleSpeedIn =
				*lbvmot_hal_data->spindle[spindle_num].spindle_speed_in;
		lbvmotStatus->spindle_status[spindle_num].at_speed =
				*lbvmot_hal_data->spindle[spindle_num].spindle_is_atspeed;
    }
    /* compute net feed and spindle scale factors */
    if ( lbvmotStatus->motion_state == LBVMOT_MOTION_COORD ) {
    ...
    if (   (lbvmotStatus->motion_state != LBVMOT_MOTION_FREE)
        && (enables & FS_ENABLED) ) {
        if (lbvmotStatus->motionType == LBV_MOTION_TYPE_TRAVERSE) {
            scale *= lbvmotStatus->rapid_scale;
        } else {
            scale *= lbvmotStatus->feed_scale;
        }
    }
    if ( enables & AF_ENABLED ) {
        /* read and clamp adaptive feed HAL pin */
        double adaptive_feed_in = *lbvmot_hal_data->adaptive_feed;
        // Clip range to +/- 1.0
        ...
        if ( adaptive_feed_in < 0.0 && lbvmotDebug->coord_tp.reverse_run == TC_DIR_FORWARD) {
            // User commands feed in reverse direction, but we're not running in reverse yet
            if (tpSetRunDir(&lbvmotDebug->coord_tp, TC_DIR_REVERSE) != TP_ERR_OK) {
                // Need to decelerate to a stop first
                adaptive_feed_out = 0.0;
            }
        } else if (adaptive_feed_in > 0.0 && lbvmotDebug->coord_tp.reverse_run == TC_DIR_REVERSE ) {
            // User commands feed in forward direction, but we're running in reverse
            if (tpSetRunDir(&lbvmotDebug->coord_tp, TC_DIR_FORWARD) != TP_ERR_OK) {
                // Need to decelerate to a stop first
                adaptive_feed_out = 0.0;
            }
        }
	...
	if ( *lbvmot_hal_data->feed_hold ) {
	    scale = 0;
	}
    }
    /*non maskable (except during spinndle synch move) feed hold inhibit pin */
	if ( enables & *lbvmot_hal_data->feed_inhibit ) {
	    scale = 0;
	}
    /* save the resulting combined scale factor */
    lbvmotStatus->net_feed_scale = scale;

    /* now do spindle scaling */
    for (spindle_num=0; spindle_num < lbvmotConfig->numSpindles; spindle_num++){
		scale = 1.0;
		if ( enables & SS_ENABLED ) {
			scale *= lbvmotStatus->spindle_status[spindle_num].scale;
		}
		/*non maskable (except during spindle synch move) spindle inhibit pin */
		if ( enables & *lbvmot_hal_data->spindle[spindle_num].spindle_inhibit ) {
			scale = 0;
		}
		/* save the resulting combined scale factor */
		lbvmotStatus->spindle_status[spindle_num].net_scale = scale;
    }

    /* read and process per-joint inputs */
    for (joint_num = 0; joint_num < ALL_JOINTS ; joint_num++) {
	/* point to joint HAL data */
	joint_data = &(lbvmot_hal_data->joint[joint_num]);
	/* point to joint data */
    ...
    for (spindle_num = 0; spindle_num < lbvmotConfig->numSpindles; spindle_num++){
        if(*(lbvmot_hal_data->spindle[spindle_num].spindle_amp_fault)){
            lbvmotStatus->spindle_status[spindle_num].fault = 1;
        }else{
            lbvmotStatus->spindle_status[spindle_num].fault = 0;
        }
		if (*(lbvmot_hal_data->spindle[spindle_num].spindle_orient)) {
			if (*(lbvmot_hal_data->spindle[spindle_num].spindle_orient_fault)) {
				lbvmotStatus->spindle_status[spindle_num].orient_state = LBVMOT_ORIENT_FAULTED;
				*(lbvmot_hal_data->spindle[spindle_num].spindle_orient) = 0;
				lbvmotStatus->spindle_status[spindle_num].orient_fault =
						*(lbvmot_hal_data->spindle[spindle_num].spindle_orient_fault);
			...
			} else if (*(lbvmot_hal_data->spindle[spindle_num].spindle_is_oriented)) {
				*(lbvmot_hal_data->spindle[spindle_num].spindle_orient) = 0;
				*(lbvmot_hal_data->spindle[spindle_num].spindle_locked) = 1;
				lbvmotStatus->spindle_status[spindle_num].locked = 1;
				lbvmotStatus->spindle_status[spindle_num].brake = 1;
				lbvmotStatus->spindle_status[spindle_num].orient_state = LBVMOT_ORIENT_COMPLETE;
...
\todo  FIXME - maybe setting to home isn't the right thing to do.  We need
   ...
   cannot be used.  This exists for completeness only, since LBV won't
...
/*! \todo FIXME FIXME FIXME - need to put a rate divider in here, run it
   at the traj rate */
    ...
    double joint_pos[LBVMOT_MAX_JOINTS] = {0,};
    ...
    switch (lbvmotConfig->kinType) {

    case KINEMATICS_IDENTITY:
	kinematicsForward(joint_pos, &lbvmotStatus->carte_pos_fb, &fflags,
	    &iflags);
	if (checkAllHomed()) {
	    lbvmotStatus->carte_pos_fb_ok = 1;
	} else {
	    lbvmotStatus->carte_pos_fb_ok = 0;
	}
	break;
	...
	if (checkAllHomed()) {
	    ...
	    if (!lbvmotStatus->carte_pos_fb_ok) {
		/* no, use home position as initial guess */
		lbvmotStatus->carte_pos_fb = lbvmotStatus->world_home;
	    }
	    /* calculate Cartesean position feedback from joint pos fb */
	    result =
		kinematicsForward(joint_pos, &lbvmotStatus->carte_pos_fb,
		&fflags, &iflags);
	...
	if ((GET_MOTION_COORD_FLAG()) || (GET_MOTION_TELEOP_FLAG())) {
	    ...
	    lbvmotStatus->carte_pos_fb = lbvmotStatus->carte_pos_cmd;
	    lbvmotStatus->carte_pos_fb_ok = 1;
	} else {
	    lbvmotStatus->carte_pos_fb_ok = 0;
	}
...
static void process_probe_inputs(void)
{
    ...
    unsigned char probe_type = lbvmotStatus->probe_type;
    ...
    /* read probe input */
    lbvmotStatus->probeVal = !!*(lbvmot_hal_data->probe_input);
    if (lbvmotStatus->probing) {
        /* check if the probe has been tripped */
        if (lbvmotStatus->probeVal ^ probe_whenclears) {
            /* remember the current position */
            lbvmotStatus->probedPos = lbvmotStatus->carte_pos_fb; 
            /* stop! */
            lbvmotStatus->probing = 0;
            lbvmotStatus->probeTripped = 1;
            tpAbort(&lbvmotDebug->coord_tp);
        /* check if the probe hasn't tripped, but the move finished */
        } else if (GET_MOTION_INPOS_FLAG() && tpQueueDepth(&lbvmotDebug->coord_tp) == 0) {
    ...
    } else if (!old_probeVal && lbvmotStatus->probeVal) {
        // not probing, but we have a rising edge on the probe.
        // this could be expensive if we don't stop.
        int i;
        int aborted = 0;

        if(!GET_MOTION_INPOS_FLAG() && tpQueueDepth(&lbvmotDebug->coord_tp) &&
           tpGetExecId(&lbvmotDebug->coord_tp) <= 0) {
    ...
    old_probeVal = lbvmotStatus->probeVal;
}
...
static void check_for_faults(void)
{
    ...
    for (spindle_num = 0; spindle_num < lbvmotConfig->numSpindles; spindle_num++){
        if(lbvmotStatus->spindle_status[spindle_num].fault && GET_MOTION_ENABLE_FLAG()){
...
static void set_operating_mode(void)
{
    ...
    /* check for disabling */
    if (!lbvmotDebug->enabling && GET_MOTION_ENABLE_FLAG()) {
	/* clear out the motion lbvmotDebug->coord_tp and interpolators */
	tpClear(&lbvmotDebug->coord_tp);
	for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
	    /* point to joint data */
	    joint = &joints[joint_num];
    ...
    /* check for lbvmotDebug->enabling */
    if (lbvmotDebug->enabling && !GET_MOTION_ENABLE_FLAG()) {
        if (*(lbvmot_hal_data->eoffset_limited)) {
            reportError("Starting beyond Soft Limits");
            *(lbvmot_hal_data->eoffset_limited) = 0;
        }
        initialize_external_offsets();
        tpSetPos(&lbvmotDebug->coord_tp, &lbvmotStatus->carte_pos_cmd);
	for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
	    /* point to joint data */
	    joint = &joints[joint_num];
    ...
    if (lbvmotDebug->teleoperating && !GET_MOTION_TELEOP_FLAG()) {
	if (GET_MOTION_INPOS_FLAG()) {

	    /* update coordinated lbvmotDebug->coord_tp position */
	    tpSetPos(&lbvmotDebug->coord_tp, &lbvmotStatus->carte_pos_cmd);
	    /* drain the cubics so they'll synch up */
	    for (joint_num = 0; joint_num < LBVMOT_MAX_JOINTS; joint_num++) {
	    ...
	    SET_MOTION_ERROR_FLAG(0);
            ...
            kinematicsForward(positions, &lbvmotStatus->carte_pos_cmd, &fflags, &iflags);
	...
	if (lbvmotDebug->coordinating && !GET_MOTION_COORD_FLAG()) {
	    if (GET_MOTION_INPOS_FLAG()) {
...
static void handle_jjogwheels(void)
{
	...
	if ( *(joint_data->jjog_vel_mode) ) {
            double v = joint->vel_limit * lbvmotStatus->net_feed_scale;
...
static void handle_ajogwheels(void)
{
    ...
    if ( lbvmotStatus->on_soft_limit ) { return; }

    for (axis_num = 0; axis_num < LBVMOT_MAX_AXIS; axis_num++) {
        double aaccel_limit;
        axis = &axes[axis_num];
        axis_data = &(lbvmot_hal_data->axis[axis_num]);
...
static void get_pos_cmds(long period)
{
    ...
    int onlimit = 0;
    int joint_limit[LBVMOT_MAX_JOINTS][2];
    ...
    while ( joint_num < LBVMOT_MAX_JOINTS ) {
    ...
    switch ( lbvmotStatus->motion_state) {
    case LBVMOT_MOTION_FREE:
	...
	for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
	    ...
	    if(joint->acc_limit > lbvmotStatus->acc)
		joint->acc_limit = lbvmotStatus->acc;
	    /* compute joint velocity limit */
            if ( get_home_is_idle(joint_num) ) {
                ...
                vel_lim = joint->vel_limit * lbvmotStatus->net_feed_scale;
		...
		if ( lbvmotStatus->overrideLimitMask ) {
                    lbvmotDebug->overriding = 1;
		}
            } else {
		SET_JOINT_INPOS_FLAG(joint, 1);
	...
	if ( (lbvmotDebug->overriding ) && ( GET_MOTION_INPOS_FLAG() ) ) {
	    lbvmotStatus->overrideLimitMask = 0;
	    lbvmotDebug->overriding = 0;
	}
	...
	case KINEMATICS_IDENTITY:
	    kinematicsForward(positions, &lbvmotStatus->carte_pos_cmd, &fflags, &iflags);
	    if (checkAllHomed()) {
		lbvmotStatus->carte_pos_cmd_ok = 1;
	    } else {
		lbvmotStatus->carte_pos_cmd_ok = 0;
	    }
	    break;
	...
	case KINEMATICS_BOTH:
	    if (checkAllHomed()) {
		/* is previous value suitable for use as initial guess? */
		if (!lbvmotStatus->carte_pos_cmd_ok) {
		    /* no, use home position as initial guess */
		    lbvmotStatus->carte_pos_cmd = lbvmotStatus->world_home;
		}
	...
	while (cubicNeedNextPoint(&(joints[0].cubic))) {
	    ...
	    tpRunCycle(&lbvmotDebug->coord_tp, period);
            /* get new commanded traj pos */
            tpGetPos(&lbvmotDebug->coord_tp, &lbvmotStatus->carte_pos_cmd);
	    ...
	    result = kinematicsInverse(&lbvmotStatus->carte_pos_cmd, positions,
		&iflags, &fflags);
	    if(result == 0)
	    {
	...
	SET_MOTION_INPOS_FLAG(0);
	if (tpIsDone(&lbvmotDebug->coord_tp)) {
	    SET_MOTION_INPOS_FLAG(1);
	}
	break;

    case LBVMOT_MOTION_TELEOP:
        for (axis_num = 0; axis_num < LBVMOT_MAX_AXIS; axis_num++) {
            axis = &axes[axis_num];
	...
	/* OUTPUT KINEMATICS - convert to joints in local array */
	result = kinematicsInverse(&lbvmotStatus->carte_pos_cmd, positions, &iflags, &fflags);
    ...
    */
	...
	if ( ! lbvmotStatus->on_soft_limit ) {
	    ...
	    SET_MOTION_ERROR_FLAG(1);
	    lbvmotStatus->on_soft_limit = 1;
	}
    } else {
	lbvmotStatus->on_soft_limit = 0;
    }
    if (   lbvmotDebug->teleoperating
        && GET_MOTION_TELEOP_FLAG()
        && lbvmotStatus->on_soft_limit ) {
        SET_MOTION_ERROR_FLAG(1);
        for (axis_num = 0; axis_num < LBVMOT_MAX_AXIS; axis_num++) {
            axis = &axes[axis_num];
            axis->teleop_tp.enable = 0;
    ...
    if (ext_offset_teleop_limit || ext_offset_coord_limit) {
        *(lbvmot_hal_data->eoffset_limited) = 1;
    } else {
        *(lbvmot_hal_data->eoffset_limited) = 0;
    }
} // get_pos_cmds()
...
1) lbvmotStatus->carte_pos_cmd
2) lbvmotStatus->joints[n].coarse_pos
3) lbvmotStatus->joints[n].pos_cmd
4) lbvmotStatus->joints[n].motor_pos_cmd
5) lbvmotStatus->joints[n].motor_pos_fb
6) lbvmotStatus->joints[n].pos_fb
7) lbvmotStatus->carte_pos_fb
...
static void compute_screw_comp(void)
{
	 ...
	 */ 
        v_max = 0.5 * joint->vel_limit * lbvmotStatus->net_feed_scale;
...
static void output_to_hal(void)
{
    ...
    joint_hal_t *joint_data;
    axis_hal_t *axis_data;
    static int old_motion_index[LBVMOT_MAX_SPINDLES] = {0};
    static int old_hal_index[LBVMOT_MAX_SPINDLES] = {0};

    /* output machine info to HAL for scoping, etc */
    *(lbvmot_hal_data->motion_enabled) = GET_MOTION_ENABLE_FLAG();
    *(lbvmot_hal_data->in_position) = GET_MOTION_INPOS_FLAG();
    *(lbvmot_hal_data->coord_mode) = GET_MOTION_COORD_FLAG();
    *(lbvmot_hal_data->teleop_mode) = GET_MOTION_TELEOP_FLAG();
    *(lbvmot_hal_data->coord_error) = GET_MOTION_ERROR_FLAG();
    *(lbvmot_hal_data->on_soft_limit) = lbvmotStatus->on_soft_limit;

    for (spindle_num = 0; spindle_num < lbvmotConfig->numSpindles; spindle_num++){
		if(lbvmotStatus->spindle_status[spindle_num].css_factor) {
			double denom = fabs(lbvmotStatus->spindle_status[spindle_num].xoffset
								- lbvmotStatus->carte_pos_cmd.tran.x);
			double speed;
			double maxpositive;
			if(denom > 0) speed = lbvmotStatus->spindle_status[spindle_num].css_factor / denom;
			else speed = lbvmotStatus->spindle_status[spindle_num].speed;

			speed = speed * lbvmotStatus->spindle_status[spindle_num].net_scale;
				maxpositive = fabs(lbvmotStatus->spindle_status[spindle_num].speed);
				// cap speed to G96 D...
			...
			*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out) = speed;
			*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out_rps) = speed/60.;
		} else {
			*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out) =
					lbvmotStatus->spindle_status[spindle_num].speed *
					lbvmotStatus->spindle_status[spindle_num].net_scale;
			*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out_rps) =
					lbvmotStatus->spindle_status[spindle_num].speed *
					lbvmotStatus->spindle_status[spindle_num].net_scale / 60.;
		}
		*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out_abs) =
				fabs(*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out));
		*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out_rps_abs) =
				fabs(*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_out_rps));
		*(lbvmot_hal_data->spindle[spindle_num].spindle_speed_cmd_rps) =
				lbvmotStatus->spindle_status[spindle_num].speed / 60.;
		*(lbvmot_hal_data->spindle[spindle_num].spindle_on) =
				((lbvmotStatus->spindle_status[spindle_num].speed *
						lbvmotStatus->spindle_status[spindle_num].net_scale) != 0) ? 1 : 0;
		*(lbvmot_hal_data->spindle[spindle_num].spindle_forward) =
				(*lbvmot_hal_data->spindle[spindle_num].spindle_speed_out > 0) ? 1 : 0;
		*(lbvmot_hal_data->spindle[spindle_num].spindle_reverse) =
				(*lbvmot_hal_data->spindle[spindle_num].spindle_speed_out < 0) ? 1 : 0;
		*(lbvmot_hal_data->spindle[spindle_num].spindle_brake) =
				(lbvmotStatus->spindle_status[spindle_num].brake != 0) ? 1 : 0;
    }

    *(lbvmot_hal_data->program_line) = lbvmotStatus->id;
    *(lbvmot_hal_data->tp_reverse) = lbvmotStatus->reverse_run;
    *(lbvmot_hal_data->motion_type) = lbvmotStatus->motionType;
    *(lbvmot_hal_data->distance_to_go) = lbvmotStatus->distance_to_go;
    if(GET_MOTION_COORD_FLAG()) {
        *(lbvmot_hal_data->current_vel) = lbvmotStatus->current_vel;
        *(lbvmot_hal_data->requested_vel) = lbvmotStatus->requested_vel;
    } else if (GET_MOTION_TELEOP_FLAG()) {
        int i;
        double v2 = 0.0;
        for(i=0; i < LBVMOT_MAX_AXIS; i++)
            if(axes[i].teleop_tp.active)
                v2 += axes[i].teleop_vel_cmd * axes[i].teleop_vel_cmd;
        if(v2 > 0.0)
            lbvmotStatus->current_vel = (*lbvmot_hal_data->current_vel) = sqrt(v2);
        else
            lbvmotStatus->current_vel = (*lbvmot_hal_data->current_vel) = 0.0;
        *(lbvmot_hal_data->requested_vel) = 0.0;
    } else {
        int i;
        double v2 = 0.0;
    ...
    /* Change the following lines to assign the variable you want to observe
       ...
       isn't in scope here. */
    lbvmot_hal_data->debug_bit_0 = joints[1].free_tp.active;
    lbvmot_hal_data->debug_bit_1 = lbvmotStatus->enables_new & AF_ENABLED;
    lbvmot_hal_data->debug_float_0 = lbvmotStatus->spindle_status[0].speed;
    lbvmot_hal_data->debug_float_1 = lbvmotStatus->spindleSync;
    lbvmot_hal_data->debug_float_2 = lbvmotStatus->vel;
    lbvmot_hal_data->debug_float_3 = lbvmotStatus->spindle_status[0].net_scale;
    lbvmot_hal_data->debug_s32_0 = lbvmotStatus->overrideLimitMask;
    lbvmot_hal_data->debug_s32_1 = lbvmotStatus->tcqlen;

    /* two way handshaking for the spindle encoder */
    for (spindle_num = 0; spindle_num < lbvmotConfig->numSpindles; spindle_num++){
		if(lbvmotStatus->spindle_status[spindle_num].spindle_index_enable
				&& !old_motion_index[spindle_num]) {
			*lbvmot_hal_data->spindle[spindle_num].spindle_index_enable = 1;
			rtapi_print_msg(RTAPI_MSG_DBG, "setting index-enable on spindle %d\n", spindle_num);
		}

		if(!*lbvmot_hal_data->spindle[spindle_num].spindle_index_enable
				&& old_hal_index[spindle_num]) {
			lbvmotStatus->spindle_status[spindle_num].spindle_index_enable = 0;
		}

		old_motion_index[spindle_num] =
				lbvmotStatus->spindle_status[spindle_num].spindle_index_enable;
		old_hal_index[spindle_num] =
				*lbvmot_hal_data->spindle[spindle_num].spindle_index_enable;
    }

    *(lbvmot_hal_data->tooloffset_x) = lbvmotStatus->tool_offset.tran.x;
    *(lbvmot_hal_data->tooloffset_y) = lbvmotStatus->tool_offset.tran.y;
    *(lbvmot_hal_data->tooloffset_z) = lbvmotStatus->tool_offset.tran.z;
    *(lbvmot_hal_data->tooloffset_a) = lbvmotStatus->tool_offset.a;
    *(lbvmot_hal_data->tooloffset_b) = lbvmotStatus->tool_offset.b;
    *(lbvmot_hal_data->tooloffset_c) = lbvmotStatus->tool_offset.c;
    *(lbvmot_hal_data->tooloffset_u) = lbvmotStatus->tool_offset.u;
    *(lbvmot_hal_data->tooloffset_v) = lbvmotStatus->tool_offset.v;
    *(lbvmot_hal_data->tooloffset_w) = lbvmotStatus->tool_offset.w;
	...
	*(joint_data->faulted) = GET_JOINT_FAULT_FLAG(joint);
	...
	if (IS_EXTRA_JOINT(joint_num) && get_homed(joint_num)) {
	    ...
	    ejoint_data = &(lbvmot_hal_data->ejoint[e]);
...
static void update_status(void)
{
    ...
    lbvmot_axis_t *axis;
    lbvmot_axis_status_t *axis_status;
#ifdef WATCH_FLAGS
	...
	axis_status = &(lbvmotStatus->axis_status[axis_num]);
    ...
    lbvmotStatus->eoffset_pose.tran.x = (&axes[0])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.tran.y = (&axes[1])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.tran.z = (&axes[2])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.a      = (&axes[3])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.b      = (&axes[4])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.c      = (&axes[5])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.u      = (&axes[6])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.v      = (&axes[7])->ext_offset_tp.curr_pos;
    lbvmotStatus->eoffset_pose.w      = (&axes[8])->ext_offset_tp.curr_pos;

    lbvmotStatus->external_offsets_applied = *(lbvmot_hal_data->eoffset_active);

    for (dio = 0; dio < lbvmotConfig->numDIO; dio++) {
	lbvmotStatus->synch_di[dio] = *(lbvmot_hal_data->synch_di[dio]);
	lbvmotStatus->synch_do[dio] = *(lbvmot_hal_data->synch_do[dio]);
    }

    for (aio = 0; aio < lbvmotConfig->numAIO; aio++) {
	lbvmotStatus->analog_input[aio] = *(lbvmot_hal_data->analog_input[aio]);
	lbvmotStatus->analog_output[aio] = *(lbvmot_hal_data->analog_output[aio]);
    }

    /*! \todo FIXME - the rest of this function is stuff that was apparently
       dropped in the initial move from lbvmot.c to control.c.  I
       don't know how much is still needed, and how much is baggage.
    */

    /* motion lbvmotDebug->coord_tp status */
    lbvmotStatus->depth = tpQueueDepth(&lbvmotDebug->coord_tp);
    lbvmotStatus->activeDepth = tpActiveDepth(&lbvmotDebug->coord_tp);
    lbvmotStatus->id = tpGetExecId(&lbvmotDebug->coord_tp);
    //KLUDGE add an API call for this
    lbvmotStatus->reverse_run = lbvmotDebug->coord_tp.reverse_run;
    lbvmotStatus->motionType = tpGetMotionType(&lbvmotDebug->coord_tp);
    lbvmotStatus->queueFull = tcqFull(&lbvmotDebug->coord_tp.queue);

    /* check to see if we should pause in order to implement
       single lbvmotDebug->stepping */

    if (lbvmotDebug->stepping && lbvmotDebug->idForStep != lbvmotStatus->id) {
...
#ifdef WATCH_FLAGS
    /*! \todo FIXME - this is for debugging */
    if ( old_motion_flag != lbvmotStatus->motionFlag ) {
	rtapi_print ( "Motion flag %04X -> %04X\n", old_motion_flag, lbvmotStatus->motionFlag );
	old_motion_flag = lbvmotStatus->motionFlag;
    }
#endif
}
...
static void plan_external_offsets(void)
{
    ...
    static int last_eoffset_enable[LBVMOT_MAX_AXIS];

    *(lbvmot_hal_data->eoffset_active) = 0; //set if any enabled
    ...
    for (axis_num = 0; axis_num < LBVMOT_MAX_AXIS; axis_num++) {
        ...
        *(axis_data->external_offset)  = axis->ext_offset_tp.curr_pos;
        ...
        if (fabs(*(axis_data->external_offset)) > ext_offset_epsilon) {
           *(lbvmot_hal_data->eoffset_active) = 1;
        }
...
static int update_teleop_with_check(int axis_num,simple_tp_t *the_tp)
{
    ...
    lbvmot_axis_t *axis = &axes[axis_num];
...
static int update_coord_with_bound(void)
{
    ...
    int ans = 0;
    lbvmot_axis_t *axis;
    double save_pos_cmd[LBVMOT_MAX_AXIS];
    double save_offset_cmd[LBVMOT_MAX_AXIS];
