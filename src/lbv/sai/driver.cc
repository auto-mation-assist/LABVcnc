...
rs274 interpreter from the LBV system.
...
This emulates the way the LBV system uses the interpreter.
...
int main (int argc, char ** argv)
{
  ...
  char default_name[] = "/etc/lbv2/sample-configs/sim/sim.var";
  int print_stack;
  int go_flag;
  char *inifile = NULL;
  int log_level = -1;
...
usage:
  ...
  if (tool_flag == 0)
    {
      if (read_tool_file(LBV2_DEFAULT_TOOLTABLE) != 0)
        exit(1);
    }
