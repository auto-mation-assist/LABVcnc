...
int ini_hal_init(int numjoints)
{
    ...
    for (int idx = 0; idx < LBVMOT_MAX_AXIS; idx++) {
...
int check_ini_hal_items(int numjoints)
{
    ...
    if (CHANGED(traj_default_velocity)) {
        ...
        if (0 != lbvTrajSetVelocity(0, NEW(traj_default_velocity))) {
            rcs_print("check_ini_hal_items:bad return value from lbvTrajSetVelocity\n");
        }
    }
    if (CHANGED(traj_max_velocity)) {
        if (debug) SHOW_CHANGE(traj_max_velocity)
        UPDATE(traj_max_velocity);
        if (0 != lbvTrajSetMaxVelocity(NEW(traj_max_velocity))) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print("check_ini_hal_items:bad return value from lbvTrajSetMaxVelocity\n");
            }
        }
    }
    if (CHANGED(traj_default_acceleration)) {
        ...
        if (0 != lbvTrajSetAcceleration(NEW(traj_default_acceleration))) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print("check_ini_hal_items:bad return value from lbvTrajSetAcceleration\n");
            }
        }
    }
    if (CHANGED(traj_max_acceleration)) {
        ...
        if (0 != lbvTrajSetMaxAcceleration(NEW(traj_max_acceleration))) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print("check_ini_hal_items:bad return value from lbvTrajSetMaxAcceleration\n");
            }
        }
    }
    ...
    if (   CHANGED(traj_arc_blend_enable)
        ...
        if (0 != lbvSetupArcBlends(old_inihal_data.traj_arc_blend_enable
                                  ,old_inihal_data.traj_arc_blend_fallback_enable
                                  ,old_inihal_data.traj_arc_blend_optimization_depth
                                  ,old_inihal_data.traj_arc_blend_gap_cycles
                                  ,old_inihal_data.traj_arc_blend_ramp_freq
            ...
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print("bad return value from lbvSetupArcBlends\n");
            }
            return -1;
        }
    }
    for (int idx = 0; idx < numjoints; idx++) {
        if (CHANGED_IDX(joint_backlash,idx) ) {
            ...
            if (0 != lbvJointSetBacklash(idx,NEW(joint_backlash[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print("check_ini_hal_items:bad return value from lbvJointSetBacklash\n");
                }
        }
        }
        if (CHANGED_IDX(joint_min_limit,idx) ) {
            ...
            if (0 != lbvJointSetMinPositionLimit(idx,NEW(joint_min_limit[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointSetMinPositionLimit\n");
                }
            }
        }
        if (CHANGED_IDX(joint_max_limit,idx) ) {
            ...
            if (0 != lbvJointSetMaxPositionLimit(idx,NEW(joint_max_limit[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointSetMaxPositionLimit\n");
                }
            }
        }
        if (CHANGED_IDX(joint_max_velocity,idx) ) {
            ...
            if (0 != lbvJointSetMaxVelocity(idx, NEW(joint_max_velocity[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointSetMaxVelocity\n");
                }
            }
        }
        if (CHANGED_IDX(joint_max_acceleration,idx) ) {
            ...
            if (0 != lbvJointSetMaxAcceleration(idx, NEW(joint_max_acceleration[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointSetMaxAcceleration\n");
                }
            }
        }
        if (   CHANGED_IDX(joint_home,idx)
            ...
            if  (0 != lbvJointUpdateHomingParams(idx, NEW(joint_home[idx]),
                                                      NEW(joint_home_offset[idx]),
                                                      NEW(joint_home_sequence[idx]))
                ) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointUpdateHomingParams\n");
                }
            }
        }
        if (CHANGED_IDX(joint_ferror,idx) ) {
            ...
            if (0 != lbvJointSetFerror(idx,NEW(joint_ferror[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointSetFerror\n");
                }
            }
        }
        if (CHANGED_IDX(joint_min_ferror,idx) ) {
            ...
            if (0 != lbvJointSetMinFerror(idx,NEW(joint_min_ferror[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvJointSetMinFerror\n");
                }
        }
        }
    } // numjoints
    ...
    for (int idx = 0; idx < LBVMOT_MAX_AXIS; idx++) {
        if (CHANGED_IDX(axis_min_limit,idx) ) {
            ...
            if (0 != lbvAxisSetMinPositionLimit(idx,NEW(axis_min_limit[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvAxisSetMinPositionLimit\n");
                }
            }
        }
        if (CHANGED_IDX(axis_max_limit,idx) ) {
            ...
            if (0 != lbvAxisSetMaxPositionLimit(idx,NEW(axis_max_limit[idx]))) {
                if (lbv_debug & LBV_DEBUG_CONFIG) {
                    rcs_print_error("check_ini_hal_items:bad return from lbvAxisSetMaxPositionLimit\n");
                }
            }
        }
