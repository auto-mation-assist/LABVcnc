...
#include "lbvglb.h"		// LBV_DEBUG
#include "lbvcfg.h"		// default values for globals
...
/*
  ...
  lbvJointSetType(int joint, unsigned char jointType);
  lbvJointSetUnits(int joint, double units);
  lbvJointSetBacklash(int joint, double backlash);
  lbvJointSetMinPositionLimit(int joint, double limit);
  lbvJointSetMaxPositionLimit(int joint, double limit);
  lbvJointSetFerror(int joint, double ferror);
  lbvJointSetMinFerror(int joint, double ferror);
  lbvJointSetHomingParams(int joint, double home, double offset, double home_vel, 
                          double search_vel, double latch_vel,
                          int use_index, int encoder_does_not_reset,
                          int ignore_limits, int is_shared, int sequence, int volatile_home));
  lbvJointActivate(int joint);
  lbvJointSetMaxVelocity(int joint, double vel);
  lbvJointSetMaxAcceleration(int joint, double acc);
  lbvJointLoadComp(int joint, const char * file, int comp_file_type);
  */

static int loadJoint(int joint, LbvIniFile *jointIniFile)
{
    char jointString[16];
    const char *inistring;
    LbvJointType jointType;
    double units;
    double backlash;
    double offset;
    double limit;
    ...
    jointIniFile->EnableExceptions(LbvIniFile::ERR_CONVERSION);
    
    try {
        // set joint type
        jointType = LBV_LINEAR;	// default
        jointIniFile->Find(&jointType, "TYPE", jointString);
        if (0 != lbvJointSetType(joint, jointType)) {
            return -1;
        }

        // set units
        if(jointType == LBV_LINEAR){
            units = lbvTrajGetLinearUnits();
        }else{
            units = lbvTrajGetAngularUnits();
        }
        if (0 != lbvJointSetUnits(joint, units)) {
            return -1;
        }
        ...
        if (0 != lbvJointSetBacklash(joint, backlash)) {
            return -1;
        }
        ...
        if (0 != lbvJointSetMinPositionLimit(joint, limit)) {
             return -1;
        }
        ...
        if (0 != lbvJointSetMaxPositionLimit(joint, limit)) {
            return -1;
        }
        ...
        if (0 != lbvJointSetFerror(joint, ferror)) {
             return -1;
        }
        ...
        if (0 != lbvJointSetMinFerror(joint, ferror)) {
            return -1;
        }
        ...
        if (0 != lbvJointSetHomingParams(joint, home, offset
        ...
        if (0 != lbvJointSetMaxVelocity(joint, maxVelocity)) {
            return -1;
        }
        ...
        if (0 != lbvJointSetMaxAcceleration(joint, maxAcceleration)) {
            return -1;
        }
        ...
        if (NULL != (inistring = jointIniFile->Find("COMP_FILE", jointString))) {
            if (0 != lbvJointLoadComp(joint, inistring, comp_file_type)) {
                return -1;
            }
        }
    }

    catch (LbvIniFile::Exception &e) {
        e.Print();
        return -1;
    }
...
/*
  ...
  Looks for [KINS]JOINTS for how many to do, up to LBV_JOINT_MAX.
 */
int iniJoint(int joint, const char *filename)
{
    LbvIniFile jointIniFile(LbvIniFile::ERR_TAG_NOT_FOUND |
                           LbvIniFile::ERR_SECTION_NOT_FOUND |
                           LbvIniFile::ERR_CONVERSION);
