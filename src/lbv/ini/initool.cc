...
#include "lbvglb.h"		// TOOL_TABLE_FILE

/*
  loadTool()

  Loads ini file params for spindle from [LBVIO] section

  TOOL_TABLE <file name>  name of tool table file

  calls:

  lbvToolSetToolTableFile(const char filename);
  */
...
static int loadTool(IniFile *toolInifile)
{
    ...
    if (NULL != (inistring = toolInifile->Find("TOOL_TABLE", "LBVIO"))) {
	if (0 != lbvToolSetToolTableFile(inistring)) {
	    rcs_print("bad return value from lbvToolSetToolTableFile\n");
	    retval = -1;
	}
    }
...
/*
  readToolChange() reads the values of [LBVIO] TOOL_CHANGE_POSITION and
...
static int readToolChange(IniFile *toolInifile)
{
    ...
    if (NULL !=
	(inistring = toolInifile->Find("TOOL_CHANGE_POSITION", "LBVIO"))) {
	/* found an entry */
...
/*
  ...
  Loads ini file parameters for tool controller, from [LBVIO] section
 */
...
int lbvToolSetToolTableFile(const char *filename)
{
