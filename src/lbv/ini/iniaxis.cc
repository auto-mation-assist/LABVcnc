...
double ext_offset_a_or_v_ratio[LBVMOT_MAX_AXIS]; // all zero
...
/*
  ...
  lbvAxisSetMinPositionLimit(int axis, double limit);
  lbvAxisSetMaxPositionLimit(int axis, double limit);
  lbvAxisSetMaxVelocity(int axis, double vel, double ext_offset_vel);
  lbvAxisSetMaxAcceleration(int axis, double acc, double ext_offset_acc);
  */

static int loadAxis(int axis, LbvIniFile *axisIniFile)
{
    char axisString[16];
    double limit;
    double maxVelocity;
    ...
    axisIniFile->EnableExceptions(LbvIniFile::ERR_CONVERSION);
    
    try {
        // set min position limit
        limit = -1e99;	                // default
        axisIniFile->Find(&limit, "MIN_LIMIT", axisString);
        if (0 != lbvAxisSetMinPositionLimit(axis, limit)) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print_error("bad return from lbvAxisSetMinPositionLimit\n");
            }
            return -1;
        }
        ...
        if (0 != lbvAxisSetMaxPositionLimit(axis, limit)) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print_error("bad return from lbvAxisSetMaxPositionLimit\n");
            }
            return -1;
        }
...
#define MAX_AV_RATIO     0.9
        ...
        if (0 != lbvAxisSetMaxVelocity(axis,
                   (1 - ext_offset_a_or_v_ratio[axis]) * maxVelocity,
                   (    ext_offset_a_or_v_ratio[axis]) * maxVelocity)) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print_error("bad return from lbvAxisSetMaxVelocity\n");
            }
            return -1;
        }
        ...
        if (0 != lbvAxisSetMaxAcceleration(axis,
                    (1 - ext_offset_a_or_v_ratio[axis]) * maxAcceleration,
                    (    ext_offset_a_or_v_ratio[axis]) * maxAcceleration)) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print_error("bad return from lbvAxisSetMaxAcceleration\n");
            }
            return -1;
        }
        ...
        if (0 != lbvAxisSetLockingJoint(axis, lockingjnum)) {
            if (lbv_debug & LBV_DEBUG_CONFIG) {
                rcs_print_error("bad return from lbvAxisSetLockingJoint\n");
            }
            return -1;
        }
    }
...
{
    LbvIniFile axisIniFile(LbvIniFile::ERR_TAG_NOT_FOUND |
