...
#***********
    ...
    def start_finish(self):
        if self.w.importmach.get_active():
            ...
            filter.set_name(_("LabvCNC 'stepconf' configuration files"))
            ...
            if not self.d._lastconfigname == "" and self.d._chooselastconfig:
                dialog.set_filename(os.path.expanduser("~/labvcnc/configs/%s.stepconf"% self.d._lastconfigname))
    ...
    def on_machinename_changed(self, *args):
        ...
        self.w.confdir.set_text("~/labvcnc/configs/%s" % temp.replace(" ","_"))
...
#***************
    def options_prepare(self):
        ...
        if  not self.w.createconfig.get_active():
           if os.path.exists(os.path.expanduser("~/labvcnc/configs/%s/custompanel.xml" % self.d.machinename)):
        ...
        if  not self.w.createconfig.get_active():
           if os.path.exists(os.path.expanduser("~/labvcnc/configs/%s/custom.clp" % self.d.machinename)):
