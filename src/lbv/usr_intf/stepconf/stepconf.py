...
# -*- encoding: utf-8 -*-
#
#    This is stepconf, a graphical configuration editor for LabvCNC
...
datadir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "share", "labvcnc","stepconf")
main_datadir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "share", "labvcnc")
wizard = os.path.join(datadir, "labvcnc-wizard.gif")
if not os.path.isfile(wizard):
    wizard = os.path.join(main_datadir, "labvcnc-wizard.gif")
if not os.path.isfile(wizard):
    print "cannot find labvcnc-wizard.gif, looked in %s and %s" % (datadir, main_datadir)
    sys.exit(1)

icondir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..")
labvcncicon = os.path.join(icondir, "labvcncicon.png")
if not os.path.isfile(labvcncicon):
    labvcncicon = os.path.join("/etc/labvcnc/labvcnc-wizard.gif")
if not os.path.isfile(labvcncicon):
    labvcncicon = os.path.join("/usr/share/labvcnc/labvcncicon.png")

distdir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "configs", "common")
if not os.path.isdir(distdir):
    distdir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "share", "doc", "labvcnc", "sample-configs", "common")
if not os.path.isdir(distdir):
    distdir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "labvcnc", "sample-configs", "common")
if not os.path.isdir(distdir):
    distdir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "share", "doc", "labvcnc", "examples", "sample-configs", "common")
if not os.path.isdir(distdir):
    distdir = "/usr/share/doc/labvcnc/examples/sample-configs/common"
...
makedirs(os.path.expanduser("~/labvcnc/configs"))
...
class Private_Data:
    def __init__(self):
        ...
        self.MESS_NO_REALTIME = _("You are using a simulated-realtime version of LabvCNC, so testing / tuning of hardware is unavailable.")
        self.MESS_KERNEL_WRONG = _("You are using a realtime version of LabvCNC but didn't load a realtime kernel so testing / tuning of hardware is\
    ...
    def save(self,basedir):
        ...
        if self.classicladder: 
           if not self.laddername == "custom.clp":
                ...
                original = os.path.expanduser("~/labvcnc/configs/%s/custom.clp" % self.machinename)
                if os.path.exists(filename):     
                  if os.path.exists(original):
                     print "custom file already exists"
                     shutil.copy( original,os.path.expanduser("~/labvcnc/configs/%s/custom_backup.clp" % self.machinename) ) 
        ...
        if self.pyvcp and not self.pyvcpname == "custompanel.xml":                
           ...
           originalname = os.path.expanduser("~/labvcnc/configs/%s/custompanel.xml" % self.machinename)
           if os.path.exists(panelname):     
                  if os.path.exists(originalname):
                     print "custom PYVCP file already exists"
                     shutil.copy( originalname,os.path.expanduser("~/labvcnc/configs/%s/custompanel_backup.xml" % self.machinename) ) 
                     print "made backup of existing custom"
        ...
        if self.createshortcut and os.path.exists(desktop):
            if os.path.exists(BASE + "/scripts/labvcnc"):
                scriptspath = (BASE + "/scripts/labvcnc")
            else:
                scriptspath ="labvcnc"
            ...
            print >>file,"Comment=" + _("Desktop Launcher for LabvCNC config made by Stepconf")
            print >>file,"Icon=%s"% labvcncicon
            file.close()
...
class StepconfApp:
    ...
    def build_base(self):
        base = os.path.expanduser("~/labvcnc/configs/%s" % self.d.machinename)
        ncfiles = os.path.expanduser("~/labvcnc/nc_files")
        if not os.path.exists(ncfiles):
            makedirs(ncfiles)
            examples = os.path.join(BASE, "share", "labvcnc", "ncfiles")
...
#*******************
    ...
    # Check for realtime-capable LabvCNC.
    # Returns True if the running version of LabvCNC is realtime-capable
...
#***************
    def testpanel(self,w):
        ...
        if self.w.radiobutton8.get_active() == True:
            ...
            panelname = os.path.expanduser("~/labvcnc/configs/%s" % self.d.machinename)
...
#**************
    def load_ladder(self,w):         
        ...
        if self.w.radiobutton4.get_active() == True:
            ...
            originalfile = filename = os.path.expanduser("~/labvcnc/configs/%s/custom.clp" % self.d.machinename)
        else:
