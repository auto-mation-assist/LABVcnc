...
/*
  ...
  There are six commands supported, Where the commands set and get contain LBV
  specific sub-commands based on the commands supported by lbvsh, but where the "lbv_"
  ...
  The get command includes one of the lbv sub-commands, described below and
  zero or more additional parameters. 
  
  ==> Set <==
  
  The set command inclides one of the lbv sub-commands, described below and
  ...
  The shutdown command tells LBV to shutdown before quitting the connection. This
  ...
  debug {<new value>}
  With get, returns the integer value of LBV_DEBUG, in the LBV. Note that
  ...
  With set, sends a command to the LBV to set the new debug level,
...
static int helpGeneral(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  Get <lbv command>\n\r");
  strcat(context->outBuf, "  Set <lbv command>\n\r");
...
static int helpHello(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  Server Name is the name of the LBV Server to which the client has connected.\n\r");
...
static int helpGet(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  Lbv command may be one of:\n\r");
...
static int helpShutdown(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  and initiates a shutdown of LBV. The command has no parameters, and\n\r");
  ...
  while (1) {
    ...
    if (context->echo && context->linked)
      if(write(context->cliSock, buf, strlen(buf)) != (ssize_t)strlen(buf)) {
        fprintf(stderr, "lbvrsh: write() failed: %s", strerror(errno));
      }
    i = 0;
    j = 0;
    while (i <= strlen(buf)) {
