...
# -*- coding:UTF-8 -*-
"""
    A GUI for LabvCNC based on gladevcp and Python
...
import labvcnc            # to get our own error system
...
if debug:
    pydevdir = '/home/lbvmesa/liclipse/plugins/org.python.pydev_4.5.4.201601292050/pysrc'
...
class gmoccapy(object):
    def __init__(self, argv):
        ...
        # needed components to comunicate with hal and labvcnc
        self.halcomp = hal.component("gmoccapy")
        self.command = labvcnc.command()
        self.stat = labvcnc.stat()

        self.error_channel = labvcnc.error_channel()
        ...
        self.widgets.window1.set_title("gmoccapy for LabvCNC {0}".format(_RELEASE))
...
#            print dro.name
    ...
    def _on_btn_jog_pressed(self, widget, button_name, shift=False):
        ...
        if not self.stat.enabled or self.stat.task_mode != labvcnc.MODE_MANUAL:
            return
        ...
        if self.distance <> 0:  # incremental jogging
            ...
            self.command.jog(labvcnc.JOG_INCREMENT, JOGMODE, joint_no_or_axis_index, dir * velocity, distance)
        else:  # continuous jogging
            self.command.jog(labvcnc.JOG_CONTINUOUS, JOGMODE, joint_no_or_axis_index, dir * velocity)
    ...
    def _on_btn_jog_released(self, widget, button_name, shift=False):
        ...
        if self.distance <> 0:
            ...
            self.command.jog(labvcnc.JOG_STOP, JOGMODE, joint_no_or_axis_index)
...
# =============================================================
    ...
    def _init_tooleditor(self):
       ...
       # get the path to the tool table
        ...
        if not tooltable:
            message = _("**** GMOCCAPY ERROR ****\n")
            message += _("**** Did not find a toolfile file in [LBVIO] TOOL_TABLE ****")
            print(message)
            self.dialogs.warning_dialog(self, _("Very critical situation"), message, sound = False)
            sys.exit()
    ...
    def _init_kinematics_type (self):
        ...
        if self.stat.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
...
# =============================================================
    ...
    def _init_file_to_load(self):
        ...
        self.widgets.ff_file_to_load.set_name("labvcnc files")
    ...
    def _periodic(self):
        # we put the poll comand in a try, so if the labvcnc pid is killed
        # from an external command, we also quit the GUI
        try:
            self.stat.poll()
        except:
            raise SystemExit, "gmoccapy can not poll labvcnc status any more"
...
# button handlers Start
    ...
    def on_tbtn_estop_toggled(self, widget, data=None):
        if widget.get_active():  # estop is active, open circuit
            self.command.state(labvcnc.STATE_ESTOP)
            self.command.wait_complete()
            self.stat.poll()
            if self.stat.task_state == labvcnc.STATE_ESTOP_RESET:
                widget.set_active(False)
        else:  # estop circuit is fine
            self.command.state(labvcnc.STATE_ESTOP_RESET)
            self.command.wait_complete()
            self.stat.poll()
            if self.stat.task_state == labvcnc.STATE_ESTOP:
    ...
    def on_tbtn_on_toggled(self, widget, data=None):
        if widget.get_active():
            if self.stat.task_state == labvcnc.STATE_ESTOP:
                widget.set_active(False)
                return
            self.command.state(labvcnc.STATE_ON)
            self.command.wait_complete()
            self.stat.poll()
            if self.stat.task_state != labvcnc.STATE_ON:
                widget.set_active(False)
            ...
            self.command.state(labvcnc.STATE_OFF)
            self._update_widgets(False)

    # The mode buttons
    def on_rbt_manual_pressed(self, widget, data=None):
        self.command.mode(labvcnc.MODE_MANUAL)
        self.command.wait_complete()

    def on_rbt_mdi_pressed(self, widget, data=None):
        self.command.mode(labvcnc.MODE_MDI)
        self.command.wait_complete()

    def on_rbt_auto_pressed(self, widget, data=None):
        self.command.mode(labvcnc.MODE_AUTO)
        self.command.wait_complete()
...
# hal status Start
    ...
    def on_hal_status_interp_idle(self, widget):
        ...
        if self.stat.task_state == labvcnc.STATE_ESTOP or self.stat.task_state == labvcnc.STATE_OFF:
            self._sensitize_widgets(widgetlist, False)
        else:
            self._sensitize_widgets(widgetlist, True)
    ...
    def on_hal_status_motion_mode_changed(self, widget, new_mode):
        ...
        if new_mode == 1 and self.stat.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
            self.widgets.gremlin.set_property("enable_dro", True)
            self.widgets.gremlin.use_joints_mode = True
            self.widgets.tbtn_switch_mode.set_active(True)
            self.widgets.ntb_jog_JA.set_page(1)
...
# =========================================================
    ...
    def on_key_event(self, widget, event, signal):
        ...
        if keyname == "Return" and signal and self.stat.task_mode == labvcnc.MODE_MDI:
        ...
        if keyname == "F3" or keyname == "F5":
            if self.stat.interp_state != labvcnc.INTERP_IDLE:
        ...
        if self.stat.task_mode == labvcnc.MODE_AUTO:
            ...
            if (keyname == "R" or keyname == "r") and self.stat.interp_state == labvcnc.INTERP_IDLE:
                if event.state & gtk.gdk.CONTROL_MASK:
                    print("R und Control gedrückt")
                    self.widgets.hal_action_reload.emit("activate")
                else:
                    self.command.auto(labvcnc.AUTO_RUN,0)

            if (keyname == "p" or keyname == "P") and self.widgets.tbtn_pause.get_sensitive():
                self.command.auto(labvcnc.AUTO_PAUSE)

            if (keyname == "S" or keyname == "s"):
                self.command.auto(labvcnc.AUTO_RESUME)
        ...
        if self.stat.task_mode != labvcnc.MODE_MANUAL or not self.widgets.ntb_main.get_current_page() == 0:
            return
    ...
    def _update_toolinfo(self, tool):
        ...
        if "G43" in self.active_gcodes and self.stat.task_mode != labvcnc.MODE_AUTO:
            self.command.mode(labvcnc.MODE_MDI)
            self.command.wait_complete()
    ...
    def on_tbtn_setup_toggled(self, widget, data=None):
        ...
        if widget.get_active():
            ...
            if self.stat.task_state == labvcnc.STATE_ESTOP:
                ...
                self.widgets.rbt_mdi.set_sensitive(False)
                self.widgets.rbt_auto.set_sensitive(False)
            if (self.stat.task_state == labvcnc.STATE_ON) and not self.all_homed:
                # machine on, but not homed, only manual allowed
                self.widgets.rbt_manual.set_sensitive(True)
                self.widgets.rbt_mdi.set_sensitive(False)
                self.widgets.rbt_auto.set_sensitive(False)
            if (self.stat.task_state == labvcnc.STATE_ON) and (self.all_homed or self.no_force_homing):
...
# this are hal-tools copied from gsreen function
    ...
    def on_btn_calibration_clicked(self, widget, data=None):
        p = os.popen("tclsh {0}/bin/lbvcalib.tcl -- -ini {1} > /dev/null &".format(TCLPATH, sys.argv[2]), "w")
...
# spindle stuff
    ...
    def _set_spindle(self, command):
        ...
        if self.stat.task_mode != labvcnc.MODE_MANUAL:
            if self.stat.interp_state == labvcnc.INTERP_READING or self.stat.interp_state == labvcnc.INTERP_WAITING:
                if self.stat.spindle[0]['direction'] > 0:
        ...
        if command == "stop":
            ...
            # labvcnc.spindle(direction, speed, spindle=0)
...
# Coolant an mist coolant button
    def on_tbtn_flood_toggled(self, widget, data=None):
        if self.stat.flood and self.widgets.tbtn_flood.get_active():
            ...
            self.widgets.tbtn_flood.set_image(self.widgets.img_coolant_on)
            self.command.flood(labvcnc.FLOOD_ON)
        else:
            self.widgets.tbtn_flood.set_image(self.widgets.img_coolant_off)
            self.command.flood(labvcnc.FLOOD_OFF)
    ...
    def on_tbtn_mist_toggled(self, widget, data=None):
        if self.stat.mist and self.widgets.tbtn_mist.get_active():
            ...
            self.widgets.tbtn_mist.set_image(self.widgets.img_mist_on)
            self.command.mist(labvcnc.MIST_ON)
        else:
            self.widgets.tbtn_mist.set_image(self.widgets.img_mist_off)
            self.command.mist(labvcnc.MIST_OFF)
...
# feed stuff
    ...
    def on_btn_show_kbd_clicked(self, widget):
        ...
        if self.widgets.ntb_button.get_current_page() == _BB_MDI and self.stat.interp_state != labvcnc.INTERP_IDLE:
            self.command.abort()
            self.command.wait_complete()
    ...
    def on_ntb_info_switch_page(self, widget, page, page_num, data=None):
        if self.stat.task_mode == labvcnc.MODE_MDI:
            self.widgets.hal_mdihistory.entry.grab_focus()
        elif self.stat.task_mode == labvcnc.MODE_AUTO:
    ...
    def _on_btn_set_value_clicked(self, widget, data=None):
        if not self.stat.task_state == labvcnc.STATE_ON or not (self.all_homed or self.no_force_homing):
            return
...
#            self.widgets.tbtn_fullsize_preview1.set_active(False)
    ...
    def on_btn_run_clicked(self, widget, data=None):
        self.command.auto(labvcnc.AUTO_RUN, self.start_line)
