...
*   with LBV through NML messages
*
*   Derived from a work by Fred Proctor & Will Shackleford (lbvsh.cc)
...
#define HAL_FIELDS \
    ...
    FIELD(hal_bit_t,estop_activate) /* pin for activating LBV ESTOP  */ \
...
\
    ARRAY(hal_bit_t,spindle_start,LBVMOT_MAX_SPINDLES+1) /* pin for starting the spindle */ \
    ARRAY(hal_bit_t,spindle_stop,LBVMOT_MAX_SPINDLES+1) /* pin for stoping the spindle */ \
    ARRAY(hal_bit_t,spindle_is_on,LBVMOT_MAX_SPINDLES+1) /* status pin for spindle is on */ \
    ARRAY(hal_bit_t,spindle_forward,LBVMOT_MAX_SPINDLES+1) /* pin for making the spindle go forward */ \
    ARRAY(hal_bit_t,spindle_runs_forward,LBVMOT_MAX_SPINDLES+1) /* status pin for spindle running forward */ \
    ARRAY(hal_bit_t,spindle_reverse,LBVMOT_MAX_SPINDLES+1) /* pin for making the spindle go reverse */ \
    ARRAY(hal_bit_t,spindle_runs_backward,LBVMOT_MAX_SPINDLES+1) /* status pin for spindle running backward */ \
    ARRAY(hal_bit_t,spindle_increase,LBVMOT_MAX_SPINDLES+1) /* pin for making the spindle go faster */ \
    ARRAY(hal_bit_t,spindle_decrease,LBVMOT_MAX_SPINDLES+1) /* pin for making the spindle go slower */ \
\
    ARRAY(hal_bit_t,spindle_brake_on,LBVMOT_MAX_SPINDLES) /* pin for activating spindle-brake */ \
    ARRAY(hal_bit_t,spindle_brake_off, LBVMOT_MAX_SPINDLES) /* pin for deactivating spindle/brake */ \
    ARRAY(hal_bit_t,spindle_brake_is_on, LBVMOT_MAX_SPINDLES) /* status pin that tells us if brake is on */ \
\
    ARRAY(hal_bit_t,joint_home,LBVMOT_MAX_JOINTS+1) /* pin for homing one joint */ \
    ARRAY(hal_bit_t,joint_unhome,LBVMOT_MAX_JOINTS+1) /* pin for unhoming one joint */ \
    ARRAY(hal_bit_t,joint_is_homed,LBVMOT_MAX_JOINTS+1) /* status pin that the joint is homed */ \
    ARRAY(hal_bit_t,joint_on_soft_min_limit,LBVMOT_MAX_JOINTS+1) /* status pin that the joint is on the software min limit */ \
    ARRAY(hal_bit_t,joint_on_soft_max_limit,LBVMOT_MAX_JOINTS+1) /* status pin that the joint is on the software max limit */ \
    ARRAY(hal_bit_t,joint_on_hard_min_limit,LBVMOT_MAX_JOINTS+1) /* status pin that the joint is on the hardware min limit */ \
    ARRAY(hal_bit_t,joint_on_hard_max_limit,LBVMOT_MAX_JOINTS+1) /* status pin that the joint is on the hardware max limit */ \
    ARRAY(hal_bit_t,joint_override_limits,LBVMOT_MAX_JOINTS+1) /* status pin that the joint is on the hardware max limit */ \
    ARRAY(hal_bit_t,joint_has_fault,LBVMOT_MAX_JOINTS+1) /* status pin that the joint has a fault */ \
...
\
    ARRAY(hal_bit_t,joint_nr_select,LBVMOT_MAX_JOINTS) /* nr. of pins to select a joint */ \
    ARRAY(hal_bit_t,axis_nr_select,LBVMOT_MAX_AXIS) /* nr. of pins to select a axis */ \
\
    ARRAY(hal_bit_t,joint_is_selected,LBVMOT_MAX_JOINTS) /* nr. of status pins for joint selected */ \
    ARRAY(hal_bit_t,axis_is_selected,LBVMOT_MAX_AXIS) /* nr. of status pins for axis selected */ \
\
    ARRAY(hal_float_t,axis_pos_commanded,LBVMOT_MAX_AXIS+1) /* status pin for commanded cartesian position */ \
    ARRAY(hal_float_t,axis_pos_feedback,LBVMOT_MAX_AXIS+1) /* status pin for actual cartesian position */ \
    ARRAY(hal_float_t,axis_pos_relative,LBVMOT_MAX_AXIS+1) /* status pin for relative cartesian position */ \
\
    FIELD(hal_float_t,jjog_speed) /* pin for setting the jog speed (halui internal) */ \
    ARRAY(hal_bit_t,jjog_minus,LBVMOT_MAX_JOINTS+1) /* pin to jog in positive direction */ \
    ARRAY(hal_bit_t,jjog_plus,LBVMOT_MAX_JOINTS+1) /* pin to jog in negative direction */ \
    ARRAY(hal_float_t,jjog_analog,LBVMOT_MAX_JOINTS+1) /* pin for analog jogging (-1..0..1) */ \
    ARRAY(hal_float_t,jjog_increment,LBVMOT_MAX_JOINTS+1) /* Incremental jogging */ \
    ARRAY(hal_bit_t,jjog_increment_plus,LBVMOT_MAX_JOINTS+1) /* Incremental jogging, positive direction */ \
    ARRAY(hal_bit_t,jjog_increment_minus,LBVMOT_MAX_JOINTS+1) /* Incremental jogging, negative direction */ \
\
    FIELD(hal_float_t,ajog_speed) /* pin for setting the jog speed (halui internal) */ \
    ARRAY(hal_bit_t,ajog_minus,LBVMOT_MAX_AXIS+1) /* pin to jog in positive direction */ \
    ARRAY(hal_bit_t,ajog_plus,LBVMOT_MAX_AXIS+1) /* pin to jog in negative direction */ \
    ARRAY(hal_float_t,ajog_analog,LBVMOT_MAX_AXIS+1) /* pin for analog jogging (-1..0..1) */ \
    ARRAY(hal_float_t,ajog_increment,LBVMOT_MAX_AXIS+1) /* Incremental jogging */ \
    ARRAY(hal_bit_t,ajog_increment_plus,LBVMOT_MAX_AXIS+1) /* Incremental jogging, positive direction */ \
    ARRAY(hal_bit_t,ajog_increment_minus,LBVMOT_MAX_AXIS+1) /* Incremental jogging, negative direction */ \
\
...
\
    ARRAY(hal_s32_t,so_counts,LBVMOT_MAX_SPINDLES+1) /* pin for the Spindle Speed Override counting */ \
    ARRAY(hal_bit_t,so_count_enable,LBVMOT_MAX_SPINDLES+1) /* pin for the Spindle Speed Override counting enable */ \
    ARRAY(hal_bit_t,so_direct_value,LBVMOT_MAX_SPINDLES+1) /* pin for enabling direct value option instead of counts */ \
    ARRAY(hal_float_t,so_scale,LBVMOT_MAX_SPINDLES+1) /* scale for the Spindle Speed Override counting */ \
    ARRAY(hal_float_t,so_value,LBVMOT_MAX_SPINDLES+1) /* current Spindle speed Override value */ \
    ARRAY(hal_bit_t,so_increase,LBVMOT_MAX_SPINDLES+1) /* pin for increasing the SO (+=scale) */ \
    ARRAY(hal_bit_t,so_decrease,LBVMOT_MAX_SPINDLES+1) /* pin for decreasing the SO (-=scale) */ \
\
...
static LBV_TASK_MODE_ENUM halui_old_mode = LBV_TASK_MODE_MANUAL;
...
static int lbvCommandSerialNumber = 0;
...
static int lbvCommandWaitDone()
{
    double end;
    for (end = 0.0; end < doneTimeout; end += LBV_COMMAND_DELAY) {
	updateStatus();
...
static int lbvCommandSend(RCS_CMD_MSG & cmd)
{
    // write command
    ...
    // wait for receive
    double end;
    for (end = 0.0; end < receiveTimeout; end += LBV_COMMAND_DELAY) {
	updateStatus();
	int serial_diff = lbvStatus->echo_serial_number - lbvCommandSerialNumber;
...
static void thisQuit()
{
    ...
    if(lbvCommandBuffer) { delete lbvCommandBuffer;  lbvCommandBuffer = 0; }
    if(lbvStatusBuffer) { delete lbvStatusBuffer;  lbvStatusBuffer = 0; }
    if(lbvErrorBuffer) { delete lbvErrorBuffer;  lbvErrorBuffer = 0; }
    exit(0);
}
...
int halui_hal_init(void)
{
    ...
    *halui_data->ro_direct_value = 0;
    ...
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->ajog_plus[LBVMOT_MAX_AXIS]), comp_id, "halui.axis.selected.plus");
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->ajog_minus[LBVMOT_MAX_AXIS]), comp_id, "halui.axis.selected.minus");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_IN, &(halui_data->ajog_increment[LBVMOT_MAX_AXIS]), comp_id, "halui.axis.selected.increment");
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->ajog_increment_plus[LBVMOT_MAX_AXIS]), comp_id, "halui.axis.selected.increment-plus");
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->ajog_increment_minus[LBVMOT_MAX_AXIS]), comp_id, "halui.axis.selected.increment-minus");
    if (retval < 0) return retval;
...
static int sendMachineOn()
{
    ...
    return lbvCommandSend(state_msg);
}
...
static int sendManual()
{
    ...
    return lbvCommandSend(mode_msg);
}
...
static int sendMdiCommand(int n)
{
    ...
    if (!halui_sent_mdi) {
        ...
        halui_old_mode = lbvStatus->task.mode;
    }
	...
	if (lbvStatus->task.mode != LBV_TASK_MODE_MDI) {
            rtapi_print("halui: %s: switched mode, but got %d instead of mdi\n", __func__, lbvStatus->task.mode);
	    return -1;
	}
    }
    strcpy(lbv_task_plan_execute_msg.command, mdi_commands[n]);
    if (lbvCommandSend(lbv_task_plan_execute_msg)) {
...
static int sendTeleop()
{
    ...
    lbv_set_teleop_enable_msg.enable = 1;
    if (lbvCommandSend(lbv_set_teleop_enable_msg)) {
        return -1;
    }
...
static int sendJoint()
{
    ...
    lbv_set_teleop_enable_msg.enable = 0;
    if (lbvCommandSend(lbv_set_teleop_enable_msg)) {
        return -1;
    }
    return lbvCommandWaitDone();
...
static int sendMistOn()
{
    ...
    return lbvCommandSend(lbv_coolant_mist_on_msg);
}
...
static int sendMistOff()
{
    ...
    return lbvCommandSend(lbv_coolant_mist_off_msg);
}
...
static int sendFloodOn()
{
    ...
    return lbvCommandSend(lbv_coolant_flood_on_msg);
}
...
static int sendFloodOff()
{
    ...
    return lbvCommandSend(lbv_coolant_flood_off_msg);
}
...
static int sendLubeOn()
{
    ...
    return lbvCommandSend(lbv_lube_on_msg);
}
...
static int sendLubeOff()
{
    ...
    return lbvCommandSend(lbv_lube_off_msg);
}
...
static int sendProgramRun(int line)
{
    ...
    sendAuto();
    return lbvCommandSend(lbv_task_plan_run_msg);
}
...
static int sendProgramPause()
{
    ...
    return lbvCommandSend(lbv_task_plan_pause_msg);
}
...
static int sendSetOptionalStop(bool state)
{
    ...
    return lbvCommandSend(lbv_task_plan_set_optional_stop_msg);
}

static int sendSetBlockDelete(bool state)
{
    LBV_TASK_PLAN_SET_BLOCK_DELETE lbv_task_plan_set_block_delete_msg;

    lbv_task_plan_set_block_delete_msg.state = state;
    return lbvCommandSend(lbv_task_plan_set_block_delete_msg);
}
...
static int sendProgramResume()
{
    ...
    return lbvCommandSend(lbv_task_plan_resume_msg);
}
...
static int sendProgramStep()
{
    ...
    return lbvCommandSend(lbv_task_plan_step_msg);
}
...
static int sendSpindleForward(int spindle)
{
	...
	lbv_spindle_on_msg.speed = +1;
    }
    return lbvCommandSend(lbv_spindle_on_msg);
}
...
static int sendSpindleOff(int spindle)
{
    ...
    return lbvCommandSend(lbv_spindle_off_msg);
}
...
static int sendSpindleIncrease(int spindle)
{
    ...
    return lbvCommandSend(lbv_spindle_increase_msg);
}
...
static int sendSpindleDecrease(int spindle)
{
    ...
    return lbvCommandSend(lbv_spindle_decrease_msg);
}
...
static int sendSpindleConstant(int spindle)
{
    ...
    return lbvCommandSend(lbv_spindle_constant_msg);
}
...
static int sendBrakeEngage(int spindle)
{
    ...
    return lbvCommandSend(lbv_spindle_brake_engage_msg);
}
...
static int sendBrakeRelease(int spindle)
{
    ...
    return lbvCommandSend(lbv_spindle_brake_release_msg);
}
...
static int sendHome(int joint)
{
    ...
    return lbvCommandSend(lbv_joint_home_msg);
}

static int sendUnhome(int joint)
{
    LBV_JOINT_UNHOME lbv_joint_unhome_msg;

    lbv_joint_unhome_msg.joint = joint;
    return lbvCommandSend(lbv_joint_unhome_msg);
}
...
static int sendAbort()
{
    ...
    return lbvCommandSend(task_abort_msg);
}
...
static void sendJogStop(int ja, int jjogmode)
{
    ...
    if (   ( (jjogmode == JOGJOINT) && (lbvStatus->motion.traj.mode == LBV_TRAJ_MODE_TELEOP) )
        || ( (jjogmode == JOGTELEOP ) && (lbvStatus->motion.traj.mode != LBV_TRAJ_MODE_TELEOP) )
       ) {
       return;
    }
...
static void sendJogCont(int ja, double speed, int jjogmode)
{
    ...
    if (lbvStatus->task.state != LBV_TASK_STATE_ON) { return; }
    if (   ( (jjogmode == JOGJOINT) && (lbvStatus->motion.traj.mode == LBV_TRAJ_MODE_TELEOP) )
        || ( (jjogmode == JOGTELEOP ) && (lbvStatus->motion.traj.mode != LBV_TRAJ_MODE_TELEOP) )
       ) {
       return;
...
static int sendFeedOverride(double override)
{
    ...
    return lbvCommandSend(lbv_traj_set_scale_msg);
}
...
static int sendSpindleOverride(int spindle, double override)
{
    ...
    return lbvCommandSend(lbv_traj_set_spindle_scale_msg);
}
...
static void hal_init_pins()
{
    ...
    *(halui_data->ajog_minus[LBVMOT_MAX_AXIS]) = old_halui_data.ajog_minus[LBVMOT_MAX_AXIS] = 0;
    *(halui_data->ajog_plus[LBVMOT_MAX_AXIS]) = old_halui_data.ajog_plus[LBVMOT_MAX_AXIS] = 0;
    *(halui_data->ajog_increment[LBVMOT_MAX_AXIS]) = old_halui_data.ajog_increment[LBVMOT_MAX_AXIS] = 0.0;
    *(halui_data->ajog_increment_plus[LBVMOT_MAX_AXIS]) = old_halui_data.ajog_increment_plus[LBVMOT_MAX_AXIS] = 0;
    *(halui_data->ajog_increment_minus[LBVMOT_MAX_AXIS]) = old_halui_data.ajog_increment_minus[LBVMOT_MAX_AXIS] = 0;
...
static bool jogging_selected_axis(local_halui_str &hal) {
    return (hal.ajog_plus[LBVMOT_MAX_AXIS] || hal.ajog_minus[LBVMOT_MAX_AXIS]);
}
...
// axis stuff (selection, homing..)
    ...
    bit = new_halui_data.ajog_minus[LBVMOT_MAX_AXIS];
    js = new_halui_data.axis_selected;
    if ((bit != old_halui_data.ajog_minus[LBVMOT_MAX_AXIS]) || (bit && ajog_speed_changed)) {
        if (bit != 0)
	    sendJogCont(js, -new_halui_data.ajog_speed,JOGTELEOP);
	else
	    sendJogStop(js,JOGTELEOP);
	old_halui_data.ajog_minus[LBVMOT_MAX_AXIS] = bit;
    }

    bit = new_halui_data.ajog_plus[LBVMOT_MAX_AXIS];
    js = new_halui_data.axis_selected;
    if ((bit != old_halui_data.ajog_plus[LBVMOT_MAX_AXIS]) || (bit && ajog_speed_changed)) {
        if (bit != 0)
	    sendJogCont(js,new_halui_data.ajog_speed,JOGTELEOP);
	else
	    sendJogStop(js,JOGTELEOP);
	old_halui_data.ajog_plus[LBVMOT_MAX_AXIS] = bit;
    }

    bit = new_halui_data.ajog_increment_plus[LBVMOT_MAX_AXIS];
    js = new_halui_data.axis_selected;
    if (bit != old_halui_data.ajog_increment_plus[LBVMOT_MAX_AXIS]) {
	if (bit)
	    sendJogIncr(js, new_halui_data.ajog_speed, new_halui_data.ajog_increment[LBVMOT_MAX_AXIS],JOGTELEOP);
	old_halui_data.ajog_increment_plus[LBVMOT_MAX_AXIS] = bit;
    }

    bit = new_halui_data.ajog_increment_minus[LBVMOT_MAX_AXIS];
    js = new_halui_data.axis_selected;
    if (bit != old_halui_data.ajog_increment_minus[LBVMOT_MAX_AXIS]) {
	if (bit)
	    sendJogIncr(js, new_halui_data.ajog_speed, -(new_halui_data.ajog_increment[LBVMOT_MAX_AXIS]),JOGTELEOP);
	old_halui_data.ajog_increment_minus[LBVMOT_MAX_AXIS] = bit;
    }
...
static void modify_hal_pins()
{
	...
	*(halui_data->estop_is_activated)=0;
	...
	if (lbvStatus->status == 1) { //which seems to have finished
	    halui_sent_mdi = 0;
	    switch (halui_old_mode) {
		case LBV_TASK_MODE_MANUAL: sendManual();break;
		case LBV_TASK_MODE_MDI: break;
		case LBV_TASK_MODE_AUTO: sendAuto();break;
    ...
    *(halui_data->program_is_paused) = lbvStatus->task.interpState == LBV_TASK_INTERP_PAUSED;
    *(halui_data->program_is_running) = lbvStatus->task.interpState == LBV_TASK_INTERP_READING ||
                                        lbvStatus->task.interpState == LBV_TASK_INTERP_WAITING;
    *(halui_data->program_is_idle) = lbvStatus->task.interpState == LBV_TASK_INTERP_IDLE;
    *(halui_data->program_os_is_on) = lbvStatus->task.optional_stop_state;
    *(halui_data->program_bd_is_on) = lbvStatus->task.block_delete_state;

    *(halui_data->mv_value) = lbvStatus->motion.traj.maxVelocity;
    *(halui_data->fo_value) = lbvStatus->motion.traj.scale; //feedoverride from 0 to 1 for 100%
    *(halui_data->ro_value) = lbvStatus->motion.traj.rapid_scale; //rapid override from 0 to 1 for 100%

    *(halui_data->mist_is_on) = lbvStatus->io.coolant.mist;
    *(halui_data->flood_is_on) = lbvStatus->io.coolant.flood;
    *(halui_data->lube_is_on) = lbvStatus->io.lube.on;

    *(halui_data->tool_number) = lbvStatus->io.tool.toolInSpindle;
    *(halui_data->tool_length_offset_x) = lbvStatus->task.toolOffset.tran.x;
    *(halui_data->tool_length_offset_y) = lbvStatus->task.toolOffset.tran.y;
    *(halui_data->tool_length_offset_z) = lbvStatus->task.toolOffset.tran.z;
    *(halui_data->tool_length_offset_a) = lbvStatus->task.toolOffset.a;
    *(halui_data->tool_length_offset_b) = lbvStatus->task.toolOffset.b;
    *(halui_data->tool_length_offset_c) = lbvStatus->task.toolOffset.c;
    *(halui_data->tool_length_offset_u) = lbvStatus->task.toolOffset.u;
    *(halui_data->tool_length_offset_v) = lbvStatus->task.toolOffset.v;
    *(halui_data->tool_length_offset_w) = lbvStatus->task.toolOffset.w;

    if (lbvStatus->io.tool.toolInSpindle == 0) {
        *(halui_data->tool_diameter) = 0.0;
    } else {
        int pocket;
        for (pocket = 0; pocket < CANON_POCKETS_MAX; pocket ++) {
            if (lbvStatus->io.tool.toolTable[pocket].toolno == lbvStatus->io.tool.toolInSpindle) {
                *(halui_data->tool_diameter) = lbvStatus->io.tool.toolTable[pocket].diameter;
                break;
            }
        }
    ...
    for (spindle = 0; spindle < num_spindles; spindle++){
        *(halui_data->spindle_is_on[spindle]) = (lbvStatus->motion.spindle[spindle].enabled);
        *(halui_data->spindle_runs_forward[spindle]) = (lbvStatus->motion.spindle[spindle].direction == 1);
        *(halui_data->spindle_runs_backward[spindle]) = (lbvStatus->motion.spindle[spindle].direction == -1);
        *(halui_data->spindle_brake_is_on[spindle]) = lbvStatus->motion.spindle[spindle].brake;
        *(halui_data->so_value[spindle]) = lbvStatus->motion.spindle[spindle].spindle_scale; //spindle-speed-override from 0 to 1 for 100%
    }

    for (joint=0; joint < num_joints; joint++) {
	*(halui_data->joint_is_homed[joint]) = lbvStatus->motion.joint[joint].homed;
	*(halui_data->joint_on_soft_min_limit[joint]) = lbvStatus->motion.joint[joint].minSoftLimit;
	*(halui_data->joint_on_soft_max_limit[joint]) = lbvStatus->motion.joint[joint].maxSoftLimit;
	*(halui_data->joint_on_hard_min_limit[joint]) = lbvStatus->motion.joint[joint].minHardLimit;
	*(halui_data->joint_on_hard_max_limit[joint]) = lbvStatus->motion.joint[joint].maxHardLimit;
	*(halui_data->joint_override_limits[joint]) = lbvStatus->motion.joint[joint].overrideLimits;
	*(halui_data->joint_has_fault[joint]) = lbvStatus->motion.joint[joint].fault;
    }

    if (axis_mask & 0x0001) {
      *(halui_data->axis_pos_commanded[0]) = lbvStatus->motion.traj.position.tran.x;	
      *(halui_data->axis_pos_feedback[0]) = lbvStatus->motion.traj.actualPosition.tran.x;	
      *(halui_data->axis_pos_relative[0]) = lbvStatus->motion.traj.actualPosition.tran.x - lbvStatus->task.g5x_offset.tran.x - lbvStatus->task.g92_offset.tran.x - lbvStatus->task.toolOffset.tran.x;
    }

    if (axis_mask & 0x0002) {
      *(halui_data->axis_pos_commanded[1]) = lbvStatus->motion.traj.position.tran.y;	
      *(halui_data->axis_pos_feedback[1]) = lbvStatus->motion.traj.actualPosition.tran.y;	
      *(halui_data->axis_pos_relative[1]) = lbvStatus->motion.traj.actualPosition.tran.y - lbvStatus->task.g5x_offset.tran.y - lbvStatus->task.g92_offset.tran.y - lbvStatus->task.toolOffset.tran.y;
    }

    if (axis_mask & 0x0004) {
      *(halui_data->axis_pos_commanded[2]) = lbvStatus->motion.traj.position.tran.z;
      *(halui_data->axis_pos_feedback[2]) = lbvStatus->motion.traj.actualPosition.tran.z;
      *(halui_data->axis_pos_relative[2]) = lbvStatus->motion.traj.actualPosition.tran.z - lbvStatus->task.g5x_offset.tran.z - lbvStatus->task.g92_offset.tran.z - lbvStatus->task.toolOffset.tran.z;
    }

    if (axis_mask & 0x0008) {
      *(halui_data->axis_pos_commanded[3]) = lbvStatus->motion.traj.position.a;
      *(halui_data->axis_pos_feedback[3]) = lbvStatus->motion.traj.actualPosition.a;
      *(halui_data->axis_pos_relative[3]) = lbvStatus->motion.traj.actualPosition.a - lbvStatus->task.g5x_offset.a - lbvStatus->task.g92_offset.a - lbvStatus->task.toolOffset.a;
    }

    if (axis_mask & 0x0010) {
      *(halui_data->axis_pos_commanded[4]) = lbvStatus->motion.traj.position.b;
      *(halui_data->axis_pos_feedback[4]) = lbvStatus->motion.traj.actualPosition.b;
      *(halui_data->axis_pos_relative[4]) = lbvStatus->motion.traj.actualPosition.b - lbvStatus->task.g5x_offset.b - lbvStatus->task.g92_offset.b - lbvStatus->task.toolOffset.b;
    }

    if (axis_mask & 0x0020) {
      *(halui_data->axis_pos_commanded[5]) = lbvStatus->motion.traj.position.c;
      *(halui_data->axis_pos_feedback[5]) = lbvStatus->motion.traj.actualPosition.c;
      *(halui_data->axis_pos_relative[5]) = lbvStatus->motion.traj.actualPosition.c - lbvStatus->task.g5x_offset.c - lbvStatus->task.g92_offset.c - lbvStatus->task.toolOffset.c;
    }

    if (axis_mask & 0x0040) {
      *(halui_data->axis_pos_commanded[6]) = lbvStatus->motion.traj.position.u;
      *(halui_data->axis_pos_feedback[6]) = lbvStatus->motion.traj.actualPosition.u;
      *(halui_data->axis_pos_relative[6]) = lbvStatus->motion.traj.actualPosition.u - lbvStatus->task.g5x_offset.u - lbvStatus->task.g92_offset.u - lbvStatus->task.toolOffset.u;
    }

    if (axis_mask & 0x0080) {
      *(halui_data->axis_pos_commanded[7]) = lbvStatus->motion.traj.position.v;
      *(halui_data->axis_pos_feedback[7]) = lbvStatus->motion.traj.actualPosition.v;
      *(halui_data->axis_pos_relative[7]) = lbvStatus->motion.traj.actualPosition.v - lbvStatus->task.g5x_offset.v - lbvStatus->task.g92_offset.v - lbvStatus->task.toolOffset.v;
    }

    if (axis_mask & 0x0100) {
      *(halui_data->axis_pos_commanded[8]) = lbvStatus->motion.traj.position.w;
      *(halui_data->axis_pos_feedback[8]) = lbvStatus->motion.traj.actualPosition.w;
      *(halui_data->axis_pos_relative[8]) = lbvStatus->motion.traj.actualPosition.w - lbvStatus->task.g5x_offset.w - lbvStatus->task.g92_offset.w - lbvStatus->task.toolOffset.w;
    }

    *(halui_data->joint_is_homed[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].homed;
    *(halui_data->joint_on_soft_min_limit[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].minSoftLimit;
    *(halui_data->joint_on_soft_max_limit[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].maxSoftLimit;
    *(halui_data->joint_on_hard_min_limit[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].minHardLimit;
    *(halui_data->joint_override_limits[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].overrideLimits;
    *(halui_data->joint_on_hard_max_limit[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].maxHardLimit;
    *(halui_data->joint_has_fault[num_joints]) = lbvStatus->motion.joint[*(halui_data->joint_selected)].fault;
...
int main(int argc, char *argv[])
{
    ...
    // init NML
    if (0 != tryNml()) {
	rcs_print_error("can't connect to lbv\n");
	thisQuit();
	exit(1);
    }
    ...
    while (!done) {
        ...
        if (!task_start_synced) {
           ...
           if (lbvStatus->motion.traj.linearUnits != 0) {
              // set once at startup, no changes are expected:
              *(halui_data->units_per_mm) = lbvStatus->motion.traj.linearUnits;
              task_start_synced = 1;
           }
        }
