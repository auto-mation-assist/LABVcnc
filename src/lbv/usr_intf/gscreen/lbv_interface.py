...
class lbv_control:
        ...
        def home_all(self, b):
                ...
                self.lbvcommand.teleop_enable(False)
        ...
        def spindle_forward(self, b, rpm=100):
                if self.masked: return
                self.lbvcommand.mode(self.lbv.MODE_MANUAL)
                self.lbvcommand.spindle(1,rpm,0);
        ...
        def incremental_jog(self, axis, direction, distance):
                ...
                if axis in (3,4,5):
                    ...
                    rate = self.jog_velocity
                self.lbvcommand.jog(self.lbv.JOG_INCREMENT
        ...
        def feed_hold(self, data):
                if self.masked: return
                self.lbvcommand.set_feed_hold(data)
        ...
        def spindle_override(self, s):
                if self.masked: return
                self.lbvcommand.spindleoverride(s)
        ...
        def opstop(self, data):
                if self.masked: return
                self.lbvcommand.set_optional_stop(data)

        def blockdel(self, data):
                if self.masked: return
                self.lbvcommand.set_block_delete(data)
...
class lbv_status:
        ...
        def get_feedrate(self):
            return self.lbvstat.feedrate

        def get_spindlerate(self):
            return self.lbvstat.spindle[0]['override']

        def get_maxvelocity(self):
            return self.lbvstat.maxvelocity
        ...
        def get_linear_units(self):
            return self.lbvstat.linear_units
        ...
        def periodic(self):
            ...
            # M codes
            temp = []; active_codes = []
            for i in sorted(self.lbvstat.mcodes[1:]):
                if i == -1: continue
                temp.append("%d" % i)
            for i in (temp):
                active_codes.append("M"+i)
            self.data.active_mcodes = active_codes
            feed_str = "%.3f" % self.lbvstat.settings[1]
            if feed_str.endswith(".000"): feed_str = feed_str[:-4]
            self.data.active_feed_command = feed_str
            self.data.active_spindle_command = "%.0f" % self.lbvstat.settings[2]

            # estop status
            self.data.estopped = self.lbvstat.task_state == self.lbv.STATE_ESTOP
            # spindle
            self.data.spindle_dir = self.lbvstat.spindle[0]['direction']
            self.data.spindle_speed = abs(self.lbvstat.spindle[0]['speed'])
            self.data.spindle_override = self.lbvstat.spindle[0]['override']
            # other
            self.data.tool_in_spindle = self.lbvstat.tool_in_spindle
            self.data.flood = self.lbvstat.flood
            self.data.mist = self.lbvstat.mist
            self.data.machine_on = self.lbvstat.task_state == self.lbv.STATE_ON
            self.data.or_limits = self.lbvstat.joint[0]['override_limits']
            self.data.feed_hold = self.lbvstat.feed_hold_enabled
            self.data.feed_override = self.lbvstat.feedrate
            self.data.velocity_override = self.lbvstat.max_velocity / self.data._maxvelocity
            self.data.rapid_override = self.lbvstat.rapidrate
            self.data.file = self.lbvstat.file
            self.data.last_line = self.data.motion_line
            self.data.motion_line = self.lbvstat.motion_line
            self.data.line =  self.lbvstat.current_line
            self.data.id =  self.lbvstat.id
            self.data.dtg = self.lbvstat.distance_to_go
            self.data.velocity = self.convert_units(self.lbvstat.current_vel) * 60.0
            self.data.delay = self.lbvstat.delay_left
            if self.lbvstat.pocket_prepped == -1:
                self.data.preppedtool = None
            else:
                self.data.preppedtool = self.lbvstat.tool_table[self.lbvstat.pocket_prepped].id
