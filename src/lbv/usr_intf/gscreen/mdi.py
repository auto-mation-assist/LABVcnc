...
class mdi:
    ...
    def index_tool(self,toolnumber):
        ...
        if self.lbvstat.task_mode != self.lbv.MODE_MDI:
            self.lbvcommand.mode(self.lbv.MODE_MDI)
            self.lbvcommand.wait_complete()
        self.lbvcommand.mdi("m6 T %f"%(toolnumber))
        self.lbvcommand.mdi("g43 h%f"%(toolnumber))

    def arbitrary_mdi(self,command):
        if self.lbvstat.task_mode != self.lbv.MODE_MDI:
            self.lbvcommand.mode(self.lbv.MODE_MDI)
            self.lbvcommand.wait_complete()
        self.lbvcommand.mdi(command)

class mdi_control:
    def __init__(self, gtk, lbv, labels, eventboxes):
        self.labels = labels
    ...
    def mdi_is_reading(self):
        self.mdi.lbvstat.poll()
        if self.mdi.lbvstat.interp_state == self.mdi.lbv.INTERP_READING:
            return True
        return False

    def set_mdi_mode(self):
        self.mdi.lbvstat.poll()
        if self.mdi.lbvstat.task_mode != self.mdi.lbv.MODE_MDI:
            self.mdi.lbvcommand.mode(self.mdi.lbv.MODE_MDI)
            self.mdi.lbvcommand.wait_complete()

    def set_axis(self,axis,value):
        premode = self.mdi.lbvstat.task_mode
        self.mdi.set_axis_origin(axis,value)
        self.mdi.lbvcommand.mode(premode)
        self.mdi.lbvcommand.wait_complete()
