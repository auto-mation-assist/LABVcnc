...
#    Gscreen a GUI for labvcnc cnc controller 
...
"""
# Gscreen is made for running labvcnc CNC machines
...
def _print_help():
            print """ Gscreen a customizable operator screen for labvcnc based on pyGTK / GLADE.\n
        It is usually loaded from labvcnc's INI file under the [DISPLAY] HEADER
        eg. DISPLAY = gscreen\n
        Options:
        --INI.................Designates the configuration file path for labvcnc
...
# BASE is the absolute path to labvcnc base
...
datadir = os.path.join(BASE, "share", "labvcnc")
...
try:
    TCLPATH = os.environ['LABVCNC_TCL_DIR']
except:
    pass
...
try:
    ...
    pass
import labvcnc
from gscreen import lbv_interface
...
# lbv_interface.py which does most of the commands and status of labvcnc
# keep in mind some of the gladeVCP widgets send-commands-to/monitor labvcnc also
...
class Gscreen: 
    ...
    def __init__(self):
        ...
        # labvcnc adds -ini to display name and optparse
        ...
        # access to LBV control
        self.lbv = lbv_interface.lbv_control(labvcnc)
        # access to LBV status
        self.status = lbv_interface.lbv_status( self.data, labvcnc)
        # access to MDI
        mdi_labels = mdi_eventboxes = []
        self.mdi_control = mdi.mdi_control(gtk, labvcnc, mdi_labels, mdi_eventboxes)
        # pull info from the INI file
        self.inifile = self.lbv.lbv.ini(self.inipath)
        ...
        self.lbv.continuous_jog_velocity(float(temp),None)
        ...
        if "a" in self.data.axis_list or "b" in self.data.axis_list or "c" in self.data.axis_list:
            ...
            self.lbv.continuous_jog_velocity(None,float(temp))
        ...
        self.data.tooltable = self.inifile.find("LBVIO","TOOL_TABLE")
    ...
    def init_general_pref(self):
        ...
        if self.data.dro_units: # set labvcnc as well
    ...
    def init_running_options(self):
        """set button widgets and labvcnc for block delete and optional stop
    ...
    def init_show_windows(self):
        ...
        self.widgets.window1.set_title("%s for labvcnc"% title)
        if self.screen2:
...
# *** GLADE callbacks ****
    ...
    def on_keycall_ABORT(self,state,SHIFT,CNTRL,ALT):
        """Calls a gladevcp hal action to abort a labvcnc run.
    ...
    def on_keycall_ESTOP(self,state,SHIFT,CNTRL,ALT):
        """Calls a gladevcp toggle button to estop labvcnc.
    ...
    def on_keycall_POWER(self,state,SHIFT,CNTRL,ALT):
        """Calls a gladevcp toggle button to power labvcnc.
           button widget is assumed to be named button_estop
           The button will emit signal 'clicked'
           This is part of the default key call routine
        """
    ...
    def on_calibration(self,*args):
        ...
        p = os.popen("tclsh %s/bin/lbvcalib.tcl -- -ini %s > /dev/null &" % (TCLPATH,self.inipath),"w")

    # opens the labvcnc status program
    def on_status(self,*args):
        """This is a callback function to launch the labvcnc status program.
        """
        p = os.popen("labvcnctop  > /dev/null &","w")
    ...
    def on_estop_clicked(self,*args):
        ...
        if self.data.estopped:
            self.lbv.estop_reset(1)
        elif not self.data.machine_on:
            self.lbv.machine_on(1)
    ...
    def on_hal_status_interp_run(self,widget):
        """This is a callback function called when labvcnc's interpeter is running.
    ...
    def on_hal_status_interp_idle(self,widget):
        """This is a callback function called when labvcnc's interpeter is idle.
            ...
            It sets labvcnc into manual mode if Gscreen is on the manual page.
        """
        print "idle"
    ...
    def on_hal_status_state_off(self,widget):
        """This is a callback function called when labvcnc's state is off.
            It sensitives widgets in a string list called data.sensitive_on_off
        """
        print "off"
    ...
    def on_hal_status_axis_homed(self,widget,data):
        """This is a callback function called when labvcnc is homed.
    ...
    def on_hal_status_not_all_homed(self,widget,data):
        """This is a callback function called when labvcnc is unhomed.
    ...
    def on_hal_status_file_loaded(self,widget,filename):
        """This is a callback function called when labvcnc loads a file.
...
# ****** do stuff *****
    ...
    def set_spindle_override(self,percent_rate,absolute=False):
        ...
        self.lbv.spindle_override(rate)
    ...
    def set_velocity_override(self,percent_rate,absolute=False):
        ...
        self.lbv.max_velocity(rate * self.data._maxvelocity)
    ...
    def set_jog_rate(self,step=None,absolute=None):
        ...
        rate = round(rate,1)
        if self.data.angular_jog_adjustment_flag:
            self.lbv.continuous_jog_velocity(None,rate)
        else:
            self.lbv.continuous_jog_velocity(rate,None)
            ...
            if len(self.data.active_axis_buttons) > 1:
                ...
                if not self.data.active_axis_buttons[0][0] == "s":
                    ...
                    if jogincr == ("continuous"): # continuous jog
                        ...
                        self.lbv.continuous_jog(self.data.active_axis_buttons[0][1],cmd)
                    else:
                        print "jog incremental"
                        if cmd == 0: return # don't want release of button to stop jog
                        distance = self.parse_increment(jogincr)
                        self.lbv.incremental_jog(self.data.active_axis_buttons[0][1],cmd,distance)
    ...
    def do_key_jog(self,axis,direction,action):
        if self.data._JOG in self.check_mode(): # jog mode active:
                    ...
                    if self.data.jog_increments[self.data.current_jogincr_index] == ("continuous"): # continuous jog
                        ...
                        self.lbv.continuous_jog(axis,cmd)
                    else:
                        print "jog incremental"
                        if cmd == 0: return # don't want release of button to stop jog
                        self.mdi_control.mdi.lbvstat.poll()
                        if self.mdi_control.mdi.lbvstat.state != 1: return
                        jogincr = self.data.jog_increments[self.data.current_jogincr_index]
                        distance = self.parse_increment(jogincr)
                        self.lbv.incremental_jog(axis,cmd,distance)
    ...
    def spindle_adjustment(self,direction,action):
        ...
        if direction and action:
            if self.data.spindle_speed:
                self.lbv.spindle_faster(1)
            elif self.widgets.s_display_fwd.get_active():
               self.lbv.spindle_forward(1,self.data.spindle_start_rpm)
            else:
                self.lbv.spindle_reverse(1,self.data.spindle_start_rpm)
            print direction,action
        elif not direction and action:
            if self.data.spindle_speed:
                if self.data.spindle_speed >100:
                    self.lbv.spindle_slower(1)
                else:
                    self.lbv.spindle_off(1)
    ...
    def adjust_spindle_rpm(self, rpm, direction=None):
            # spindle control
             ...
             if direction > 0:
                print "forward"
                self.lbv.spindle_forward(1, float(rpm))
             elif direction < 0:
                print "reverse"
                self.lbv.spindle_reverse(1, float(rpm))
             else:
                self.lbv.spindle_off(1)
    ...
    def home_selected(self):
        ...
        if len(self.data.active_axis_buttons) > 1:
            ...
            self.lbv.home_selected(self.data.active_axis_buttons[0][1])
    ...
    def unhome_selected(self):
        if len(self.data.active_axis_buttons) > 1:
            ...
            self.lbv.unhome_selected(self.data.active_axis_buttons[0][1])
    ...
    def reload_tooltable(self):
        ...
        # tell labvcnc that the tooltable may have changed
        self.lbv.reload_tooltable(1)
    ...
    # check labvcnc for status, error and then update the readout
    def timer_interrupt(self):
        self.lbv.mask()
        self.lbvstat = labvcnc.stat()
        self.lbverror = labvcnc.error_channel()
        self.lbvstat.poll()
        self.data.task_mode = self.lbvstat.task_mode 
        ...
        if e:
            ...
            elif kind in (labvcnc.NML_TEXT, labvcnc.OPERATOR_TEXT):
                self.notify(_("Message"),text,INFO_ICON,3)
            elif kind in (labvcnc.NML_DISPLAY, labvcnc.OPERATOR_DISPLAY):
                self.notify(_("Message"),text,INFO_ICON,3)
        self.lbv.unmask()
