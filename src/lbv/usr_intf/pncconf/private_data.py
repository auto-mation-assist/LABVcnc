...
class Private_Data:
    def __init__(self,app,bin_dir,base_dir):
        ...
        self.DATADIR = labvcnc.SHARE + "/labvcnc/pncconf"
        self.WIZARD = os.path.join(self.DATADIR, "labvcnc-wizard.gif")
        if not os.path.isfile(self.WIZARD):
            self.WIZARD = os.path.join("/etc/labvcnc/labvcnc-wizard.gif")
        if not os.path.isfile(self.WIZARD):
            self.WIZARD = os.path.join("/usr/share/labvcnc/labvcnc-wizard.gif")
        if not os.path.isfile(self.WIZARD):
            wizdir = os.path.join(os.path.abspath(BIN), "..")
            self.WIZARD = os.path.join(wizdir, "labvcnc-wizard.gif")

        self.ICONDIR = os.path.join(os.path.abspath(BIN), "..")
        self.LABVCNCICON = os.path.join(self.ICONDIR, "labvcncicon.png")
        if not os.path.isfile(self.LABVCNCICON):
            self.LABVCNCICON = os.path.join("/etc/labvcnc/labvcnc-wizard.gif")
        if not os.path.isfile(self.LABVCNCICON):
            self.LABVCNCICON = os.path.join("/usr/share/labvcnc/labvcncicon.png")

        self.DISTDIR = os.path.join(os.path.abspath(BIN), "..", "configs", "common")
        if not os.path.isdir(self.DISTDIR):
            self.DISTDIR = os.path.join(os.path.abspath(BIN), "..", "share", "doc", "labvcnc", "sample-configs", "common")
        if not os.path.isdir(self.DISTDIR):
            self.DISTDIR = os.path.join(os.path.abspath(BIN), "..", "labvcnc", "sample-configs", "common")
        if not os.path.isdir(self.DISTDIR):
            self.DISTDIR = "/usr/share/doc/labvcnc/examples/sample-configs/common"
        self.HELPDIR = os.path.join(BASE, "share", "labvcnc", "pncconf", "pncconf-help")
        if not os.path.exists(self.HELPDIR):
            self.HELPDIR = os.path.join(BASE, "src", "lbv", "usr_intf", "pncconf", "pncconf-help")
