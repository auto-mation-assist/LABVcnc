...
class HAL:
    ...
    def write_halfile(self, base):
        ...
        print >>file, _("# Using LabvCNC version:  %s") % self.d.labvcnc_version
        ...
        print >>file, "loadrt [LBVMOT]LBVMOT servo_period_nsec=[LBVMOT]SERVO_PERIOD num_joints=[KINS]JOINTS"
        ...
        if  self.d.classicladder and self.d.ladderhaltype == 1 and self.d.ladderconnect: # external estop program
            ...
            print >>file, "net estop-outcl      classicladder.0.out-00  =>  iocontrol.0.lbv-enable-in"
            print >>file
            print >>file, _("# **** Setup for external estop ladder program -END ****")
        elif estop:
            print >>file, "net estop-ext     =>  iocontrol.0.lbv-enable-in"
        else:
            print >>file, "net estop-out     =>  iocontrol.0.lbv-enable-in"
        print >>file
        ...
        if self.d.classicladder: 
           if not self.d.laddername == "custom.clp":
                ...
                original = os.path.expanduser("~/labvcnc/configs/%s/custom.clp" % self.d.machinename)
                if os.path.exists(filename): # check for the master file to copy from 
                  if os.path.exists(original):
                     ...
                     #shutil.copy( original,os.path.expanduser("~/labvcnc/configs/%s/backups/custom_backup.clp" % self.d.machinename) ) 
        ...
        if self.d.pyvcp and not self.d.pyvcpexist:                
           ...
           originalname = os.path.expanduser("~/labvcnc/configs/%s/pyvcp-panel.xml" % self.d.machinename)
           if os.path.exists(panelname):     
                  if os.path.exists(originalname):
                     #print "custom PYVCP file already exists"
                     writebackup(originalname)
                     #shutil.copy( originalname,os.path.expanduser("~/labvcnc/configs/%s/backups/pyvcp-panel_backup.xml" % self.d.machinename) ) 
                     print "made backup of existing custom"
    ...
    def write_readme(self, base):
        ...
        elif self.d.frontend == _PD._TKLABVCNC: display = "Tklabvcnc"
        ...
        print >>file, self.d.machinename,_("configures LabvCNC as:\n")
