#!/usr/bin/env python
# -*- encoding: utf-8 -*-
#    This is pncconf, a graphical configuration editor for LabvCNC
#    Chris Morley copyright 2009
#    This is based from stepconf, a graphical configuration editor for labvcnc
...
#import mesatest
try:
    LABVCNCVERSION = os.environ['LABVCNCVERSION']
except:
    LABVCNCVERSION = 'UNAVAILABLE'
...
class App:
    def __init__(self, dbgstate=0):
        ...
        self.d = data.Data(self, _PD, BASE, LABVCNCVERSION)
...
PNCconf will use internal firmware data"%self._p.FIRMDIR),True)
    ...
    def load_config(self):
        ...
        filter.set_name(_("LabvCNC 'PNCconf' configuration files"))
        ...
        if not self.d._lastconfigname == "" and self.d._chooselastconfig:
            dialog.set_filename(os.path.expanduser("~/labvcnc/configs/%s.pncconf"% self.d._lastconfigname))
        dialog.add_shortcut_folder(os.path.expanduser("~/labvcnc/configs"))
        dialog.set_current_folder(os.path.expanduser("~/labvcnc/configs"))
        dialog.show_all()
        result = dialog.run()
        if result == gtk.RESPONSE_OK:
    ...
    def parse_xml(self, driver, boardtitle, firmname, xml_path):
            ...
            for i,j in enumerate(pins):
                ...
                tempfunc = tempfunc.upper().strip() # normalise capitalization: Peters XMLs are different from labvcncs
    ...
    def add_device_rule(self):
        text = []
        sourcefile = "/tmp/"
        if os.path.exists("/etc/udev/rules.d/50-LABVCNC-general.rules"):
            text.append( "General rule already exists\n")
        else:
            text.append("adding a general rule first\nso your device will be found\n")
            filename = os.path.join(sourcefile, "LABVCNCtempGeneral.rules")
            file = open(filename, "w")
            print >>file, ("# This is a rule for LabvCNC's hal_input\n")
            print >>file, ("""SUBSYSTEM="input", MODE="0660", GROUP="plugdev" """) 
            file.close()
            p=os.popen("gksudo cp  %sLABVCNCtempGeneral.rules /etc/udev/rules.d/50-LABVCNC-general.rules"% sourcefile )
            time.sleep(.1)
            p.flush()
            p.close()
            os.remove('%sLABVCNCtempGeneral.rules'% sourcefile)
        text.append(("disconect USB device please\n"))
        if not self.warning_dialog("\n".join(text),False):return

        os.popen('less /proc/bus/input/devices >> %sLABVCNCnojoytemp.txt'% sourcefile)
        text = ["Plug in USB device please"]
        if not self.warning_dialog("\n".join(text),False):return
        time.sleep(1)

        os.popen('less /proc/bus/input/devices >> %sLABVCNCjoytemp.txt'% sourcefile).read()
        diff = os.popen (" less /proc/bus/input/devices  | diff   %sLABVCNCnojoytemp.txt %sLABVCNCjoytemp.txt "%(sourcefile, sourcefile) ).read()
        self.widgets.help_window.set_title(_("USB device Info Search"))

        os.remove('%sLABVCNCnojoytemp.txt'% sourcefile)
        os.remove('%sLABVCNCjoytemp.txt'% sourcefile)
        if diff =="":
            ...
            if not self.warning_dialog("\n".join(text),False):return
            tempname = sourcefile+"LABVCNCtempspecific.rules"
            ...
            for i in ("(",")"):
                temp = name.replace(i,"")
                name = temp
            newname = "50-LABVCNC-%s.rules"% name.replace(" ","_")
            os.popen("gksudo cp  %s /etc/udev/rules.d/%s"% (tempname,newname) )
            time.sleep(1)
            os.remove('%sLABVCNCtempspecific.rules'% sourcefile)
    ...
    def search_for_device_rule(self):
        ...
        for entry in os.listdir("/etc/udev/rules.d"):
            if fnmatch.fnmatch( entry,"50-LABVCNC-*"):
    ...
    def gladevcp_sanity_check(self):
                if os.path.exists(os.path.expanduser("~/labvcnc/configs/%s/gvcp-panel.ui" % self.d.machinename)):
...
if you change related options later -such as spindle feedback- the HAL connection will not update"),False):
    ...
    def pyvcp_sanity_check(self):
              if os.path.exists(os.path.expanduser("~/labvcnc/configs/%s/pyvcp-panel.xml" % self.d.machinename)):
