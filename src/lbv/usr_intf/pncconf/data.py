...
class Data:
    def __init__(self, app, private_data_instance,base_dir, labvcnc_version):
        global _PD
        global _APP
        global _BASE
        _APP = app
        ...
        _BASE = base_dir
        self.labvcnc_version = labvcnc_version
        ...
        self.machinename = _("my_LabvCNC_machine")
        ...
        # LabvCNC assorted defaults and options
...
If you have a REALLY large config that you wish to convert to this newer version of PNConf - ask on the LabvCNC forum - it may be possible..") )
    ...
    def save(self,basedir):
        ...
        if not os.path.exists(ncfiles):
            ...
            examples = os.path.join(_BASE, "share", "labvcnc", "ncfiles")
        ...
        if self.createshortcut and os.path.exists(desktop):
            if os.path.exists(_BASE + "/scripts/labvcnc"):
                scriptspath = (_BASE + "/scripts/labvcnc")
            else:
                scriptspath ="labvcnc"
            ...
            print >>file,"Comment=" + _("Desktop Launcher for LabvCNC config made by PNCconf")
            print >>file,"Icon=%s"% _PD.LABVCNCICON
            file.close()
