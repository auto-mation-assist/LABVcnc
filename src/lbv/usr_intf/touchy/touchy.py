...
gtk.glade.bindtextdomain("labvcnc", LOCALEDIR)
gtk.glade.textdomain("labvcnc")
...
import labvcnc
from touchy import lbv_interface
...
class touchy:
		...
		self.wTree.get_widget('MainWindow').grab_focus()
                ...
                self.listing = listing.listing(gtk, labvcnc, listing_labels, listing_eventboxes)

                # lbv interface
                self.labvcnc = lbv_interface.lbv_control(labvcnc, self.listing, self.wTree.get_widget("error"))
                self.labvcnc.continuous_jog_velocity(self.mv_val)
                self.hal = hal_interface.hal_interface(self, self.labvcnc, self.mdi_control, labvcnc)
                ...
                self.filechooser = filechooser.filechooser(gtk, labvcnc, filechooser_labels, filechooser_eventboxes, self.listing)
                ...
                self.status = lbv_interface.lbv_status(gtk, labvcnc, self.listing, relative, absolute, distance,
        ...
        def home_selected(self, b):
                self.labvcnc.home_selected(self.wheelxyz)

        def unhome_selected(self, b):
                self.labvcnc.unhome_selected(self.wheelxyz)
        ...
        def periodic_status(self):
                ...
                if self.current_file != self.status.lbvstat.file:
                    self.current_file = self.status.lbvstat.file
