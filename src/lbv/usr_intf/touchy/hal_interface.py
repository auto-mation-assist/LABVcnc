...
class hal_interface:
    def __init__(self, gui, lbv_control, mdi_control, lbv):
        self.gui = gui
        self.lbv_control = lbv_control
        self.lbv = lbv
        self.lbv_stat = self.lbv.stat()
    ...
    def periodic(self, mdi_mode):
        ...
        if xp ^ self.xp: self.lbv_control.continuous_jog(0, xp)
        self.xp = xp

        xn = self.c["jog.continuous.x.negative"]
        if xn ^ self.xn: self.lbv_control.continuous_jog(0, -xn)
        self.xn = xn

        yp = self.c["jog.continuous.y.positive"]
        if yp ^ self.yp: self.lbv_control.continuous_jog(1, yp)
        self.yp = yp

        yn = self.c["jog.continuous.y.negative"]
        if yn ^ self.yn: self.lbv_control.continuous_jog(1, -yn)
        self.yn = yn

        zp = self.c["jog.continuous.z.positive"]
        if zp ^ self.zp: self.lbv_control.continuous_jog(2, zp)
        self.zp = zp

        zn = self.c["jog.continuous.z.negative"]
        if zn ^ self.zn: self.lbv_control.continuous_jog(2, -zn)
        self.zn = zn

        ap = self.c["jog.continuous.a.positive"]
        if ap ^ self.ap: self.lbv_control.continuous_jog(3, ap)
        self.ap = ap

        an = self.c["jog.continuous.a.negative"]
        if an ^ self.an: self.lbv_control.continuous_jog(3, -an)
        self.an = an

        bp = self.c["jog.continuous.b.positive"]
        if bp ^ self.bp: self.lbv_control.continuous_jog(4, bp)
        self.bp = bp

        bn = self.c["jog.continuous.b.negative"]
        if bn ^ self.bn: self.lbv_control.continuous_jog(4, -bn)
        self.bn = bn

        cp = self.c["jog.continuous.c.positive"]
        if cp ^ self.cp: self.lbv_control.continuous_jog(5, cp)
        self.cp = cp

        cn = self.c["jog.continuous.c.negative"]
        if cn ^ self.cn: self.lbv_control.continuous_jog(5, -cn)
        self.cn = cn

        up = self.c["jog.continuous.u.positive"]
        if up ^ self.up: self.lbv_control.continuous_jog(6, up)
        self.up = up

        un = self.c["jog.continuous.u.negative"]
        if un ^ self.un: self.lbv_control.continuous_jog(6, -un)
        self.un = un

        vp = self.c["jog.continuous.v.positive"]
        if vp ^ self.vp: self.lbv_control.continuous_jog(7, vp)
        self.vp = vp

        vn = self.c["jog.continuous.v.negative"]
        if vn ^ self.vn: self.lbv_control.continuous_jog(7, -vn)
        self.vn = vn

        wp = self.c["jog.continuous.w.positive"]
        if wp ^ self.wp: self.lbv_control.continuous_jog(8, wp)
        self.wp = wp

        wn = self.c["jog.continuous.w.negative"]
        if wn ^ self.wn: self.lbv_control.continuous_jog(8, -wn)
        self.wn = wn

        quillup = self.c["quill-up"]
        if quillup and not self.quillup: 
            self.lbv_control.quill_up()
        self.quillup = quillup

        singleblock = self.c["single-block"]
        if singleblock ^ self.singleblock: self.lbv_control.single_block(singleblock)
        ...
        if cyclestart and not self.cyclestart:
            ...
            if mdi_mode:
                ...
                self.lbv_control.cycle_start()
        self.cyclestart = cyclestart

        abort = self.c["abort"]
        if abort and not self.abort: self.lbv_control.abort()
        self.abort = abort

        self.lbv_stat.poll()
        self.c["jog.active"] = self.lbv_stat.task_mode == self.lbv.MODE_MANUAL

        if self.lbv_stat.paused:
            # blink
            self.c["status-indicator"] = not self.c["status-indicator"]
        else:
            if self.lbv_stat.queue > 0 or self.lbv_stat.interp_state != self.lbv.INTERP_IDLE:
