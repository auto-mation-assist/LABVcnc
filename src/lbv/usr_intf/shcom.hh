...
extern int lbvCommandSerialNumber;

// the NML channels to the LBV task
extern RCS_CMD_CHANNEL *lbvCommandBuffer;
extern RCS_STAT_CHANNEL *lbvStatusBuffer;
// LBV_STAT *lbvStatus;

// the NML channel for errors
extern NML *lbvErrorBuffer;
...
extern double lbvTimeout;

enum LBV_UPDATE_TYPE {
    LBV_UPDATE_NONE = 1,
    LBV_UPDATE_AUTO
};
extern LBV_UPDATE_TYPE lbvUpdateType;

enum LBV_WAIT_TYPE {
    LBV_WAIT_RECEIVED = 2,
    LBV_WAIT_DONE
};
extern LBV_WAIT_TYPE lbvWaitType;
...
extern int lbvTaskNmlGet();
extern int lbvErrorNmlGet();
extern int tryNml(double retry_time=10.0, double retry_interval=1.0);
extern int updateStatus();
extern int updateError();
extern int lbvCommandWaitReceived();
extern int lbvCommandWaitDone();
extern int lbvCommandSend(RCS_CMD_MSG & cmd);
