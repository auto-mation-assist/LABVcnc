/********************************************************************
* Description: lbvlcd.cc
*   LBV interface for LCD displays
*
...
static int loadFiles()
{
  ...
  if ((dp = opendir(LBV2_NCFILES_DIR)) == NULL) {
    printf("Cannot open nc files folder: %s\n", LBV2_NCFILES_DIR);
    return -1;
    }

  if(!chdir(LBV2_NCFILES_DIR)) { perror("chdir"); return -1; }
...
static int copyFiles(int fromUSB)
{
  ...
  if (opendir(LBV2_NCFILES_DIR) == NULL) {
    printf("Cannot open nc files folder: %s\n", LBV2_NCFILES_DIR);
    return -1;
    }
  if(!chdir(LBV2_NCFILES_DIR)) { perror("chdir"); return -1; }
...
static int openProgram(char *s)
{
  ...
  sprintf(fname, "%s/%s.ngc", LBV2_NCFILES_DIR, s);
...
static int jog(int jnum, float speed)
{
  ...
  if (lbvStatus->task.mode != LBV_TASK_MODE_MANUAL) return 0;
...

{
  switch (lbvStatus->task.mode) {
    case LBV_TASK_MODE_MANUAL: sendAuto(); break;
    case LBV_TASK_MODE_AUTO: sendManual(); break;
    case LBV_TASK_MODE_MDI: sendAuto(); break;
    }
  return 0;
}
...
static int nextKey()
{
  ...
  if ((curScreen == stMain) && (lbvStatus->task.mode == LBV_TASK_MODE_MANUAL)) {
    temp = jogMode;
    temp--;
    if (temp < 0) temp = 4;
...
static void slowLoop()
{
  ...
  if (lbvStatus->task.file[0] != 0) {
    fname = extractFileName(lbvStatus->task.file);
    if (strcmp(fname, programName) != 0) {
      ...
      totalSteps = stepCount(lbvStatus->task.file);
      }
    }

  feedOverride = (int)floor(lbvStatus->motion.traj.scale * 100.0 + 0.5);
...
static void updatePositions()
{
  ...
  if (units == unmm)
    sprintf(numStr, "%7.2f", lbvStatus->motion.traj.actualPosition.tran.x * conversion);
  else
    sprintf(numStr, "%7.3f", lbvStatus->motion.traj.actualPosition.tran.x * conversion);
  widgetSetStr(XPOSWIDGET, numStr, oldXStr);   
  if (units == unmm)
    sprintf(numStr, "%7.2f", lbvStatus->motion.traj.actualPosition.tran.y * conversion);
  else
    sprintf(numStr, "%7.3f", lbvStatus->motion.traj.actualPosition.tran.y * conversion);
  widgetSetStr(YPOSWIDGET, numStr, oldYStr);
  if (units == unmm)
    sprintf(numStr, "%6.2f", lbvStatus->motion.traj.actualPosition.tran.z * conversion);
  else
    sprintf(numStr, "%6.2f", lbvStatus->motion.traj.actualPosition.tran.z * conversion);
