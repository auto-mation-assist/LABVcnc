/********************************************************************
* Description: lbvrsh.cc  (source for labvcncrsh)
*   Extended telnet based LabvCNC interface
*
...
/*
  ...
  labvcncrsh {-- --port <port number> --name <server name> --connectpw <password>
  ...
  With -- --connectpw <password> Sets the connection password to 'password'. Default LBV
  With -- --enablepw <password> Sets the enable password to 'password'. Default LBVTOO
  ...
  There are six commands supported, Where the commands set and get contain LabvCNC
  specific sub-commands based on the commands supported by labvcncrsh, but where the 
  usual prefix ( "lbv_") is omitted. Commands and most parameters are not case sensitive.
  ...
  The get command includes one of the LabvCNC sub-commands, described below and
  zero or more additional parameters. 
  
  ==> Set <==
  
  The set command inclides one of the LabvCNC sub-commands, described below and
  ...
  The shutdown command tells LabvCNC to shutdown before quitting the connection. This
  ...
  debug {<new value>}
  With get, returns the integer value of LBV_DEBUG, in LabvCNC. Note that
  it may not be true that the local LBV_DEBUG variable here (in labvcncrsh and
  the GUIs that use it) is the same as the LBV_DEBUG value in the LabvCNC. This
  can happen if LabvCNC is started from one .ini file, and the GUI is started
  with another that has a different value for DEBUG.
  With set, sends a command to the LabvCNC to set the new debug level,
  ...
  With no arg, forces an update of the LabvCNC status. With "none", doesn't
  ...
  error
  Returns the current LabvCNC error string, or "ok" if no error.

  operator_display
  Returns the current LabvCNC operator display string, or "ok" if none.

  operator_text
  Returns the current LabvCNC operator text string, or "ok" if none.
  ...
  LBV_AXIS_STAT::units numerical value of user units per mm or deg.
  ...
  corresponding native user linear units of the LabvCNC trajectory
  ...
  user angular units of the LabvCNC trajectory level. Like with linear units,
  ...
  4> Get shutdown to notify LabvCNC to actually shutdown.
*/
...
int enabledConn = -1;
char pwd[16] = "LBV\0";
char enablePWD[16] = "LBVTOO\0";
char serverName[24] = "LBVNETSVR\0";
int sessions = 0;
int maxSessions = -1;
...
static cmdResponseType setSetWait(char *s, connectionRecType *context)
{
   switch (checkReceivedDoneNone(s)) {
     ...
     case 2: {
       fprintf(stderr, "labvcncrsh: 'set set_wait' asked for 'none', but that setting has been removed as it may cause commands to be lost\n");
...
static cmdResponseType setWait(char *s, connectionRecType *context)
{
  switch (checkReceivedDoneNone(s)) {
    case -1: return rtStandardError;
    case 0: 
      if (lbvCommandWaitReceived() != 0) return rtStandardError;
      break;
    case 1: 
      if (lbvCommandWaitDone() != 0) return rtStandardError;
      break;
    case 2: ;
...
static cmdResponseType setUpdate(char *s, connectionRecType *context)
{
  switch (checkNoneAuto(s)) {
    case 0: lbvUpdateType = LBV_UPDATE_NONE; break;
    case 1: lbvUpdateType = LBV_UPDATE_AUTO; break;
    default: return rtStandardError;
    }
  return rtNoError;
}
...
static cmdResponseType setBrake(char *s, connectionRecType *context)
{
	...
	s = strtok(NULL, delims);
	if (sscanf(s, "%d", &spindle) > 0){// there is a spindle number
		if (spindle < 0 || spindle > LBVMOT_MAX_SPINDLES) return rtStandardError;
		s = strtok(NULL, delims);
	} else {
		spindle = -1;
	}
...
static cmdResponseType setHome(char *s, connectionRecType *context)
{
  ...
  if ((joint < -1) || (joint > LBVMOT_MAX_JOINTS)) return rtStandardError;
  sendHome(joint);
  return rtNoError;
}
...
static cmdResponseType setOpen(char *s, connectionRecType *context)
{
  ...
  if (context->progName[sizeof(context->progName) - 1] != '\0') {
    fprintf(stderr, "labvcncrsh: 'set open' filename too long for context (got %lu bytes, max %lu)", (unsigned long)strlen(pch), (unsigned long)sizeof(context->progName));
    return rtStandardError;
  }
...
int commandSet(connectionRecType *context)
{
  ...
  if ((cmd > scMachine) && (lbvStatus->task.state != LBV_TASK_STATE_ON)) {
...
static cmdResponseType getDebug(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pUpdateStr, lbvStatus->debug);
  return rtNoError;
}
...
static cmdResponseType getSetWait(char *s, connectionRecType *context)
{
  ...
  switch (lbvWaitType) {
    case LBV_WAIT_RECEIVED: sprintf(context->outBuf, pSetWaitStr, "RECEIVED"); break;
    case LBV_WAIT_DONE: sprintf(context->outBuf, pSetWaitStr, "DONE"); break;
    default: return rtStandardError;
    }
  return rtNoError;
}
...
static cmdResponseType getTimeout(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pTimeoutStr, lbvTimeout);
  return rtNoError;
}
...
static cmdResponseType getError(char *s, connectionRecType *context)
{
  ...
  if (updateError() != 0)
    sprintf(context->outBuf, pErrorStr, "lbv_error: bad status from LabvCNC");
  else
    if (error_string[0] == 0)
...
static cmdResponseType getOperatorDisplay(char *s, connectionRecType *context)
{
  ...
  if (updateError() != 0)
    sprintf(context->outBuf, pOperatorDisplayStr, "lbv_operator_display: bad status from LabvCNC");
  else
...
static cmdResponseType getOperatorText(char *s, connectionRecType *context)
{
  ...
  if (updateError() != 0)
    sprintf(context->outBuf, pOperatorTextStr, "lbv_operator_text: bad status from LabvCNC");
  else
...
static cmdResponseType getMode(char *s, connectionRecType *context)
{
  ...
  switch (lbvStatus->task.mode) {
    case LBV_TASK_MODE_MANUAL: sprintf(context->outBuf, pModeStr, "MANUAL"); break;
    case LBV_TASK_MODE_AUTO: sprintf(context->outBuf, pModeStr, "AUTO"); break;
    case LBV_TASK_MODE_MDI: sprintf(context->outBuf, pModeStr, "MDI"); break;
...
static cmdResponseType getSpindle(char *s, connectionRecType *context)
{
  ...
  for (n = 0; n < lbvStatus->motion.traj.spindles; n++){
	  if (n == spindle || spindle == -1){
		  if (lbvStatus->motion.spindle[n].increasing > 0)
			sprintf(context->outBuf, pSpindleStr, n, "INCREASE");
		  else
			if (lbvStatus->motion.spindle[n].increasing < 0)
			  sprintf(context->outBuf, pSpindleStr, n, "DECREASE");
			else
			  if (lbvStatus->motion.spindle[n].direction > 0)
...
static cmdResponseType getTool(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pToolStr, lbvStatus->io.tool.toolInSpindle);
  return rtNoError; 
}
...
static cmdResponseType getToolOffset(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pToolOffsetStr, lbvStatus->task.toolOffset.tran.z);
  return rtNoError; 
}
...
static cmdResponseType getAbsCmdPos(char *s, connectionRecType *context)
{
  ...
  if ((axis == -1) || (axis == 0)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.tran.x);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 1)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.tran.y);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 2)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.tran.z);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 3)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.a);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 4)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.b);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 5)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.c);
    strcat(context->outBuf, buf);
    }
  return rtNoError;
}
...
static cmdResponseType getAbsActPos(char *s, connectionRecType *context)
{
  ...
  if ((axis == -1) || (axis == 0)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.tran.x);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 1)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.tran.y);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 2)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.tran.z);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 3)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.a);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 4)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.b);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 5)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.c);
    strcat(context->outBuf, buf);
    }
  return rtNoError;
}
...
static cmdResponseType getRelCmdPos(char *s, connectionRecType *context)
{
  ...
  if ((axis == -1) || (axis == 0)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.tran.x - lbvStatus->task.g5x_offset.tran.x - lbvStatus->task.g92_offset.tran.x);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 1)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.tran.y - lbvStatus->task.g5x_offset.tran.y - lbvStatus->task.g92_offset.tran.y);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 2)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.tran.z - lbvStatus->task.g5x_offset.tran.z - lbvStatus->task.g92_offset.tran.z);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 3)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.a - lbvStatus->task.g5x_offset.a - lbvStatus->task.g92_offset.a);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 4)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.b - lbvStatus->task.g5x_offset.b - lbvStatus->task.g92_offset.b);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 5)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.position.c - lbvStatus->task.g5x_offset.c - lbvStatus->task.g92_offset.c);
    strcat(context->outBuf, buf);
    }
  return rtNoError;
}
...
static cmdResponseType getRelActPos(char *s, connectionRecType *context)
{
  ...
  if ((axis == -1) || (axis == 0)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.tran.x - lbvStatus->task.g5x_offset.tran.x - lbvStatus->task.g92_offset.tran.x);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 1)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.tran.y - lbvStatus->task.g5x_offset.tran.y - lbvStatus->task.g92_offset.tran.y);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 2)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.tran.z - lbvStatus->task.g5x_offset.tran.z - lbvStatus->task.g92_offset.tran.z);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 3)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.a - lbvStatus->task.g5x_offset.a - lbvStatus->task.g92_offset.a);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 4)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.b - lbvStatus->task.g5x_offset.b - lbvStatus->task.g92_offset.b);
    strcat(context->outBuf, buf);
    }
  if ((axis == -1) || (axis == 5)) {
    sprintf(buf, " %f", lbvStatus->motion.traj.actualPosition.c - lbvStatus->task.g5x_offset.c - lbvStatus->task.g92_offset.c);
    strcat(context->outBuf, buf);
    }
  return rtNoError;
}
...
static cmdResponseType getJointPos(char *s, connectionRecType *context)
{
  ...
  if (joint == -1) {
    ...
    for (i=0; i<6; i++) {
      sprintf(buf, " %f", lbvStatus->motion.joint[i].input);
      strcat(context->outBuf, buf);
      }
    }
  else
    sprintf(context->outBuf, "%s %d %f", pJointPos, joint, lbvStatus->motion.joint[joint].input);
...
static cmdResponseType getPosOffset(char *s, connectionRecType *context)
{
  ...
  if (s == NULL) {
    ...
    sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.tran.x + lbvStatus->task.g92_offset.tran.x));
    strcat(context->outBuf, buf);
    sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.tran.y + lbvStatus->task.g92_offset.tran.y));
    strcat(context->outBuf, buf);
    sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.tran.z + lbvStatus->task.g92_offset.tran.z));
    strcat(context->outBuf, buf);
    sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.a + lbvStatus->task.g92_offset.a));
    strcat(context->outBuf, buf);
    sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.b + lbvStatus->task.g92_offset.b));
    strcat(context->outBuf, buf);
    sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.c + lbvStatus->task.g92_offset.c));
    strcat(context->outBuf, buf);
    }
  else
    {
      switch (s[0]) {
        case 'X': sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.tran.x + lbvStatus->task.g92_offset.tran.x)); break;
        case 'Y': sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.tran.y + lbvStatus->task.g92_offset.tran.y)); break;
        case 'Z': sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.tran.z + lbvStatus->task.g92_offset.tran.z)); break;
        case 'A': 
        case 'R': sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.a + lbvStatus->task.g92_offset.a)); break;
        case 'B': 
        case 'P': sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.b + lbvStatus->task.g92_offset.b)); break;
        case 'C': 
        case 'W': sprintf(buf, " %f", convertLinearUnits(lbvStatus->task.g5x_offset.c + lbvStatus->task.g92_offset.c));
      }
...
static cmdResponseType getJointLimit(char *s, connectionRecType *context)
{
  ...
  if (s == NULL) {
    ...
    for (i=0; i<6; i++) {
      if (lbvStatus->motion.joint[i].minHardLimit)
        strcpy(buf, " MINHARD");
      else
        if (lbvStatus->motion.joint[i].minSoftLimit)
	  strcpy(buf, " MINSOFT");
	else
	  if (lbvStatus->motion.joint[i].maxSoftLimit)
	    strcpy(buf, " MAXSOFT");
	  else
	    if (lbvStatus->motion.joint[i].maxHardLimit)
	      strcpy(buf, " MAXHARD");
	    else strcpy(buf, " OK");
      strcat(context->outBuf, buf);
      }
  ...
  else
    {
      ...
      if (lbvStatus->motion.joint[joint].minHardLimit)
        ...
        if (lbvStatus->motion.joint[joint].minSoftLimit)
	  strcpy(buf, "MINSOFT");
	else
	  if (lbvStatus->motion.joint[joint].maxSoftLimit)
	    strcpy(buf, "MAXSOFT");
	  else
	    if (lbvStatus->motion.joint[joint].maxHardLimit)
	      strcpy(buf, "MAXHARD");
	    else strcpy(buf, "OK");
...
static cmdResponseType getJointFault(char *s, connectionRecType *context)
{
  ...
  if (s == NULL) {
    ...
    for (i=0; i<6; i++) {
      if (lbvStatus->motion.joint[i].fault)
...
static cmdResponseType getOverrideLimits(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pOverrideLimits, lbvStatus->motion.joint[0].overrideLimits);
  return rtNoError;
}
...
static cmdResponseType getJointHomed(char *s, connectionRecType *context)
{
  ...
  if (s == NULL) {
    ...
    for (i=0; i<6; i++) {
      if (lbvStatus->motion.joint[i].homed)
...
//  printf("Program name = %s", lbvStatus->task.file[0]);
  if (lbvStatus->task.file[0] != 0)
    sprintf(context->outBuf, pProgram, lbvStatus->task.file);
  return rtNoError;
}
...
static cmdResponseType getProgramLine(char *s, connectionRecType *context)
{
  ...
  if ((programStartLine< 0) || (lbvStatus->task.readLine < programStartLine))
    lineNo = lbvStatus->task.readLine;
  else
    if (lbvStatus->task.currentLine > 0)
      if ((lbvStatus->task.motionLine > 0) && 
        (lbvStatus->task.motionLine < lbvStatus->task.currentLine))
	  lineNo = lbvStatus->task.motionLine;
      else lineNo = lbvStatus->task.currentLine;
    else lineNo = 0;
...
static cmdResponseType getProgramStatus(char *s, connectionRecType *context)
{
  ...
  switch (lbvStatus->task.interpState) {
      ...
      case LBV_TASK_INTERP_WAITING: sprintf(context->outBuf, pProgramStatus, "RUNNING"); break;
      case LBV_TASK_INTERP_PAUSED: sprintf(context->outBuf, pProgramStatus, "PAUSED"); break;
...
static cmdResponseType getProgramCodes(char *s, connectionRecType *context)
{
  ...
  for (i=1; i<ACTIVE_G_CODES; i++) {
      code = lbvStatus->task.activeGCodes[i];
      if (code == -1) continue;
      if (code % 10) sprintf(buf, "G%.1f ", (double) code / 10.0);
      else sprintf(buf, "G%d ", code / 10);
      strcat(context->outBuf, buf);
    }
  sprintf(buf, "F%.0f ", lbvStatus->task.activeSettings[1]);
  strcat(context->outBuf, buf);
  sprintf(buf, "S%.0f", fabs(lbvStatus->task.activeSettings[2]));
  strcat(context->outBuf, buf);
  return rtNoError;
}
...
static cmdResponseType getJointType(char *s, connectionRecType *context)
{
  ...
  if (s == NULL) {
    ...
    for (i=0; i<6; i++) {
      switch (lbvStatus->motion.joint[i].jointType) {
        case LBV_LINEAR: strcat(context->outBuf, " LINEAR"); break;
	case LBV_ANGULAR: strcat(context->outBuf, " ANGULAR"); break;
	default: strcat(context->outBuf, "CUSTOM");
      ...
      switch (lbvStatus->motion.joint[joint].jointType) {
        case LBV_LINEAR: strcpy(buf, " LINEAR"); break;
	case LBV_ANGULAR: strcpy(buf, " ANGULAR"); break;
...
static cmdResponseType getJointUnits(char *s, connectionRecType *context)
{
  ...
  if (s == NULL) {
    ...
    for (i=0; i<6; i++) {
      switch (lbvStatus->motion.joint[i].jointType) {
        case LBV_LINEAR: 
	  if (CLOSE(lbvStatus->motion.joint[i].units, 1.0, LINEAR_CLOSENESS))
	    strcat(context->outBuf, " MM");
	  else 
	    if (CLOSE(lbvStatus->motion.joint[i].units, INCH_PER_MM,
	      LINEAR_CLOSENESS)) strcat(context->outBuf, " INCH");
	    else
	      if (CLOSE(lbvStatus->motion.joint[i].units, CM_PER_MM,
	        LINEAR_CLOSENESS)) strcat(context->outBuf, " CM");
	      else strcat(context->outBuf, " CUSTOM");
	  break;
	case LBV_ANGULAR:
	  if (CLOSE(lbvStatus->motion.joint[i].units, 1.0, ANGULAR_CLOSENESS))
	    strcat(context->outBuf, " DEG");
	  else
  	    if (CLOSE(lbvStatus->motion.joint[i].units, RAD_PER_DEG, ANGULAR_CLOSENESS))
	      strcat(context->outBuf, " RAD");
	    else
	      if (CLOSE(lbvStatus->motion.joint[i].units, GRAD_PER_DEG, ANGULAR_CLOSENESS))
	...
	default: strcat(context->outBuf, " CUSTOM");
      ...
      switch (lbvStatus->motion.joint[joint].jointType) {
        case LBV_LINEAR: 
	  if (CLOSE(lbvStatus->motion.joint[joint].units, 1.0, LINEAR_CLOSENESS))
	    strcpy(buf, "MM");
	  else 
	    if (CLOSE(lbvStatus->motion.joint[joint].units, INCH_PER_MM,
	      LINEAR_CLOSENESS)) strcpy(buf, "INCH");
	    else
	      if (CLOSE(lbvStatus->motion.joint[joint].units, CM_PER_MM,
	        LINEAR_CLOSENESS)) strcpy(buf, "CM");
	      else strcpy(buf, "CUSTOM");
	  break;
	case LBV_ANGULAR:
	  if (CLOSE(lbvStatus->motion.joint[joint].units, 1.0, ANGULAR_CLOSENESS))
	    strcpy(buf, "DEG");
	  else
  	    if (CLOSE(lbvStatus->motion.joint[joint].units, RAD_PER_DEG, ANGULAR_CLOSENESS))
	      strcpy(buf, "RAD");
	    else
	      if (CLOSE(lbvStatus->motion.joint[joint].units, GRAD_PER_DEG, ANGULAR_CLOSENESS))
	        strcpy(buf, "GRAD");
	      else strcpy(buf, "CUSTOM");
	  break;
	default: strcpy(buf, "CUSTOM");
...
static cmdResponseType getUserLinearUnits(char *s, connectionRecType *context)
{
  ...
  if (CLOSE(lbvStatus->motion.traj.linearUnits, 1.0, LINEAR_CLOSENESS))
    ...
    if (CLOSE(lbvStatus->motion.traj.linearUnits, INCH_PER_MM, LINEAR_CLOSENESS))
      sprintf(context->outBuf, userLinearUnits, "INCH");
    else
      if (CLOSE(lbvStatus->motion.traj.linearUnits, CM_PER_MM, LINEAR_CLOSENESS))
...
static cmdResponseType getUserAngularUnits(char *s, connectionRecType *context)
{
  ...
  if (CLOSE(lbvStatus->motion.traj.angularUnits, 1.0, ANGULAR_CLOSENESS))
    ...
    if (CLOSE(lbvStatus->motion.traj.angularUnits, RAD_PER_DEG, ANGULAR_CLOSENESS))
      sprintf(context->outBuf, pUserAngularUnits, "RAD");
    else
      if (CLOSE(lbvStatus->motion.traj.angularUnits, GRAD_PER_DEG, ANGULAR_CLOSENESS))
...
static cmdResponseType getProbeValue(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pProbeValue, lbvStatus->motion.traj.probeval);  
  return rtNoError;
}
...
static cmdResponseType getProbeTripped(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pProbeTripped, lbvStatus->motion.traj.probe_tripped);  
  return rtNoError;
}
...
static cmdResponseType getKinematicsType(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pKinematicsType, lbvStatus->motion.traj.kinematics_type);  
  return rtNoError;
}
...
static cmdResponseType getFeedOverride(char *s, connectionRecType *context)
{
  ...
  percent = (int)floor(lbvStatus->motion.traj.scale * 100.0 + 0.5);
...
static cmdResponseType getIniFile(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pIniFile, lbv_inifile);
  return rtNoError;
}
...
static cmdResponseType getSpindleOverride(char *s, connectionRecType *context)
{
  ...
  for (n = 0; n < lbvStatus->motion.traj.spindles; n++){
	  if (n == spindle || spindle == -1){
		  percent = (int)floor(lbvStatus->motion.spindle[n].spindle_scale * 100.0 + 0.5);
...
static cmdResponseType getOptionalStop(char *s, connectionRecType *context)
{
  ...
  sprintf(context->outBuf, pOptionalStop, lbvStatus->task.optional_stop_state);
  return rtNoError;
}
...
int commandGet(connectionRecType *context)
{
  ...
  if (lbvUpdateType == LBV_UPDATE_AUTO) updateStatus();
  strupr(pch);
  cmd = lookupSetCommand(pch);
  if (cmd > scIni)
    if (lbvUpdateType == LBV_UPDATE_AUTO) updateStatus();
...
static int helpGeneral(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  Get <LabvCNC command>\n\r");
  strcat(context->outBuf, "  Set <LabvCNC command>\n\r");
...
static int helpHello(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  Server Name is the name of the LabvCNC Server to which the client has connected.\n\r");
...
static int helpShutdown(connectionRecType *context)
{
  ...
  strcat(context->outBuf, "  and initiates a shutdown of LabvCNC. The command has no parameters, and\n\r");
...
// handle the labvcncrsh command in context->inBuf
...
void *readClient(void *arg)
{
  ...
  while (1) {
    ...
    if (len < 0) {
      fprintf(stderr, "labvcncrsh: error reading from client: %s\n", strerror(errno));
      goto finished;
    }
    if (len == 0) {
      printf("labvcncrsh: eof from client\n");
      goto finished;
    }

    if (context->echo && context->linked)
      if(write(context->cliSock, buf, len) != (ssize_t)len) {
        fprintf(stderr, "labvcncrsh: write() failed: %s", strerror(errno));
      }
...
finished:
  printf("labvcncrsh: disconnecting client %s (%s)\n", context->hostName, context->version);
...
int sockMain()
{
    ...
    while (1) {
      ...
      if ((maxSessions == -1) || (sessions <= maxSessions)) {
        ...
        if (thrd == NULL) {
          fprintf(stderr, "labvcncrsh: out of memory\n");
          exit(1);
        }
...
static void usage(char* pname) {
    printf("Usage: \n");
    printf("         %s [Options] [-- LabvCNC_Options]\n"
           "Options:\n"
           "         --help       this help\n"
          ...
          ,pname,port,serverName,pwd,enablePWD,maxSessions,defaultPath,lbv_inifile
          );
}
...
int main(int argc, char *argv[])
{
    ...
    // init NML
    if (tryNml() != 0) {
	rcs_print_error("can't connect to LabvCNC\n");
	thisQuit();
	exit(1);
    }
