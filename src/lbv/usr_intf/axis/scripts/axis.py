#!/usr/bin/env python2
#    This is a component of AXIS, a front-end for LabvCNC
...
inifile = labvcnc.ini(sys.argv[2])
...
root_window.tk.call("set", "version", labvcnc.version)

try:
    root_window.tk.call("set","::MAX_JOINTS"         ,labvcnc.MAX_JOINTS)
    root_window.tk.call("set","::MAX_AXIS"           ,labvcnc.MAX_AXIS)
    root_window.tk.call("set","::STATE_ESTOP"        ,labvcnc.STATE_ESTOP)
    root_window.tk.call("set","::STATE_ESTOP_RESET"  ,labvcnc.STATE_ESTOP_RESET)
    root_window.tk.call("set","::STATE_OFF"          ,labvcnc.STATE_OFF)
    root_window.tk.call("set","::STATE_ON"           ,labvcnc.STATE_ON)
    root_window.tk.call("set","::TASK_MODE_MANUAL"   ,labvcnc.MODE_MANUAL)
    root_window.tk.call("set","::TASK_MODE_MDI"      ,labvcnc.MODE_MDI)
    root_window.tk.call("set","::TASK_MODE_AUTO"     ,labvcnc.MODE_AUTO)
    root_window.tk.call("set","::INTERP_IDLE"        ,labvcnc.INTERP_IDLE)
    root_window.tk.call("set","::INTERP_READING"     ,labvcnc.INTERP_READING)
    root_window.tk.call("set","::INTERP_PAUSED"      ,labvcnc.INTERP_PAUSED)
    root_window.tk.call("set","::INTERP_WAITING"     ,labvcnc.INTERP_WAITING)
    root_window.tk.call("set","::TRAJ_MODE_FREE"     ,labvcnc.TRAJ_MODE_FREE)
    root_window.tk.call("set","::KINEMATICS_IDENTITY",labvcnc.KINEMATICS_IDENTITY)
...
def General_Halt():
    text = _("Do you really want to close LabvCNC?")
...
def install_help(app):
    ...
    fixed = app.tk.call("labvcnc::standard_fixed_font")
...
def joints_mode():
    return s.motion_mode == labvcnc.TRAJ_MODE_FREE
...
def masked_axes_count():
    ct = 0
    for i in range(labvcnc.MAX_JOINTS):
...
class LivePlotter:
    ...
    def start(self):
        ...
        if not os.path.exists(labvcnc.nmlfile):
            return False
        try:
            self.stat = labvcnc.stat()
        except labvcnc.error:
            return False
    ...
    def error_task(self):
        ...
        while error:
            kind, text = error
            if kind in (labvcnc.NML_ERROR, labvcnc.OPERATOR_ERROR):
                icon = "error"
            else:
                icon = "info"
    ...
    def update(self):
        ...
        try:
            self.stat.poll()
        except labvcnc.error, detail:
            print "error", detail
            del self.stat
            return

        if  (   (self.stat.motion_mode == labvcnc.TRAJ_MODE_COORD)
            and (self.stat.task_mode   == labvcnc.MODE_MANUAL)
            ):
            set_motion_teleop(1)
        if      ( (self.stat.motion_mode == labvcnc.TRAJ_MODE_TELEOP)
            and   not vars.teleop_mode.get() ):
            vars.teleop_mode.set(1)

        # With joints_axes support and jogging while mdi
        #  (ref: lbvtaskmain.cc:allow_while_idle_type())
        ...
        if (     enable_tab_change
            ...
            if (self.stat.task_mode == labvcnc.MODE_MANUAL):
                root_window.tk.eval(pane_top + ".tabs raise manual")
            if (self.stat.task_mode == labvcnc.MODE_MDI):
                root_window.tk.eval(pane_top + ".tabs raise mdi")
            if (self.stat.task_mode == labvcnc.MODE_AUTO):
...
This means this function returns True when the mdi tab is visible."""
    if do_poll: s.poll()
    if s.task_state != labvcnc.STATE_ON: return False
    return s.interp_state == labvcnc.INTERP_IDLE or (s.task_mode == labvcnc.MODE_MDI and s.queued_mdi_commands < vars.max_queued_mdi_commands.get())

# If LabvCNC is not already in one of the modes given, switch it to the
...
def open_file_guts(f, filtered=False, addrecent=True):
    ...
    ensure_mode(labvcnc.MODE_MANUAL)
    if addrecent:
        add_recent_file(f)
    if not filtered:
        global loaded_file
    ...
    try:
        ...
        initcode = inifile.find("LBV", "RS274NGC_STARTUP_CODE") or ""
        if initcode == "":
...
for j in range(labvcnc.MAX_JOINTS):
...
# https://github.com/LabvCNC/labvcnc/issues/146
...
def activate_ja_widget(i, force=0):
    ...
    if get_jog_mode() and (not kins_is_trivkins or (kins_is_trivkins and s.kinematics_type == labvcnc.KINEMATICS_BOTH)):
...
def ja_from_rbutton():
    ...
    try: # ja may be a joint number or an axis coordinate letter
        ...
        if a not in range(labvcnc.MAX_JOINTS):
    ...
    if get_jog_mode():
        ...
        elif kins_is_trivkins and s.kinematics_type == labvcnc.KINEMATICS_IDENTITY:
...
def motion_modename(x):
    if x ==   labvcnc.TRAJ_MODE_FREE: return "FREE"
    if x ==  labvcnc.TRAJ_MODE_COORD: return "COORD"
    if x == labvcnc.TRAJ_MODE_TELEOP: return "TELEOP"

def task_modename(x):
    if x ==    labvcnc.MODE_MDI: return "MDI"
    if x == labvcnc.MODE_MANUAL: return "MANUAL"
    if x ==   labvcnc.MODE_AUTO: return "AUTO"

def task_statename(x):
    if x ==       labvcnc.STATE_ESTOP: return "STATE_ESTOP"
    if x == labvcnc.STATE_ESTOP_RESET: return "STATE_ESTOP_RESET"
    if x ==         labvcnc.STATE_OFF: return "STATE_OFF"
    if x ==          labvcnc.STATE_ON: return "STATE_ON"

def interp_statename(x):
    if x ==    labvcnc.INTERP_IDLE: return "IDLE"
    if x == labvcnc.INTERP_READING: return "READING"
    if x ==  labvcnc.INTERP_PAUSED: return "PAUSED"
    if x == labvcnc.INTERP_WAITING: return "WAITING"
...
class TclCommands(nf.TclCommands):
    ...
    def launch_website(event=None):
        import webbrowser
        webbrowser.open("http://www.labvcnc.org/")
    ...
    def estop_clicked(event=None):
        s.poll()
        if s.task_state == labvcnc.STATE_ESTOP:
            c.state(labvcnc.STATE_ESTOP_RESET)
        else:
            c.state(labvcnc.STATE_ESTOP)

    def onoff_clicked(event=None):
        s.poll()
        if s.task_state == labvcnc.STATE_ESTOP_RESET:
            c.state(labvcnc.STATE_ON)
        else:
            c.state(labvcnc.STATE_OFF)
    ...
    def task_run(*event):
        ...
        ensure_mode(labvcnc.MODE_AUTO)
        c.auto(labvcnc.AUTO_RUN, program_start_line)
        program_start_line = 0
        t.tag_remove("ignored", "0.0", "end")
        o.set_highlight_line(None)

    def task_step(*event):
        if s.task_mode != labvcnc.MODE_AUTO or s.interp_state != labvcnc.INTERP_IDLE:
            o.set_highlight_line(None)
            if run_warn(): return
        ensure_mode(labvcnc.MODE_AUTO)
        c.auto(labvcnc.AUTO_STEP)

    def task_pause(*event):
        if s.task_mode != labvcnc.MODE_AUTO or s.interp_state not in (labvcnc.INTERP_READING, labvcnc.INTERP_WAITING):
            return
        ensure_mode(labvcnc.MODE_AUTO)
    ...
    def task_reverse(*event):
        s.poll()
        if s.task_mode != labvcnc.MODE_AUTO:
            return
    ...
    def task_resume(*event):
        ...
        if not s.paused:
            return
        if s.task_mode not in (labvcnc.MODE_AUTO, labvcnc.MODE_MDI):
            return
        ensure_mode(labvcnc.MODE_AUTO, labvcnc.MODE_MDI)
        c.auto(labvcnc.AUTO_RESUME)
    ...
    def task_pauseresume(*event):
        ...
        if s.paused:
            ...
            c.auto(labvcnc.AUTO_RESUME)
        elif s.interp_state != labvcnc.INTERP_IDLE:
            c.auto(labvcnc.AUTO_PAUSE)

    def task_stop(*event):
        if s.task_mode == labvcnc.MODE_AUTO and vars.running_line.get() != 0:
    ...
    def ensure_manual(*event):
        ...
        if  (     all_homed()
             and (s.motion_mode == labvcnc.TRAJ_MODE_TELEOP)
                  or
                 (s.motion_mode == labvcnc.TRAJ_MODE_COORD)
            ):
            set_motion_teleop(1)
        else:
            ensure_mode(labvcnc.MODE_MANUAL)
    ...
    def home_joint(event=None):
        ...
        if jora in trajcoordinates:
            if s.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
    ...
    def spindle_increase(event=None):
        c.spindle(labvcnc.SPINDLE_INCREASE)
    def spindle_decrease(event=None):
        c.spindle(labvcnc.SPINDLE_DECREASE)
    def spindle_constant(event=None):
        if not manual_ok(): return
        ensure_mode(labvcnc.MODE_MANUAL)
        c.spindle(labvcnc.SPINDLE_CONSTANT)
    def set_first_line(lineno):
        if not manual_ok(): return
    ...
    def spindle_forward_toggle(*args):
        ...
        if s.spindle[0]['direction'] == 0:
            c.spindle(labvcnc.SPINDLE_FORWARD,default_spindle_speed)
        else:
            c.spindle(labvcnc.SPINDLE_OFF)

    def spindle_backward_toggle(*args):
        if not manual_ok(): return "break"
        s.poll()
        if s.spindle[0]['direction'] == 0:
            c.spindle(labvcnc.SPINDLE_REVERSE,default_spindle_speed)
        else:
            c.spindle(labvcnc.SPINDLE_OFF)
    ...
    def toggle_teleop_mode(*args):
        s.poll()
        set_motion_teleop(not (s.motion_mode == labvcnc.TRAJ_MODE_TELEOP))
    ...
    def toggle_override_limits(*args):
        s.poll()
        if s.interp_state != labvcnc.INTERP_IDLE: return
...
vars.labvcnctop_command.set(os.path.join(os.path.dirname(sys.argv[0]), "labvcnctop"))
...
def activate_ja_widget_or_set_feedrate(jora):
    ...
    if joints_mode() and s.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
        if jora == 10: jora = 0
...
def get_jog_mode():
    ...
    if  (    (s.kinematics_type == labvcnc.KINEMATICS_IDENTITY)
        ...
        if s.motion_mode == labvcnc.TRAJ_MODE_FREE:
            teleop_mode = 0
            jjogmode = True
        else:
            teleop_mode = 1
            jjogmode = False
    if (   (    jjogmode and s.motion_mode != labvcnc.TRAJ_MODE_FREE)
        or (not jjogmode and s.motion_mode != labvcnc.TRAJ_MODE_TELEOP) ):
        set_motion_teleop(teleop_mode)
    return jjogmode

# Note: require labvcnc.MAX_JOINTS >= labvcnc.MAX_AXIS
jog_after = [None]  * labvcnc.MAX_JOINTS
jog_cont  = [False] * labvcnc.MAX_JOINTS
jogging   = [0]     * labvcnc.MAX_JOINTS
def jog_on(a, b):
    if not manual_ok(): return
    if not manual_tab_visible(): return
    if a < 3 or a > 5:
    ...
    if jogincr != _("Continuous"):
        ...
        if s.state != 1: return
        distance = parse_increment(jogincr)
        jog(labvcnc.JOG_INCREMENT, jjogmode, a, b, distance)
        jog_cont[a] = False
    else:
        jog(labvcnc.JOG_CONTINUOUS, jjogmode, a, b)
        jog_cont[a] = True
        jogging[a] = b
...
def jog_off_actual(a):
    ...
    jjogmode = get_jog_mode()
    if jog_cont[a]:
        jog(labvcnc.JOG_STOP, jjogmode, a)
...
random_toolchanger = int(inifile.find("LBVIO", "RANDOM_TOOLCHANGER") or 0)
vars.lbvini.set(sys.argv[2])
jointcount = int(inifile.find("KINS", "JOINTS"))
open_directory = inifile.find("DISPLAY", "PROGRAM_PREFIX") or open_directory
vars.machine.set(inifile.find("LBV", "MACHINE"))
...
axis_type = [None] * labvcnc.MAX_AXIS
for a in range(labvcnc.MAX_AXIS):
...
nmlfile = inifile.find("LBV", "NML_FILE")
if nmlfile:
    labvcnc.nmlfile = os.path.join(os.path.dirname(sys.argv[2]), nmlfile)
...
tooltable = inifile.find("LBVIO", "TOOL_TABLE")
...
statfail=0
statwait=.01
while ((s.joints == 0) or (s.kinematics_type < labvcnc.KINEMATICS_IDENTITY)):
    ...
    if statfail > 8:
        raise SystemExit, (
            "A configuration error is preventing LabvCNC from starting.\n"
...
for i in range(labvcnc.MAX_AXIS):
...
for i in range(num_joints, labvcnc.MAX_JOINTS):
    ...
    for c,cmd,w in _dynamic_childs.values():
        ...
        if postgui_halfile:
            if postgui_halfile.lower().endswith('.tcl'):
                res = os.spawnvp(os.P_WAIT, "haltcl", ["haltcl", "-i", vars.lbvini.get(), postgui_halfile])
            else:
                res = os.spawnvp(os.P_WAIT, "halcmd", ["halcmd", "-i", vars.lbvini.get(), "-f", postgui_halfile])
