...
static PyMethodDef Ini_methods[] = {
    {"find", (PyCFunction)Ini_find, METH_VARARGS,
        ...
        "(section,option) order, not the labvcnc order."},
...
#define LBV_COMMAND_TIMEOUT 5.0  // how long to wait until timeout
#define LBV_COMMAND_DELAY   0.01 // how long to sleep between checks

static int lbvWaitCommandComplete(pyCommandChannel *s, double timeout) {
    double start = etime();

    do {
        double now = etime();
        if(s->s->peek() == LBV_STAT_TYPE) {
           LBV_STAT *stat = (LBV_STAT*)s->s->get_address();
        ...
        esleep(fmin(timeout - (now - start), LBV_COMMAND_DELAY));
    } while (etime() - start < timeout);
    return -1;
}

static int lbvSendCommand(pyCommandChannel *s, RCS_CMD_MSG & cmd) {
    if (s->c->write(&cmd)) {
        return -1;
    }
    ...
    double start = etime();
    while (etime() - start < LBV_COMMAND_TIMEOUT) {
        LBV_STAT *stat = (LBV_STAT*)s->s->get_address();
        int serial_diff = stat->echo_serial_number - s->serial;
        if(s->s->peek() == LBV_STAT_TYPE &&
           serial_diff >= 0) {
...
static int Stat_init(pyStatChannel *self, PyObject *a, PyObject *k) {
    ...
    RCS_STAT_CHANNEL *c =
        new RCS_STAT_CHANNEL(lbvFormat, "lbvStatus", "xlbv", file);
    if(!c) {
...
static bool check_stat(RCS_STAT_CHANNEL *lbvStatusBuffer) {
    if(!lbvStatusBuffer->valid()) {
        PyErr_Format( error, "lbvStatusBuffer invalid err=%d", lbvStatusBuffer->error_type);
        return false;
    }
    return true;
}
...
static PyObject *poll(pyStatChannel *s, PyObject *o) {
    ...
    if(s->c->peek() == LBV_STAT_TYPE) {
        LBV_STAT *lbvStatus = static_cast<LBV_STAT*>(s->c->get_address());
        memcpy(&s->status, lbvStatus, sizeof(LBV_STAT));
    }
    Py_INCREF(Py_None);
    return Py_None;
}
...
// LBV_SPINDLE_STAT motion.spindle
    // MOVED TO THE "spindle" TUPLE OF DICTS

// io
// LBV_TOOL_STAT io.tool
...
static PyObject *pose(const LbvPose &p) {
...
static PyObject *Stat_joint_position(pyStatChannel *s) {
    PyObject *res = PyTuple_New(LBVMOT_MAX_JOINTS);
...
static PyObject *Stat_activlbvodes(pyStatChannel *s) {
...
static PyObject *Stat_din(pyStatChannel *s) {
    return int_array(s->status.motion.synch_di, LBVMOT_MAX_AIO);
}

static PyObject *Stat_dout(pyStatChannel *s) {
    return int_array(s->status.motion.synch_do, LBVMOT_MAX_AIO);
}
...
static PyObject *Stat_ain(pyStatChannel *s) {
    return double_array(s->status.motion.analog_input, LBVMOT_MAX_AIO);
}

static PyObject *Stat_aout(pyStatChannel *s) {
    return double_array(s->status.motion.analog_output, LBVMOT_MAX_AIO);
}
...
static PyObject *Stat_axis(pyStatChannel *s) {
    PyObject *res = PyTuple_New(LBVMOT_MAX_AXIS);
...
static PyObject *Stat_spindle(pyStatChannel *s) {
    PyObject *res = PyTuple_New(LBVMOT_MAX_SPINDLES);
    for(int i=0; i<LBVMOT_MAX_SPINDLES; i++) {
...
// XXX LBV_JOINT_STAT motion.joint[]
...
static PyGetSetDef Stat_getsetlist[] = {
    ...
    {(char*)"mcodes", (getter)Stat_activlbvodes},
...
static int Command_init(pyCommandChannel *self, PyObject *a, PyObject *k) {
    ...
    RCS_CMD_CHANNEL *c =
        new RCS_CMD_CHANNEL(lbvFormat, "lbvCommand", "xlbv", file);
    if(!c) {
...
static PyObject *block_delete(pyCommandChannel *s, PyObject *o) {
    int t;
    LBV_TASK_PLAN_SET_BLOCK_DELETE m;
...
static PyObject *optional_stop(pyCommandChannel *s, PyObject *o) {
    int t;
    LBV_TASK_PLAN_SET_OPTIONAL_STOP m;
...
static PyObject *task_plan_synch(pyCommandChannel *s) {
    LBV_TASK_PLAN_SYNCH synch;
    lbvSendCommand(s, synch);
    Py_INCREF(Py_None);
    return Py_None;
}

static PyObject *maxvel(pyCommandChannel *s, PyObject *o) {
    LBV_TRAJ_SET_MAX_VELOCITY m;
...
static PyObject *mist(pyCommandChannel *s, PyObject *o) {
    ...
    switch(dir) {
        case LOCAL_MIST_ON:
        {
            LBV_COOLANT_MIST_ON m;
            lbvSendCommand(s, m);
        }
            break;
        case LOCAL_MIST_OFF:
        {
            LBV_COOLANT_MIST_OFF m;
            lbvSendCommand(s, m);
        }
            break;
        default:
...
static PyObject *flood(pyCommandChannel *s, PyObject *o) {
    ...
    switch(dir) {
        case LOCAL_FLOOD_ON:
        {
            LBV_COOLANT_FLOOD_ON m;
            lbvSendCommand(s, m);
        }
            break;
        case LOCAL_FLOOD_OFF:
        {
            LBV_COOLANT_FLOOD_OFF m;
            lbvSendCommand(s, m);
        }
            break;
        default:
...
static PyObject *lbvabort(pyCommandChannel *s, PyObject *o) {
    LBV_TASK_ABORT m;
    lbvSendCommand(s, m);
    Py_INCREF(Py_None);
    return Py_None;
...
static PyObject *program_open(pyCommandChannel *s, PyObject *o) {
    LBV_TASK_PLAN_CLOSE m0;
    lbvSendCommand(s, m0);

    LBV_TASK_PLAN_OPEN m;
    char *file;
    int len;
...
static PyObject *lbvauto(pyCommandChannel *s, PyObject *o) {
    int fn;
    LBV_TASK_PLAN_RUN run;
    LBV_TASK_PLAN_PAUSE pause;
    LBV_TASK_PLAN_REVERSE reverse;
    LBV_TASK_PLAN_FORWARD forward;
    LBV_TASK_PLAN_RESUME resume;
    LBV_TASK_PLAN_STEP step;

    if(PyArg_ParseTuple(o, "ii", &fn, &run.line) && fn == LOCAL_AUTO_RUN) {
        lbvSendCommand(s, run);
    } else {
        PyErr_Clear();
        if(!PyArg_ParseTuple(o, "i", &fn)) return NULL;
        switch(fn) {
        case LOCAL_AUTO_PAUSE:
            lbvSendCommand(s, pause);
            break;
        case LOCAL_AUTO_RESUME:
            lbvSendCommand(s, resume);
            break;
        case LOCAL_AUTO_STEP:
            lbvSendCommand(s, step);
            break;
        case LOCAL_AUTO_REVERSE:
            lbvSendCommand(s, reverse);
            break;
        case LOCAL_AUTO_FORWARD:
            lbvSendCommand(s, forward);
            break;
        default:
...
static PyObject *set_min_limit(pyCommandChannel *s, PyObject *o) {
    LBV_JOINT_SET_MIN_POSITION_LIMIT m;
...
static PyObject *set_max_limit(pyCommandChannel *s, PyObject *o) {
    LBV_JOINT_SET_MAX_POSITION_LIMIT m;
    if(!PyArg_ParseTuple(o, "id", &m.joint, &m.limit))
        return NULL;
...
static PyObject *wait_complete(pyCommandChannel *s, PyObject *o) {
    double timeout = LBV_COMMAND_TIMEOUT;
    if (!PyArg_ParseTuple(o, "|d:lbv.command.wait_complete", &timeout))
        return NULL;
    return PyInt_FromLong(lbvWaitCommandComplete(s, timeout));
}
...
static PyMethodDef Command_methods[] = {
    ...
    {"abort", (PyCFunction)lbvabort, METH_NOARGS},
    ...
    {"auto", (PyCFunction)lbvauto, METH_VARARGS},
...
static int Error_init(pyErrorChannel *self, PyObject *a, PyObject *k) {
    ...
    NML *c = new NML(lbvFormat, "lbvError", "xlbv", file);
    if(!c) {
...
#define TYPECASE(x, f) _TYPECASE(PASTE(x, _TYPE), x, f)
    switch(type) {
        TYPECASE(LBV_OPERATOR_ERROR, error)
        TYPECASE(LBV_OPERATOR_TEXT, text)
        TYPECASE(LBV_OPERATOR_DISPLAY, display)
...
static PyObject *Logger_start(pyPositionLogger *s, PyObject *o) {
    ...
    while(!s->exit) {
        ...
        if(s->st->c->valid() && s->st->c->peek() == LBV_STAT_TYPE) {
            LBV_STAT *status = static_cast<LBV_STAT*>(s->st->c->get_address());
            int colornum = 2;
...
static PyObject *Logger_last(pyPositionLogger *s, PyObject *o) {
    int flag=1;
    if(!PyArg_ParseTuple(o, "|i:lbv.positionlogger.last", &flag)) return NULL;
...
static PyTypeObject PositionLoggerType = {
    PyObject_HEAD_INIT(NULL)
    0,                      /*ob_size*/
    "labvcnc.positionlogger",   /*tp_name*/
...
initlabvcnc(void) {
    ...
    m = Py_InitModule3("labvcnc", lbv_methods, "Interface to LabvCNC");

    PyType_Ready(&Stat_Type);
    PyType_Ready(&Command_Type);
    PyType_Ready(&Error_Type);
    PyType_Ready(&Ini_Type);
    error = PyErr_NewException((char*)"labvcnc.error", PyExc_RuntimeError, NULL);
    ...
    PyModule_AddStringConstant(m, "PREFIX", LBV2_HOME);
    PyModule_AddStringConstant(m, "SHARE", LBV2_HOME "/share");
    PyModule_AddStringConstant(m, "nmlfile", LBV2_DEFAULT_NMLFILE);

    PyModule_AddIntConstant(m, "OPERATOR_ERROR", LBV_OPERATOR_ERROR_TYPE);
    PyModule_AddIntConstant(m, "OPERATOR_TEXT", LBV_OPERATOR_TEXT_TYPE);
    PyModule_AddIntConstant(m, "OPERATOR_DISPLAY", LBV_OPERATOR_DISPLAY_TYPE);
    ...
    ENUMX(9, LBV_TASK_INTERP_IDLE);
    ENUMX(9, LBV_TASK_INTERP_READING);
    ENUMX(9, LBV_TASK_INTERP_PAUSED);
    ENUMX(9, LBV_TASK_INTERP_WAITING);

    ENUMX(9, LBV_TASK_MODE_MDI);
    ENUMX(9, LBV_TASK_MODE_MANUAL);
    ENUMX(9, LBV_TASK_MODE_AUTO);

    ENUMX(9, LBV_TASK_STATE_OFF);
    ENUMX(9, LBV_TASK_STATE_ON);
    ENUMX(9, LBV_TASK_STATE_ESTOP);
    ENUMX(9, LBV_TASK_STATE_ESTOP_RESET);
    ...
    ENUMX(4, LBV_TRAJ_MODE_FREE);
    ENUMX(4, LBV_TRAJ_MODE_COORD);
    ENUMX(4, LBV_TRAJ_MODE_TELEOP);

    ENUMX(4, LBV_MOTION_TYPE_TRAVERSE);
    ENUMX(4, LBV_MOTION_TYPE_FEED);
    ENUMX(4, LBV_MOTION_TYPE_ARC);
    ENUMX(4, LBV_MOTION_TYPE_TOOLCHANGE);
    ENUMX(4, LBV_MOTION_TYPE_PROBING);
    ENUMX(4, LBV_MOTION_TYPE_INDEXROTARY);
    ...
    ENUMX(4, LBV_DEBUG_CONFIG);
    ENUMX(4, LBV_DEBUG_VERSIONS);
    ENUMX(4, LBV_DEBUG_TASK_ISSUE);
    ENUMX(4, LBV_DEBUG_NML);
    ENUMX(4, LBV_DEBUG_MOTION_TIME);
    ENUMX(4, LBV_DEBUG_INTERP);
    ENUMX(4, LBV_DEBUG_RCS);
    ENUMX(4, LBV_DEBUG_INTERP_LIST);

    ENUMX(9, LBV_TASK_EXEC_ERROR);
    ENUMX(9, LBV_TASK_EXEC_DONE);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_MOTION);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_MOTION_QUEUE);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_IO);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_MOTION_AND_IO);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_DELAY);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_SYSTEM_CMD);
    ENUMX(9, LBV_TASK_EXEC_WAITING_FOR_SPINDLE_ORIENTED);

    ENUMX(7, LBVMOT_MAX_JOINTS);
    ENUMX(7, LBVMOT_MAX_AXIS);
