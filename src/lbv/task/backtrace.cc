...
extern int done;
extern int lbvOperatorError(int id, const char *fmt, ...);
extern int lbvOperatorText(int id, const char *fmt, ...);
...
void backtrace(int signo)
{
	...
	lbvOperatorError(0, "backtrace for %s (pid %s signal %d): gdb failed to start", name_buf, pid_buf, signo);
	...
	if (signo == SIGUSR1) {  // continue running after backtrace
	    ...
	    lbvOperatorText(0, "backtrace for %s stored in %s, continuing", name_buf, filename);
	    fprintf(stderr, "%s: backtrace stored in %s, continuing\n", name_buf, filename);
	} else {
	    // this takes lbvmodule.cc:LBV_COMMAND_TIMEOUT seconds to display:
	    if (status == 0) // backtrace succeeded
		lbvOperatorError(0, "%s (pid %d) died on signal %d, backtrace stored in %s",
