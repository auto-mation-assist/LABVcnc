...
/*
  ...
  units". "External units" are the units used by the LBV motion planner.
...
#include "lbvglb.h"		// TRAJ_MAX_VELOCITY

//#define LBVCANON_DEBUG

//Simple compile-time debug macro
#ifdef LBVCANON_DEBUG
...
static CANON_POSITION unoffset_and_unrotate_pos(const LbvPose pos) {
...
static LbvPose to_ext_pose(double x, double y, double z, double a, double b, double c, double u, double v, double w) {
    LbvPose result;
...
static LbvPose to_ext_pose(const CANON_POSITION & pos) {
    LbvPose result;
...
static int axis_valid(int n) {
    return lbvStatus->motion.traj.axis_mask & (1<<n);
}
...
/* External call to update the canon end point.
   Called by lbvtask during skipping of lines (run-from-line) */
...
static void send_g5x_msg(int index) {
    ...
    /* append it to interp list so it gets updated at the right time, not at
       read-ahead time */
    LBV_TRAJ_SET_G5X set_g5x_msg;

    set_g5x_msg.g5x_index = index;

    set_g5x_msg.origin = to_ext_pose(canon.g5xOffset);

    for (int s = 0; s < lbvStatus->motion.traj.spindles; s++){
...
void USE_LENGTH_UNITS(CANON_UNITS in_unit)
{
    ...
    lbvStatus->task.programUnits = in_unit;
}
                               ...
                               double u, double v, double w)
{
    ...
    // Pure linear move:
    if (canon.cartesian_move && !canon.angular_move) {
	tx = dx? (dx / FROM_EXT_LEN(lbvAxisGetMaxAcceleration(0))): 0.0;
	ty = dy? (dy / FROM_EXT_LEN(lbvAxisGetMaxAcceleration(1))): 0.0;
	tz = dz? (dz / FROM_EXT_LEN(lbvAxisGetMaxAcceleration(2))): 0.0;
	tu = du? (du / FROM_EXT_LEN(lbvAxisGetMaxAcceleration(6))): 0.0;
	tv = dv? (dv / FROM_EXT_LEN(lbvAxisGetMaxAcceleration(7))): 0.0;
	tw = dw? (dw / FROM_EXT_LEN(lbvAxisGetMaxAcceleration(8))): 0.0;
	...
	ta = da? (da / FROM_EXT_ANG(lbvAxisGetMaxAcceleration(3))): 0.0;
	tb = db? (db / FROM_EXT_ANG(lbvAxisGetMaxAcceleration(4))): 0.0;
	tc = dc? (dc / FROM_EXT_ANG(lbvAxisGetMaxAcceleration(5))): 0.0;
...
*/
    ...
    // Pure linear move:
    if (canon.cartesian_move && !canon.angular_move) {
	tx = dx? fabs(dx / FROM_EXT_LEN(lbvAxisGetMaxVelocity(0))): 0.0;
	ty = dy? fabs(dy / FROM_EXT_LEN(lbvAxisGetMaxVelocity(1))): 0.0;
	tz = dz? fabs(dz / FROM_EXT_LEN(lbvAxisGetMaxVelocity(2))): 0.0;
	tu = du? fabs(du / FROM_EXT_LEN(lbvAxisGetMaxVelocity(6))): 0.0;
	tv = dv? fabs(dv / FROM_EXT_LEN(lbvAxisGetMaxVelocity(7))): 0.0;
	tw = dw? fabs(dw / FROM_EXT_LEN(lbvAxisGetMaxVelocity(8))): 0.0;
        out.tmax = MAX3(tx, ty ,tz);
        out.tmax = MAX4(tu, tv, tw, out.tmax);
	...
	ta = da? fabs(da / FROM_EXT_ANG(lbvAxisGetMaxVelocity(3))): 0.0;
	tb = db? fabs(db / FROM_EXT_ANG(lbvAxisGetMaxVelocity(4))): 0.0;
	tc = dc? fabs(dc / FROM_EXT_ANG(lbvAxisGetMaxVelocity(5))): 0.0;
        out.tmax = MAX3(ta, tb, tc);
...
static void flush_segments(void) {
    ...
    LBV_TRAJ_LINEAR_MOVE linearMoveMsg;
    ...
    linearMoveMsg.type = LBV_MOTION_TYPE_FEED;
                       ...
                       double u, double v, double w)
{
    ...
    if (canon.rotary_unlock_for_traverse != -1)
        linearMoveMsg.type = LBV_MOTION_TYPE_INDEXROTARY;
    else
        linearMoveMsg.type = LBV_MOTION_TYPE_TRAVERSE;
...
void RIGID_TAP(int line_number, double x, double y, double z, double scale)
{
    ...
    LBV_TRAJ_RIGID_TAP rigidTapMsg;
    double unused=0;
                    ...
                    unsigned char probe_type)
{
    ...
    probeMsg.type = LBV_MOTION_TYPE_PROBING;
...
{
    LBV_TRAJ_SET_TERM_COND setTermCondMsg;
    ...
    case CANON_CONTINUOUS:
        setTermCondMsg.cond = LBV_TRAJ_TERM_COND_BLEND;
        setTermCondMsg.tolerance = TO_EXT_LEN(canon.motionTolerance);
        break;
    case CANON_EXACT_PATH:
        setTermCondMsg.cond = LBV_TRAJ_TERM_COND_EXACT;
        break;

    case CANON_EXACT_STOP:
    default:
        setTermCondMsg.cond = LBV_TRAJ_TERM_COND_STOP;
        break;
    }
...
{
    flush_segments();
    LBV_TRAJ_SET_SPINDLESYNC spindlesyncMsg;
...
static CANON_POSITION get_axis_max_velocity()
{
    ...
    maxvel.x = axis_valid(0) ? FROM_EXT_LEN(lbvAxisGetMaxVelocity(0)) : 0.0;
    maxvel.y = axis_valid(1) ? FROM_EXT_LEN(lbvAxisGetMaxVelocity(1)) : 0.0;
    maxvel.z = axis_valid(2) ? FROM_EXT_LEN(lbvAxisGetMaxVelocity(2)) : 0.0;

    maxvel.a = axis_valid(3) ? FROM_EXT_ANG(lbvAxisGetMaxVelocity(3)) : 0.0;
    maxvel.b = axis_valid(4) ? FROM_EXT_ANG(lbvAxisGetMaxVelocity(4)) : 0.0;
    maxvel.c = axis_valid(5) ? FROM_EXT_ANG(lbvAxisGetMaxVelocity(5)) : 0.0;

    maxvel.u = axis_valid(6) ? FROM_EXT_LEN(lbvAxisGetMaxVelocity(6)) : 0.0;
    maxvel.v = axis_valid(7) ? FROM_EXT_LEN(lbvAxisGetMaxVelocity(7)) : 0.0;
    maxvel.w = axis_valid(8) ? FROM_EXT_LEN(lbvAxisGetMaxVelocity(8)) : 0.0;
    return maxvel;
}
...
static CANON_POSITION get_axis_max_acceleration()
{
    ...
    maxacc.x = axis_valid(0) ? FROM_EXT_LEN(lbvAxisGetMaxAcceleration(0)) : 0.0;
    maxacc.y = axis_valid(1) ? FROM_EXT_LEN(lbvAxisGetMaxAcceleration(1)) : 0.0;
    maxacc.z = axis_valid(2) ? FROM_EXT_LEN(lbvAxisGetMaxAcceleration(2)) : 0.0;

    maxacc.a = axis_valid(3) ? FROM_EXT_ANG(lbvAxisGetMaxAcceleration(3)) : 0.0;
    maxacc.b = axis_valid(4) ? FROM_EXT_ANG(lbvAxisGetMaxAcceleration(4)) : 0.0;
    maxacc.c = axis_valid(5) ? FROM_EXT_ANG(lbvAxisGetMaxAcceleration(5)) : 0.0;

    maxacc.u = axis_valid(6) ? FROM_EXT_LEN(lbvAxisGetMaxAcceleration(6)) : 0.0;
    maxacc.v = axis_valid(7) ? FROM_EXT_LEN(lbvAxisGetMaxAcceleration(7)) : 0.0;
    maxacc.w = axis_valid(8) ? FROM_EXT_LEN(lbvAxisGetMaxAcceleration(8)) : 0.0;
    return maxacc;
}
              ...
              double u, double v, double w)
{
    ...
    LBV_TRAJ_CIRCULAR_MOVE circularMoveMsg;
    LBV_TRAJ_LINEAR_MOVE linearMoveMsg;
    ...
    double v1 = FROM_EXT_LEN(lbvAxisGetMaxVelocity(axis1));
    double v2 = FROM_EXT_LEN(lbvAxisGetMaxVelocity(axis2));

    // Get planar acceleration bounds
    double a1 = FROM_EXT_LEN(lbvAxisGetMaxAcceleration(axis1));
    double a2 = FROM_EXT_LEN(lbvAxisGetMaxAcceleration(axis2));
    ...
    if(canon.xy_rotation && canon.activePlane != CANON_PLANE_XY) {
        ...
        if (axis_valid(axis3)) {
            double v3 = FROM_EXT_LEN(lbvAxisGetMaxVelocity(axis3));
            double a3 = FROM_EXT_LEN(lbvAxisGetMaxAcceleration(axis3));
...
//COMPUTE ACCEL
    ...
    if (rotation == 0) {
        ...
        linearMoveMsg.type = LBV_MOTION_TYPE_ARC;
        linearMoveMsg.vel = toExtVel(vel);
        ...
        circularMoveMsg.type = LBV_MOTION_TYPE_ARC;
...
void START_SPINDLE_CLOCKWISE(int s, int wait_for_atspeed)
{
    ...
    if(canon.spindle[s].css_maximum) {
		...
		lbv_spindle_on_msg.speed = canon.spindle[s].dir * canon.spindle[s].css_maximum;
		lbv_spindle_on_msg.factor = canon.spindle[s].dir * canon.spindle[s].css_factor;
		lbv_spindle_on_msg.xoffset = TO_EXT_LEN(canon.g5xOffset.x + canon.g92Offset.x + canon.toolOffset.tran.x);
    } else {
        lbv_spindle_on_msg.speed = canon.spindle[s].dir * canon.spindle[s].speed;
     //   canon.css_numerator = 0; FIXME: Do we need this?
    }
    lbv_spindle_on_msg.wait_for_spindle_at_speed = wait_for_atspeed;
    interp_list.append(lbv_spindle_on_msg);
}
...
void SET_SPINDLE_SPEED(int s, double r)
{
    ...
    LBV_SPINDLE_SPEED lbv_spindle_speed_msg;

    flush_segments();

    lbv_spindle_speed_msg.spindle = s;
    if(canon.spindle[s].css_maximum) {
		if(canon.lengthUnits == CANON_UNITS_INCHES){
			canon.spindle[s].css_factor = 12 / (2 * M_PI) * canon.spindle[s].speed * TO_EXT_LEN(25.4);
		} else {
		...
		lbv_spindle_speed_msg.speed =  canon.spindle[s].dir * canon.spindle[s].css_maximum;
		lbv_spindle_speed_msg.factor =  canon.spindle[s].dir * canon.spindle[s].css_factor;
		lbv_spindle_speed_msg.xoffset = TO_EXT_LEN(canon.g5xOffset.x + canon.g92Offset.x + canon.toolOffset.tran.x);
	} else {
        lbv_spindle_speed_msg.speed = canon.spindle[s].dir * canon.spindle[s].speed;
		//   canon.css_numerator = 0; FIXME: Do we need this?
    }
    interp_list.append(lbv_spindle_speed_msg);
}
...
void STOP_SPINDLE_TURNING(int s)
{
    ...
    flush_segments();
    lbv_spindle_off_msg.spindle = s;
    interp_list.append(lbv_spindle_off_msg);
...
{
    LBV_SPINDLE_WAIT_ORIENT_COMPLETE o;
...
void SET_TOOL_TABLE_ENTRY(int pocket, int toolno, LbvPose offset, double diameter,
                          double frontangle, double backangle, int orientation) {
    LBV_TOOL_SET_OFFSET o;
    flush_segments();
    o.pocket = pocket;
    o.toolno = toolno;
    o.offset = offset;
...
/*
  LBV has no tool length offset. To implement it, we save it here,
  and apply it when necessary
  */
void USE_TOOL_LENGTH_OFFSET(LbvPose offset)
{
    LBV_TRAJ_SET_OFFSET set_offset_msg;
...
void START_CHANGE()
{
    LBV_TOOL_START_CHANGE lbv_start_change_msg;

    flush_segments();

    interp_list.append(lbv_start_change_msg);
}
...
void CHANGE_TOOL(int slot)
{
    ...
    if (have_tool_change_position) {
        ...
        linearMoveMsg.vel = linearMoveMsg.ini_maxvel = toExtVel(vel);
        linearMoveMsg.acc = toExtAcc(acc);
        linearMoveMsg.type = LBV_MOTION_TYPE_TOOLCHANGE;
	linearMoveMsg.feed_mode = 0;
        linearMoveMsg.indexer_jnum = -1;
...
{
    LBV_TOOL_PREPARE prep_for_tool_msg;
...
{
    LBV_TOOL_SET_NUMBER lbv_tool_set_number_msg;
    
    lbv_tool_set_number_msg.tool = pocket_number;

    interp_list.append(lbv_tool_set_number_msg);
}
...
{
    LBV_TRAJ_SET_FO_ENABLE set_fo_enable_msg;
    flush_segments();
...
{
    LBV_MOTION_ADAPTIVE lbvmotAdaptiveMsg;
    flush_segments();

    lbvmotAdaptiveMsg.status = 0;
    interp_list.append(lbvmotAdaptiveMsg);
}
...
{
    LBV_TRAJ_SET_SO_ENABLE set_so_enable_msg;
    flush_segments();
...
{
    LBV_TRAJ_SET_FH_ENABLE set_feed_hold_msg;
    flush_segments();
...
void FLOOD_OFF()
{
    LBV_COOLANT_FLOOD_OFF flood_off_msg;
...
void FLOOD_ON()
{
    LBV_COOLANT_FLOOD_ON flood_on_msg;
...
void MESSAGE(char *s)
{
    LBV_OPERATOR_DISPLAY operator_display_msg;
...
void MIST_OFF()
{
    LBV_COOLANT_MIST_OFF mist_off_msg;
...
void MIST_ON()
{
    LBV_COOLANT_MIST_ON mist_on_msg;
...
void TURN_PROBE_ON()
{
    LBV_TRAJ_CLEAR_PROBE_TRIPPED_FLAG clearMsg;
...
{
    LBV_TASK_PLAN_OPTIONAL_STOP stopMsg;
...
void INIT_CANON()
{
    ...
    for (int s = 0; s < LBVMOT_MAX_SPINDLES; s++) {
        canon.spindle[s].speed = 0.0;
        canon.spindle[s].synched = 0;
    }
    canon.optional_program_stop = ON; //set enabled by default (previous LBV behaviour)
    canon.block_delete = ON; //set enabled by default (previous LBV behaviour)
    canon.cartesian_move = 0;
    canon.angular_move = 0;
    canon.linearFeedRate = 0.0;
    canon.angularFeedRate = 0.0;
    ZERO_LBV_POSE(canon.toolOffset);
...
{
    va_list ap;
    LBV_OPERATOR_ERROR operator_error_msg;
...
CANON_TOOL_TABLE GET_EXTERNAL_TOOL_TABLE(int pocket)
{
	...
	retval.toolno = -1;
        ZERO_LBV_POSE(retval.offset);
        retval.frontangle = 0.0;
        retval.backangle = 0.0;
	retval.diameter = 0.0;
        retval.orientation = 0;
    } else {
	retval = lbvStatus->io.tool.toolTable[pocket];
    }
...
CANON_POSITION GET_EXTERNAL_POSITION()
{
    ...
    pos = lbvStatus->motion.traj.position;

    if (GET_EXTERNAL_OFFSET_APPLIED() ) {
        LbvPose eoffset = GET_EXTERNAL_OFFSETS();
...
CANON_POSITION GET_EXTERNAL_PROBE_POSITION()
{
    ...
    pos = lbvStatus->motion.traj.probedPosition;
...
{
    return lbvStatus->motion.traj.probe_tripped;
}
...
double GET_EXTERNAL_TRAVERSE_RATE()
{
    ...
    traverse =
	TO_PROG_LEN(FROM_EXT_LEN(lbvStatus->motion.traj.maxVelocity));
...
double GET_EXTERNAL_LENGTH_UNITS(void)
{
    ...
    u = lbvStatus->motion.traj.linearUnits;
...
double GET_EXTERNAL_ANGLE_UNITS(void)
{
    ...
    u = lbvStatus->motion.traj.angularUnits;
...
{
    return lbvStatus->io.coolant.mist;
}

int GET_EXTERNAL_FLOOD()
{
    return lbvStatus->io.coolant.flood;
}
...
{
    if (lbvStatus->motion.spindle[spindle].speed == 0) {
	return CANON_STOPPED;
    }

    if (lbvStatus->motion.spindle[spindle].speed >= 0.0) {
...
int GET_EXTERNAL_QUEUE_EMPTY(void)
{
    ...
    return lbvStatus->motion.traj.queue == 0 ? 1 : 0;
}
...
{
    int toolno = lbvStatus->io.tool.toolInSpindle;
    int pocket;

    for (pocket = 1; pocket < CANON_POCKETS_MAX; pocket++) {
        if (lbvStatus->io.tool.toolTable[pocket].toolno == toolno) {
            return pocket;
        }
    }
...
{
    return lbvStatus->io.tool.pocketPrepped;
}

int GET_EXTERNAL_TC_FAULT()
{
    return lbvStatus->io.fault;
}

int GET_EXTERNAL_TC_REASON()
{
    return lbvStatus->io.reason;
}

int GET_EXTERNAL_FEED_OVERRIDE_ENABLE()
{
    return lbvStatus->motion.traj.feed_override_enabled;
}

int GET_EXTERNAL_SPINDLE_OVERRIDE_ENABLE(int spindle)
{
    return lbvStatus->motion.spindle[spindle].spindle_override_enabled;
}

int GET_EXTERNAL_ADAPTIVE_FEED_ENABLE()
{
    return lbvStatus->motion.traj.adaptive_feed_enabled;
}

int GET_EXTERNAL_FEED_HOLD_ENABLE()
{
    return lbvStatus->motion.traj.feed_hold_enabled;
}

int GET_EXTERNAL_AXIS_MASK() {
    return lbvStatus->motion.traj.axis_mask;
}

int GET_EXTERNAL_OFFSET_APPLIED(void) {
    return lbvGetExternalOffsetApplied();
}

LbvPose GET_EXTERNAL_OFFSETS() {
    return lbvGetExternalOffsets();
}
...
{
    if ((index < 0) || (index >= LBVMOT_MAX_DIO))
	return -1;

    if (lbvStatus->task.input_timeout == 1)
	return -1;

#ifdef INPUT_DEBUG
    printf("GET_EXTERNAL_DIGITAL_INPUT called\n di[%d]=%d \n timeout=%d \n",index,lbvStatus->motion.synch_di[index],lbvStatus->task.input_timeout);
#endif
    return (lbvStatus->motion.synch_di[index] != 0) ? 1 : 0;
}
...
#ifdef INPUT_DEBUG
    printf("GET_EXTERNAL_ANALOG_INPUT called\n ai[%d]=%g \n timeout=%d \n",index,lbvStatus->motion.analog_input[index],lbvStatus->task.input_timeout);
#endif
    if ((index < 0) || (index >= LBVMOT_MAX_AIO))
	return -1;

    if (lbvStatus->task.input_timeout == 1)
	return -1;

    return lbvStatus->motion.analog_input[index];
}
