...
#include "lbvmotcfg.h" // Just for LBVMOT_NUM_SPINDLES
...
/* motion path-following tolerance is used to set the max path-following
   ...
   If this param is 0, then it will behave as lbv always did, allowing
   ...
   CanonSpindle_t spindle[LBVMOT_MAX_SPINDLES];
...
extern void CANON_UPDATE_END_POINT(double x, double y, double z, 
				   double a, double b, double c,
				   double u, double v, double w);
/* Called from lbvtask to update the canon position during skipping through
...
/* Tool Functions */
extern void SET_TOOL_TABLE_ENTRY(int pocket, int toolno, LbvPose offset, double diameter,
                                 double frontangle, double backangle, int orientation);
extern void USE_TOOL_LENGTH_OFFSET(LbvPose offset);
...
startup one would want to tell lbv2 what tool it has loaded. As the last
toolnumber before shutdown isn't currently written, there is no provision
to allow lbv2 to safely restart knowing what tool is in the spindle.
Using CHANGE_TOOL_NUMBER one can tell lbv2 (without any physical action)
...
/* to be called by lbvTaskPlanExecute when done interpreting.  This causes the
...
extern LbvPose GET_EXTERNAL_OFFSETS();
