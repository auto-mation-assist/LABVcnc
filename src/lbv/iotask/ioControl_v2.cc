...
// extend  LBV_IO_ABORT_REASON_ENUM from lbv.hh
enum {
    LBV_ABORT_BY_TOOLCHANGER_FAULT = LBV_ABORT_USER + 1
};
...
enum {
    ...
    TI_TC_FAULT = 16,
    TI_TC_ABORT = 32,
    TI_LBV_ABORT_SIGNALED = 64,
    TI_LBV_ABORT_ACKED = 128,
...
struct iocontrol_str {
    ...
    hal_bit_t *lbv_abort; 	/* output, signals lbv-originated abort to toolchanger */
    hal_bit_t *lbv_abort_ack; 	/* input, handshake line to acknowledge abort_tool_change */
    hal_s32_t *lbv_reason;     	/* output, convey cause for LBV-originated abort to toolchanger.
				 * UI informational. Valid during lbv-abort True.
				 */
    ...
    hal_s32_t *toolchanger_reason;         /* input, convey reason code for toolchanger-originated
					    ...
					    * on a toolchange abort, the reason is passed to LBV in the
					    * lbvioStat message.
...
static int iniLoad(const char *filename)
{
    ...
    if (lbv_debug & LBV_DEBUG_IOCONTROL) {
    ...
    inifile.Find(&proto, "PROTOCOL_VERSION", "LBVIO");
    rtapi_print_msg(RTAPI_MSG_DBG,"%s: [LBVIO] using v%d protocol\n",progname,proto);
...
int iocontrol_hal_init(void)
{
    ...
    BITPIN( HAL_IN , "iocontrol.%d.lbv-enable-in", &(iocontrol_data->lbv_enable_in));
    ...
    // v2 pins
    if (proto > V1) {
	BITPIN( HAL_OUT, "iocontrol.%d.lbv-abort", &(iocontrol_data->lbv_abort));
	BITPIN( HAL_IN , "iocontrol.%d.lbv-abort-ack", &(iocontrol_data->lbv_abort_ack));
	S32PIN( HAL_OUT, "iocontrol.%d.lbv-reason", &(iocontrol_data->lbv_reason));
...
void hal_init_pins(void)
{
	...
	*(iocontrol_data->lbv_abort) = 0;
	*(iocontrol_data->lbv_reason) = 0;
...
static char *str_input(int status)
{
    ...
    if (status & TI_LBV_ABORT_SIGNALED) strcat(seen," TI_LBV_ABORT_SIGNALED");
    if (status & TI_LBV_ABORT_ACKED) strcat(seen," TI_LBV_ABORT_ACKED");
 ...
 *               lbvioStatus.tool.*
 *               lbvioStatus.aux.estop
 *               lbvioStatus.lube.level
 *
...
int read_inputs(void)
{
	...
	// an LBV-side abort is in progress.
	if (*iocontrol_data->lbv_abort) {
	    if (*iocontrol_data->lbv_abort_ack) {
		// the toolchanger acknowledged the fact so deassert
		*(iocontrol_data->lbv_abort) = 0;
		// a completed LBV-side abort always forces IDLE state
		*(iocontrol_data->state) = ST_IDLE;
		retval |= TI_LBV_ABORT_ACKED;
	    } else {
		// waiting for abort being acked by toolchanger.
		retval |= TI_LBV_ABORT_SIGNALED;
	    }
	}
	...
	if ((proto > V1) && (*iocontrol_data->toolchanger_faulted))  {
	     ...
	     * the first place
	     */
	    *(iocontrol_data->lbv_reason) = LBV_ABORT_BY_TOOLCHANGER_FAULT;
	    *(iocontrol_data->lbv_abort) = 1;
	    retval |= TI_TC_ABORT;
	    // give up on request
	...
	if (*iocontrol_data->tool_changed) {
	    ...
	    *(iocontrol_data->tool_number) = lbvioStatus.tool.toolInSpindle; // likewise in HAL
	    ...
	    lbvioStatus.tool.pocketPrepped = -1; // reset the tool prepped number, -1 to permit tool 0 to be loaded
...
void update_status(int status, int serial)
{
    ...
    lbvioStatus.status = status;
    if (status_reported != status) {
	rtapi_print_msg(RTAPI_MSG_DBG, "%s: updating status=%s state=%s fault=%d reason=%d\n",
			progname,strcs[lbvioStatus.status],strstate[*(iocontrol_data->state)],
			lbvioStatus.fault, lbvioStatus.reason
			);
	status_reported = lbvioStatus.status;  // just print this once
    }
    lbvioStatus.command_type = LBV_IO_STAT_TYPE;
    lbvioStatus.echo_serial_number = serial;
...
int main(int argc, char *argv[])
{
	 ...
	 * an NML message has to be pushed to LBV.
	...
	if (input_status & (TI_START_CHANGE|TI_CHANGING)) {
		...
		update_status(RCS_ERROR, lbvioCommand->serial_number);
	    } else {
		update_status(RCS_EXEC, lbvioCommand->serial_number);
	    }
	}

	//	if (input_status & (TI_PREPARE_COMPLETE|TI_CHANGE_COMPLETE|TI_START_CHANGE_ACKED|TI_LBV_ABORT_ACKED)) {
	if (input_status & (TI_PREPARE_COMPLETE|TI_CHANGE_COMPLETE|TI_START_CHANGE_ACKED)) {
	    update_status(RCS_DONE, lbvioCommand->serial_number);
	}
	...
	/* lbvioStatus.status is set to RCS_DONE here which would cause
	 ...
	 * until the step is completed need to set lbvioStatus.status to RCS_EXEC
	 * (LBV_TOOL_PREPARE_TYPE, LBV_TOOL_LOAD_TYPE)
	 * to cause an abort, set lbvioStatus.reason and
	 * lbvioStatus.status to RCS_ERROR.
	 */
	lbvioStatus.status = RCS_DONE;
	...
	case LBV_TOOL_ABORT_TYPE:
	    ...
	    rtapi_print_msg(RTAPI_MSG_DBG, "LBV_TOOL_ABORT reason=%d\n",((LBV_TOOL_ABORT *) lbvioCommand)->reason);
		...
		*(iocontrol_data->lbv_reason) = ((LBV_TOOL_ABORT *) lbvioCommand)->reason;
		*(iocontrol_data->lbv_abort) = 1;      // notify TC of abort conditon
	    }
	...
	{
	    int p = ((LBV_TOOL_PREPARE*)lbvioCommand)->pocket;
		...
		*(iocontrol_data->tool_prep_number) = lbvioStatus.tool.toolTable[p].toolno;
		if (lbvioStatus.tool.toolTable[p].toolno != t) // sanity check
		    rtapi_print_msg(RTAPI_MSG_DBG, "LBV_TOOL_PREPARE: mismatch: tooltable[%d]=%d, got %d\n", 
				    p, lbvioStatus.tool.toolTable[p].toolno, t);
	    }
	...
	case LBV_TOOL_START_CHANGE_TYPE:
	    rtapi_print_msg(RTAPI_MSG_DBG, "LBV_START_CHANGE\n");
	...
	case LBV_TOOL_SET_NUMBER_TYPE:
	{
	    ...
	    number = ((LBV_TOOL_SET_NUMBER *) lbvioCommand)->tool;
	    rtapi_print_msg(RTAPI_MSG_DBG, "LBV_TOOL_SET_NUMBER pocket=%d old_loaded=%d new_number=%d\n",
			    number, lbvioStatus.tool.toolInSpindle,
			    lbvioStatus.tool.toolTable[number].toolno);
	    lbvioStatus.tool.toolInSpindle = lbvioStatus.tool.toolTable[number].toolno;
	    load_tool(number);
	    *(iocontrol_data->tool_number) = lbvioStatus.tool.toolInSpindle; //likewise in HAL
	}
	break;
	...
	lbvioStatus.heartbeat++;
	lbvioStatus.reason = toolchanger_reason;  // always piggyback current fault code
	lbvioStatusBuffer->write(&lbvioStatus);
