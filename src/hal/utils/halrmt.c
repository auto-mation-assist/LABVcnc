...
*   Simple telnet interface to LBV2 HAL commands (halcmd)
*
...
/*******************************************************************
  ...
  With -- --port Waits for socket connections (Telnet) on specified socket, without port
            uses default port 5006. (note: labvcncrsh uses 5007 as default)
...
/* non-LBV related uses of halrmt may want to avoid libnml dependency */
#ifndef NO_INI
...
char pwd[16] = "LBV\0";              // Connect password
char enablePWD[16] = "LBVTOO\0";     // Enable password
char serverName[24] = "LBVNETSVR\0"; // Server name written in hello response
...
//    retval = do_loadusr_cmd(argv);
    ...
    int m=0;
    char *argv[MAX_TOK+3];
    static char *rtmod_dir = LBV2_RTLIB_DIR;
    ...
    argv[0] = LBV2_BIN_DIR "/labvcnc_module_helper";
    argv[1] = "insert";
    argv[2] = mod_path;
    /* loop thru remaining arguments */
    n = 0;
...
static int doLoadUsr(char *args[])
{
    ...
    if ( prog_path[0] == '\0' ) {
	/* no luck yet, try the lbv2/bin directory where
