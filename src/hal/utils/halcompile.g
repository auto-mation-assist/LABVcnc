...
""", file=f)
    ...
    for v in docs:
        if not v: continue
        v = ":".join(map(str, v))
        print("MODULE_INFO(labvcnc, %s);" % q(v), file=f)
...
def find_modinc():
    ...
    for e in ['src', 'etc/labvcnc', '/etc/labvcnc', 'share/labvcnc']:
...
def build_usr(tempdir, filename, mode, origfilename):
    ...
    print("\t$(CC) $(EXTRA_CFLAGS) -URTAPI -U__MODULE__ -DULAPI -Os %s -o $@ $< -Wl,-rpath,$(LIBDIR) -L$(LIBDIR) -llabvcnchal %s" % (
...
def document(filename, outfilename):
    ...
    print(".TH %s \"9\" \"%s\" \"LabvCNC Documentation\" \"HAL Component\"" % (
...
def usage(exitval=0):
    print("""%(name)s: Build, compile, and install LabvCNC HAL components
