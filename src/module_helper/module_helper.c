...
run LBV.  Without a scheme like this, we have to rely on sudo
 ...
 * if it's inside the LBV2_RTLIB_DIR, or if it's in a directory
...
void check_whitelist_module_path(char *mod, int argc, char **argv) {
    ...
    if(strncmp(mod, LBV2_RTLIB_DIR, strlen(LBV2_RTLIB_DIR)) == 0)
        return;
 ...
 * in the LBV2_RTLIB_DIR with the extension MODULE_EXT.
 */
void check_whitelist_module(char *mod, int argc, char **argv) {
    char *end;
    DIR *d = opendir(LBV2_RTLIB_DIR);
