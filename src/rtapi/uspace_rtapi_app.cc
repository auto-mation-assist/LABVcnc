...
static int do_load_cmd(string name, vector<string> args) {
    ...
    if(w == NULL) {
        char what[LINELEN+1];
        snprintf(what, LINELEN, "%s/%s.so", LBV2_RTLIB_DIR, name.c_str());
    ...
    if(getuid() == 0) {
        ...
        if(fallback_uid == 0)
        {
            fprintf(stderr,
                ...
                "    sudo env RTAPI_UID=`id -u` RTAPI_FIFO_PATH=$HOME/.rtapi_fifo gdb " LBV2_BIN_DIR "/rtapi_app\n");
            exit(1);
        }
...
static RtapiApp *makeApp()
{
    ...
    void *dll = nullptr;
    if(detect_xenomai()) {
        dll = dlopen(LBV2_HOME "/lib/libuspace-xenomai.so.0", RTLD_NOW);
        if(!dll) fprintf(stderr, "dlopen: %s\n", dlerror());
    } else if(detect_rtai()) {
        dll = dlopen(LBV2_HOME "/lib/libuspace-rtai.so.0", RTLD_NOW);
        if(!dll) fprintf(stderr, "dlopen: %s\n", dlerror());
    }
    if(dll)
    {
