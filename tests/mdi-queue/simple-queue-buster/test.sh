...
labvcnc -r labvcncrsh-test.ini &


# let labvcnc come up
TOGO=80
while [  $TOGO -gt 0 ]; do
    echo trying to connect to labvcncrsh TOGO=$TOGO
...
if [  $TOGO -eq 0 ]; then
    echo connection to labvcncrsh timed out
    exit 1
fi
...
(
    ...
    # ask labvcncrsh to not read the next command until it's done running
    # the current one
