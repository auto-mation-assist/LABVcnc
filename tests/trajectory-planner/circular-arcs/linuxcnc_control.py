...
class LabvcncControl:
    ...
    def running(self, do_poll=True):
        ...
        if do_poll:
            self.s.poll()
        return (self.s.task_mode == labvcnc.MODE_AUTO and
                self.s.interp_state != labvcnc.INTERP_IDLE)

    def set_mode(self,m):
        '''
        set LBV mode if possible, else throw LabvcncError
        return current mode
        '''
        self.s.poll()
        ...
        if self.running(do_poll=False):
            raise LabvcncError("interpreter running - cant change mode")
        self.c.mode(m)
    ...
    def prepare_for_mdi(self):
        '''
        check wether ok to run MDI commands.
        throw  LabvcncError if told so.
        return current mode
        '''

        self.s.poll()
        if self.s.estop:
            raise LabvcncError("machine in ESTOP")

        if not self.s.enabled:
            raise LabvcncError("machine not enabled")

        if not self.s.homed:
            raise LabvcncError("machine not homed")

        if self.running():
            raise LabvcncError("interpreter not idle")

        return self.set_mode(labvcnc.MODE_MDI)
    ...
    def open_program(self,filename):
        '''Open an nc file'''
        self.s.poll()
        self.set_mode(labvcnc.MODE_AUTO)
        self.c.wait_complete()
        sleep(.25)
    ...
    def run_full_program(self):
        ...
        self.s.poll()
        self.c.auto(labvcnc.AUTO_RUN, 0)
        self.c.wait_complete(self.timeout)
    ...
    def wait_on_program(self):
        self.s.poll()
        while self.s.exec_state != labvcnc.EXEC_DONE or self.s.state != labvcnc.RCS_DONE and self.s.task_state == labvcnc.STATE_ON:
            sleep(.25)
            self.s.poll()
            if self.s.task_state != labvcnc.STATE_ON:
                return False
    ...
    def check_rcs_error(self):
        self.s.poll()
        if self.s.state == labvcnc.RCS_ERROR:
