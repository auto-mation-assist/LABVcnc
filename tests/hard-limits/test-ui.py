...
def assert_wait_complete(command):
    ...
    assert((r == labvcnc.RCS_DONE) or (r == labvcnc.RCS_ERROR))
...
c.jog(labvcnc.JOG_CONTINUOUS, 1, 0, -0.1)
...
# let labvcnc react to the limit switch
...
while (time.time() - start_time < 5):
    ...
    if error != None:
        if error[1] == expected_error:
            ...
            print "labvcnc sent other error %d: %s" % (error[0], error[1])
    time.sleep(0.1)

if error == None or error[1] != expected_error:
    print "no limit switch error from LabvCNC"
    sys.exit(1)

print "labvcnc sent error %d: %s" % (error[0], error[1])
print_status(s)
...
c.jog(labvcnc.JOG_CONTINUOUS, 1, 0, 1)
...
# let labvcnc react to the limit switch untripping
start_time = time.time()
while (time.time() - start_time < 5):
    s.poll()
