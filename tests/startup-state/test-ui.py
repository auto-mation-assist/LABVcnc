...
assert(s.estop == 1)
assert(s.exec_state == labvcnc.EXEC_DONE)
...
assert(s.interp_state == labvcnc.INTERP_IDLE)
...
assert(s.motion_mode == labvcnc.TRAJ_MODE_FREE)
...
assert(s.state == labvcnc.STATE_ESTOP)

assert(s.task_mode == labvcnc.MODE_MANUAL)
assert(s.task_paused == 0)
assert(s.task_state == labvcnc.STATE_ESTOP)
