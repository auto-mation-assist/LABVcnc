...
proc ::tooledit::sendaxis {cmd} {
  ...
  switch $cmd {
    ...
    tool_table_filename {
      ...
      if [catch {set f [send axis inifindall LBVIO TOOL_TABLE]} msg] {
        return -code error "::tooledit::sendaxis tool_table_filename <$msg>"
      }
      if {[llength $f] > 1} {
        set f [lindex $f 0] ;# use first item specified for compatibility
        puts stderr [format [_ "%s: Axis inifile specifies multiple inifile items for: \[LBVIO\]TOOL_TABLE"] $prog]
