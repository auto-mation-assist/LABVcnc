...
#   ngcgui [Options] -i LabvCNC_inifile_name
#   ngcgui [Options]
#
#   Options:
#          [-S subroutine_file]
...
# named "O" words available since: LabvCNC 2.3.0, April 19, 2009
...
#      3.7 Calling Files of the LabvCNC ________ manual
...
# The labvcnc gcode language does not provide a mechanism for returning
...
#  1) The directory location for ngc gcode files used in labvcnc is specified
#     in the ini file by: [DISPLAY]PROGRAM_PREFIX.
#     In labvcnc2.5, multiple directories can be specified using
...
# Notes:
#   0. configuring ngcgui is simplified with labvcnc2.5; support for
#      labvcnc2.4 will cease when labvcnc2.5 is released
...
#   3. LabvCNC gcode supports labels for conditional blocks and subroutines
...
#      This seems consistent with the trajectory of LabvCNC gcode and
...
#      cause the current value (lbv_rel_act_pos) to be entered into the
...
#  13. Lbv gcode (2.3 19apr09) allows a single semicolon use for comments.
...
#  14. Features requiring labvcnc-2.4pre (that I can remember):
...
#
# 19. For labvcnc 2.4, the tcl proc embed_in_axis_tab will embed directly
...
# use the tcl-package named Lbv to set up I18n support
if [catch {package require Labvcnc} msg] {
  # if user is trying to use as standalone in an unconfigured (non-Lbv)
          ...
          if {$h >= 0} {
            ...
            if [info exists ::ngc($h,axis,page)] {
              lappend tnames [$::ngc(any,axis,parent) \
                            itlbvget $::ngc($h,axis,page) -text]
            }
          }
        }
        set thisone [$::ngc(any,axis,parent) \
                         itlbvget $::ngc($hdl,axis,page) -text]
  ...
  switch $cmd {
    ping {
      ...
      if {[string first "X server insecure" $msg] >= 0} {
         ...
         puts stdout "[_ "You should upgrade labvcnc to >= labvcnc2.5"]"
    ...
    file {
      ...
      if ![catch {send axis "remote open_file_name $f"} msg] {
        if {"$msg" == ""} {
         ...
         # try method that may work for axis in labvcnc2.3.x
...
proc ::ngcgui::entrykeybinding {ax w v} {
  ...
  if [catch {
    set value [lbv_rel_act_pos $coord]
    switch $axis {
    ...
    # silently ignore, lbv_rel_act_pos will fail in standalone
...
proc ::ngcgui::usage {hdl ay_name} {
  ...
  $prog \[Options\] -i LabvCNC_inifile_name
  $prog \[Options\]
...
proc ::ngcgui::embed_in_axis_tab {f args} {
  ...
  # package require Labvcnc ;# needs labvcnc v2.5.x, segfaults labvcnc v2.4.x
  # just invoking lbv_init works with v2.4 and v2.5
  if  [catch {lbv_init} msg] {
