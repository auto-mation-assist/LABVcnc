...
###############################################################
# Description:  lbvtuning.tcl
#               LBV system identification and autotuning program
#
...
if {! [string length [info commands lbv_plat]]} {
    error "lbvtuning needs to run from \"lbvsh\""
}

# set cycle time for updating, using tklbv's displayCycleTime if
...
proc setVoltage {} {
    global stepvolts
    global activeAxis
    lbv_axis_enable $activeAxis 1
    lbv_axis_set_output $activeAxis $stepvolts
}
...
proc logIt {} {
   ...
   global stepfilename
   global stepssaved
   lbv_log_open $stepfilename$stepssaved pos_voltage $stepsize 0 $activeAxis
   lbv_wait done
   lbv_log_start
}
...
proc doTune  {} {
    ...
    scan [exec -- /usr/local/nist/lbv/system $string] %s%s%s pgain igain dgain
}
...
proc popupTuning {{w .logwindow}} {
    ...
    set ampenable [lbv_axis_enable $activeAxis]
