...
###############################################################
# Description:  lbvcalib.tcl
#               A Tcl/Tk script that interfaces with LabvCNC
...
###############################################################
# LBV parameter setting application
# Needs lbvsh to run
...
set ::EC(numjoints) [lbv_ini "JOINTS" "KINS"]
...
set ::EC(thisinifile) "$::LBV_INIFILE"
...
# Wizard logo
set lname $::env(LABVCNC_HOME)/share/labvcnc/labvcnc-wizard.gif
...
wm title . [msgcat::mc "LabvCNC Calibration"]
...
proc incompatible_ini_file {} {
   ...
   set fname [file tail $::LBV_INIFILE]
...
proc halcmdSubstitute {s} {
    ...
    while {$pos < [string length $s]
        ...
        append result [lbv_ini $var $section]
