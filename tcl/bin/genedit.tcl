...
# Mod for a script menu that looks for *.ncw files in $labvcnc::TCL_SCRIPT_DIR directory.
###############################################################
...
proc geneditStart {name {ifilename "untitled.txt"} {itypes { {"All files" *} {"Text files" {.txt} }}}} {
	...
	foreach file $files {
        	...
        	set geneditfname [file rootname $file]
            	$scriptmenu add command -label $geneditfname -command "source $labvcnc::TCL_SCRIPT_DIR/$file"
    	    }
    	}
    }
}
