...
#               This file validates a LabvCNC configuration
...
# Load the labvcnc.tcl file, which defines variables for various useful paths
source [file join [file dirname [info script]] .. labvcnc.tcl]

set logo [labvcnc::image_search labvcnc-wizard]
image create photo machinelogo

option add *font [labvcnc::standard_font]
option add *Text.font [labvcnc::standard_fixed_font]
...
# start on this node if no ~/.labvcncrc:
...
# use initialize_config for bwidget and .labvcncrc
proc initialize_config {} {
    ...
    if {$result != 0} {
        ...
        if {[catch {open ~/.labvcncrc} programin]} {
            return 
        } else {
...
proc title {node} {
  ...
  wm title . "[msgcat::mc "LabvCNC Configuration Selector"] $txt"
}
...
proc ok_to_copy_config {filename} {
    ...
    #    specified by the environmental variable LABVCNC_AUX_EXAMPLES),
...
set ::configs_dir_list $labvcnc::CONFIG_DIR
set ::myconfigs_node   $labvcnc::USER_CONFIG_DIR
# order convention for items in the labvcnc::USER_CONFIG_DIR list:
set ::sampleconfigs [lindex $::configs_dir_list end] ;# last item

# check for auxiliary applications example configs installed in
# known directory defined by: labvcnc::LABVCNC_AUX_ExAMPLES
set aux_examples $labvcnc::LABVCNC_AUX_EXAMPLES
set ::aux_dir_list {}
...
# variable LABVCNC_AUX_CONFIGS (a colon (:) separated path)
# This is useful for testing auxiliary apps without using a deb install
if [info exists ::env(LABVCNC_AUX_CONFIGS)] {
    foreach auxdir [split $::env(LABVCNC_AUX_CONFIGS) :] {
        if {   [file isdirectory $auxdir] \
            ...
            puts stderr "pickconfig: LABVCNC_AUX_CONFIGS invalid directory:"
            puts stderr "    $auxdir"
...
proc prompt_copy configname {
    ...
    set obsoletedir [file normalize [file join ~ lbv2]]
    ...
    if {$::always_update_nc_files || ![file exists $ncfiles]} {
        ...
        set refname  $labvcnc::NCFILES_DIR
        ...
        set _libs [glob [file join $labvcnc::NCFILES_DIR *_lib]]
        foreach lib $_libs {
           if ![file isdir $lib] continue
           lappend liblist $lib
        }
        foreach lib $liblist {
           set refname  [file join $labvcnc::NCFILES_DIR $lib]
        ...
        set msg "${msg}(debug, LabvCNC vmajor=#<_vmajor> vminor=#<_vminor>)\n"
    ...
    while {1} {
		...
		# PROGRAM_PREFIX=/home/username/labvcnc/nc_files
    ...
    tk_dialog .d [msgcat::mc "Configuration Copied"] [msgcat::mc "The configuration file has been copied to %s. Next time, choose this location when starting LabvCNC." $copytodir] info 0 [msgcat::mc "OK"]

    return $copytodir/[file tail $configname]
}


# add the selection set if a ::last_ini has been found in ~/.labvcncrc
...
proc make_shortcut {inifile} {
    ...
    switch [file extension $inifile] {
      ".ini"  {set name [getVal $inistring LBV MACHINE]}
      ".demo" {set name [lindex [split [file tail $inifile] "."] 0]
              }
      default {set name labvcnc-other}
    }
    if {"$name" == ""} {set name LabvCNC-other}
    ...
    exec labvcncmkdesktop $inifile $name > $filename
...
while {1} {
    ...
    if { $::choice == "OK" } {
        ...
        # test for ~/.labvcncrc file and modify if needed.
        # or make this file and add the var.

        if {[file exists ~/.labvcncrc]} {
            if {$::inifile == $::last_ini} {
                exit
            } else {
                if {[catch {open ~/.labvcncrc} programin]} {
                ...
                if {[catch {open ~/.labvcncrc w} fileout]} {
                    puts stdout [msgcat::mc "can't save %s" ~/.labvcncrc ]
                    exit
                }
                puts $fileout $ret
                catch {close $fileout}
        ...
        set newfilestring "# .labvcncrc is a startup configuration file for LabvCNC. \n# format is INI like. \n# \[SECTION_NAME\] \n# VARNAME = varvalue \n# where section name is the name of the system writing to this file \n\n# written by pickconfig.tcl \n\[PICKCONFIG\]\nLAST_CONFIG = $::inifile\n"
                
        if {[catch {open ~/.labvcncrc w+} fileout]} {
            puts stderr [msgcat::mc "can't save %s" ~/.labvcncrc ]
            exit
        }
