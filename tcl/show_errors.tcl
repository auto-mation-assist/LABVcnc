...
# Load the Labvcnc package, which defines variables for various useful paths
...
proc create_report {} {
  set   info_file   /tmp/labvcnc_info.txt ;# created by labvcnc_info
  set   report_file /tmp/labvcnc.report ;# created by this script
  exec  labvcnc_info -s                 ;# -s: suppress opening by a viewer
  ...
  puts  $fd $txt
  puts  $fd "-----------------------------------------------------------------------"
  puts  $fd "Info report created by labvcnc_info:"
  close $fd
  exec  cat $info_file >> $report_file
  popup_msg "Report file created by labvcnc_info:\n\n$report_file"
}
...
wm ti . [msgcat::mc "LabvCNC Errors"]
frame .f
label .f.b -bitmap error
label .f.l -justify l -wraplength 400 -text [msgcat::mc "LabvCNC terminated with an error.  When reporting problems, please create a report file and include in your message."]
