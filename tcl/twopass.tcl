...
proc ::tp::source_the_files {} {
  ...
  foreach file_plus_args $::TP(runfiles) {
    ...
    if {[file extension $f] == ".tcl"} {
      ...
      #       the way the labvcnc script uses haltcl to use tcl files
...
# the labvcnc script exports LABVCNC_TCL_DIR
# make sure it is first in case run-in-place with existing deb install
if [info exists ::auto_path] {
  # prepend:
  set ::auto_path [lreplace $::auto_path -1 -1 $::env(LABVCNC_TCL_DIR)]
} else {
  set ::auto_path $::env(LABVCNC_TCL_DIR)
}
