...
exec ${LABVCNC_LBVSH-lbvsh} "$0" "$@"
###############################################################
# Description:  tklbv
#               A Tcl/Tk GUI script that defines default  
#               lbv interface functionality. 
#
...
###############################################################
# Uses the lbvsh library of tcl/tk functions.
# Provides default display or operators for each NML message
###############################################################


# Load the lbv package, which defines variables for various useful paths
...
set ::KINEMATICS_IDENTITY 1     ;# src/lbv/kinematics/kinematics.h
set ::MAX_JOINTS 9              ;# src/lbv/motion/lbvmotcfg.h
...
# 3. Adds info to help menu that calls tcl.sysinfo.tcl and uses lbv/lbvversion

# Set global that signifies we're the tklbv, so that other sourced scripts
# can know how to pack themselves. This lets these scripts pack into the
# main window "." if "tklbv" is not defined, or in their own top-level window
# if "tklbv" is defined. lbvlog.tcl looks at this.
set tklbv 1
...
set TKLBVCONF $labvcnc::TCL_DIR/TkLabvCNC
if {[file exists $TKLBVCONF]} {
    option readfile $TKLBVCONF startupFile
}
...
foreach f {TkLabvCNC /usr/X11R6/lib/X11/app-defaults/TkLabvCNC /etc/X11/app-defaults/TkLabvCNC} {
    if {[file exists $f]} {
	option readfile $f
	break
    }
...
set ::numjoints [lbv_ini "JOINTS" "KINS"]
if {$::numjoints == 0} {
  puts "$::prog: \[KINS\]JOINTS is missing or 0"
  exit 1
}

set ::coordnames [ lbv_ini "COORDINATES" "TRAJ" ]
...
set temp [lbv_ini "CYCLE_TIME" "DISPLAY"]
...
if { $windows == 0 } {
    ...
    source $labvcnc::TCL_BIN_DIR/genedit.tcl

    # load the LBV calibrator
#    source $labvcnc::TCL_BIN_DIR/lbvcalib.tcl

    # load the LBV data logger
#    source $labvcnc::TCL_BIN_DIR/lbvlog.tcl

    # load the LBV performance tester
    source $labvcnc::TCL_BIN_DIR/lbvtesting.tcl

    # load the LBV debug level setter
    source $labvcnc::TCL_BIN_DIR/lbvdebug.tcl

    # load the backplotter
    source $labvcnc::TCL_BIN_DIR/tkbackplot.tcl

    # load balloon help
    source $labvcnc::TCL_SCRIPT_DIR/balloon.tcl
    source $labvcnc::TCL_SCRIPT_DIR/lbvhelp.tcl
    set do_balloons [lbv_ini "BALLOON_HELP" "DISPLAY"]
    if {$do_balloons == ""} {
        set do_balloons 0
    }
}
...
set toolfilename [lbv_ini "TOOL_TABLE" "LBVIO"]
if {[string length $toolfilename] == 0} {
    set toolfilename "lbv.tbl"
}

set paramfilename [lbv_ini "PARAMETER_FILE" "RS274NGC"]
if {[string length $paramfilename] == 0} {
    set paramfilename "lbv.var"
}
...
proc popupParamEditor {} {
    ...
    button .paramEditor.buttons.load -text [msgcat::mc "Load Parameter File"] -command {geneditSaveFile paramEditor; lbv_task_plan_init}
...
set helpfilename [lbv_ini "HELP_FILE" "DISPLAY"]
...
proc popupHelp {} {
    ...
    if {[file isfile $labvcnc::HELP_DIR/$helpfilename]} {
        geneditStart help $labvcnc::HELP_DIR/$helpfilename
    } else {
        geneditStart help $labvcnc::HELP_DIR/tklinucnc.txt
    }
...
proc popupDiagnostics {} {
    ...
    toplevel $d
    wm title $d [msgcat::mc "LabvCNC Diagnostics"]
...
proc popupAbout {} {
    ...
    toplevel .about
    wm title .about [msgcat::mc "About TkLabvCNC"]
    message .about.msg -aspect 1000 -justify center -font {Helvetica 12 bold} -text [ format "%s\n(LabvCNC %s)" [msgcat::mc "TkLabvcnc\n\nTcl/Tk GUI for LabvCNC\n\nGPL Version 2 (2012)"] $env(LABVCNCVERSION) ]
    frame .about.buttons
...
proc toolenter {} {
  ...
  if {"$cmd" == ""} return
  set restore [lbv_mode]
  lbv_mode mdi
  lbv_mdi "G10 L1 P$::tentry(toolno) $cmd"
  lbv_mdi G43
  lbv_mode $restore
}
...
# set waiting on LBV to wait until command is received, not finished, with
# timeout of 10 seconds
lbv_set_timeout 1.0
lbv_set_wait received
...
set programDirectory [lbv_ini "PROGRAM_PREFIX" "DISPLAY"]
...
set temp [lbv_ini "POSITION_OFFSET" "DISPLAY"]
if {$temp == "RELATIVE"} {
    set coords relative
} elseif {$temp == "MACHINE"} {
    set coords machine
...
set temp [lbv_ini "POSITION_FEEDBACK" "DISPLAY"]
if {$temp == "ACTUAL"} {
    set actcmd actual
...
proc toggleJointWorld {} {
    if { [lbv_kinematics_type] == $::KINEMATICS_IDENTITY} {
        return
    }
    if {$::jointworld == "joint"} {
...
proc jogNeg {jora} {
    ...
    if {$::jogType == "continuous"} {
        lbv_jog      $jatojog $jogmode -$speed
    } else {
        lbv_jog_incr $jatojog $jogmode -$speed $::jogIncrement
    }
}
...
proc jogStop {jora} {
    ...
    if {$::jogType == "continuous"} {
        lbv_jog_stop $jatostop $jogmode
    }
}

# toggles the limit override state
proc toggleLimitOverride {} {
    if {[lbv_override_limit]} {
        lbv_override_limit 0
    } else {
        lbv_override_limit 1
    }
}

# toggles the optional stop
proc toggleOptionalStop {} {
    if {[lbv_optional_stop]} {
        lbv_optional_stop 0
    } else {
        lbv_optional_stop 1
    }
...
set tooleditor [lbv_ini "TOOL_EDITOR" "DISPLAY"]
if {$tooleditor == ""} {
...
$filemenu add command -label [msgcat::mc "Reload Tool Table"] \
                      -command "lbv_load_tool_table [file normalize $toolfilename]"
$filemenu add command -label [msgcat::mc "Reset"] -command {lbv_task_plan_init} -underline 0
...
$settingsmenu add command -label [msgcat::mc "Calibration..."] -command "exec $labvcnc::TCL_BIN_DIR/lbvcalib.tcl -- -ini $LBV_INIFILE &"
...
$unitsmenu add command -label [msgcat::mc "auto"] -command {lbv_linear_unit_conversion auto} -underline 0
$unitsmenu add command -label [msgcat::mc "inches"] -command {lbv_linear_unit_conversion inch} -underline 0
$unitsmenu add command -label [msgcat::mc "mm"] -command {lbv_linear_unit_conversion mm} -underline 0
$unitsmenu add command -label [msgcat::mc "cm"] -command {lbv_linear_unit_conversion cm} -underline 0
...
$utilsmenu add command -label [msgcat::mc "Hal Scope"] -command {exec halscope -- -ini $LBV_INIFILE &}
...
if { $windows == 0 } {
    set files [exec /bin/ls $labvcnc::TCL_SCRIPT_DIR]
    foreach file $files {
	if {[string match *.tcl $file]} {
	    set fname [file rootname $file]
	    # if it's executable, arrange for direct execution
	    if {[file executable $labvcnc::TCL_SCRIPT_DIR/$file]} {
		$scriptsmenu add command -label [msgcat::mc "Set Coordinates"] -command "exec $labvcnc::TCL_SCRIPT_DIR/$file -- -ini $LBV_INIFILE &"
	    }
	}
    }
}
...
$scriptsmenu add command -label [msgcat::mc "HAL Show"] -command "exec $labvcnc::TCL_BIN_DIR/halshow.tcl &"
$scriptsmenu add command -label [msgcat::mc "HAL Config"] -command "exec $labvcnc::TCL_BIN_DIR/halconfig.tcl -- -ini $LBV_INIFILE &"
...
#$helpmenu add command -label [msgcat::mc "Info..."] -command {catch {exec tcl/sysinfo.tcl -- -ini $LBV_INIFILE}} -underline 0
$helpmenu add separator
...
$estopmenu add command -label [msgcat::mc "Estop on"] -command {lbv_estop on}
$estopmenu add command -label [msgcat::mc "Estop off"] -command {lbv_estop off}
$estopmenu add separator
$estopmenu add command -label [msgcat::mc "Machine on"] -command {lbv_machine on}
$estopmenu add command -label [msgcat::mc "Machine off"] -command {lbv_machine off}
...
$modemenu add command -label [msgcat::mc "Manual"] -command {lbv_mode manual}
$modemenu add command -label [msgcat::mc "Auto"] -command {lbv_mode auto}
$modemenu add command -label [msgcat::mc "MDI"] -command {lbv_mode mdi}
...
$mistmenu add command -label [msgcat::mc "Mist on"] -command {lbv_mist on}
$mistmenu add command -label [msgcat::mc "Mist off"] -command {lbv_mist off}
...
$floodmenu add command -label [msgcat::mc "Flood on"] -command {lbv_flood on}
$floodmenu add command -label [msgcat::mc "Flood off"] -command {lbv_flood off}
...
$lubemenu add command -label [msgcat::mc "Lube on"] -command {lbv_lube on}
$lubemenu add command -label [msgcat::mc "Lube off"] -command {lbv_lube off}

# FIXME-- there's probably a better place to put the lube button
# if {[lbv_ini LUBE_WRITE_INDEX LBVIO] != ""} {
...
bind $decrbutton <ButtonPress-1> {lbv_spindle decrease}
bind $decrbutton <ButtonRelease-1> {lbv_spindle constant}
...
$spindlemenu add command -label [msgcat::mc "Spindle forward"] -command {lbv_spindle forward}
$spindlemenu add command -label [msgcat::mc "Spindle reverse"] -command {lbv_spindle reverse}
$spindlemenu add command -label [msgcat::mc "Spindle off"] -command {lbv_spindle off}
...
bind $incrbutton <ButtonPress-1> {lbv_spindle increase}
bind $incrbutton <ButtonRelease-1> {lbv_spindle constant}
...
$brakemenu add command -label [msgcat::mc "Brake on"] -command {lbv_brake on}
$brakemenu add command -label [msgcat::mc "Brake off"] -command {lbv_brake off}
...
bind $abortbutton <ButtonPress-1> {tklbv_abort}
...
for {set joint 0} {$joint < $::MAX_JOINTS} {incr joint} {
  set temp [lbv_ini "TYPE" "JOINT_$joint"]
  switch $temp {
...
set userfont [lbv_ini "POSITION_FONT" "DISPLAY"]
if {$userfont != ""} {
...
bind $limoridebutton <ButtonPress-1> {lbv_override_limit}
...
bind $homebutton   <ButtonPress-1>   {lbv_home $::activeJA}
...
proc setAxisOffset {axisnum offset} {
    ...
    set restore_mode [lbv_mode]
    lbv_mode mdi
    lbv_mdi $string
    lbv_mode $restore_mode
}
...
proc popupOride {} {
    ...
    button .oridepopup.buttons.ok -text [msgcat::mc "OK"] -default active -command {set feedoverride $popupOrideEntry; lbv_feed_override $feedoverride; destroy .oridepopup}
    ...
    bind .oridepopup <Return> {set feedoverride $popupOrideEntry; lbv_feed_override $feedoverride; destroy .oridepopup}
...
proc popupSoride {} {
    ...
    button .soridepopup.buttons.ok -text [msgcat::mc "OK"] -default active -command {set spindleoverride $popupSorideEntry; lbv_spindle_override $spindleoverride; destroy .soridepopup}
    ...
    bind .soridepopup <Return> {set spindleoverride $popupSorideEntry; lbv_spindle_override $spindleoverride; destroy .soridepopup}

    focus .soridepopup.input.entry
    .soridepopup.input.entry select range 0 end
}

set realfeedoverride [lbv_feed_override]
set feedoverride $realfeedoverride

set realspindleoverride [lbv_spindle_override]
set spindleoverride $realspindleoverride

# read the max feed override from the ini file
set temp [lbv_ini "MAX_FEED_OVERRIDE" "DISPLAY"]
if {[string length $temp] == 0} {
    set temp 1
}
...
set temp [lbv_ini "MIN_SPINDLE_OVERRIDE" "DISPLAY"]
if {[string length $temp] == 0} {
    set temp 1
}
...
set temp [lbv_ini "MAX_SPINDLE_OVERRIDE" "DISPLAY"]
if {[string length $temp] == 0} {
    set temp 1
}
...
set oridescale [scale $oridebottom.scale  -from 1 -to $maxFeedOverride -variable feedoverride -orient horizontal -showvalue 0 -command {lbv_feed_override} -takefocus 0]
...
set soridescale [scale $soridebottom.scale  -from $minSpindleOverride -to $maxSpindleOverride -variable spindleoverride -orient horizontal -showvalue 0 -command {lbv_spindle_override} -takefocus 0]
...
set programrunbutton [button $programframe.run -text [msgcat::mc "Run"] -command {lbv_run $runMark ; set runMark 0} -takefocus 0]
set programpausebutton [button $programframe.pause -text [msgcat::mc "Pause"] -command {lbv_pause} -takefocus 0]
set programresumebutton [button $programframe.resume -text [msgcat::mc "Resume"] -command {lbv_resume} -takefocus 0]
set programstepbutton [button $programframe.step -text [msgcat::mc "Step"] -command {lbv_step} -takefocus 0]
set programverifybutton [button $programframe.verify -text [msgcat::mc "Verify"] -command {lbv_run -1} -takefocus 0]
...
bind . <KeyPress-F3> {lbv_mode manual}
bind . <KeyPress-F4> {lbv_mode auto}
bind . <KeyPress-F5> {lbv_mode mdi}
bind . <KeyPress-F6> {lbv_task_plan_init}
...
bind ManualBindings <KeyPress-b> {lbv_brake off}
bind ManualBindings <Shift-KeyPress-B> {lbv_brake on}
bind ManualBindings <$modifier-t> {popupToolOffset}
...
bind ManualBindings <Home> {lbv_home $::activeJA}

bind AutoBindings <KeyPress-1> {lbv_feed_override 10}
bind AutoBindings <KeyPress-2> {lbv_feed_override 20}
bind AutoBindings <KeyPress-3> {lbv_feed_override 30}
bind AutoBindings <KeyPress-4> {lbv_feed_override 40}
bind AutoBindings <KeyPress-5> {lbv_feed_override 50}
bind AutoBindings <KeyPress-6> {lbv_feed_override 60}
bind AutoBindings <KeyPress-7> {lbv_feed_override 70}
bind AutoBindings <KeyPress-8> {lbv_feed_override 80}
bind AutoBindings <KeyPress-9> {lbv_feed_override 90}
bind AutoBindings <KeyPress-0> {lbv_feed_override 100}
...
bind AutoBindings <KeyPress-r> {lbv_run $runMark ; set runMark 0}
bind AutoBindings <KeyPress-p> {lbv_pause}
bind AutoBindings <KeyPress-s> {lbv_resume}
bind AutoBindings <KeyPress-a> {lbv_step}
bind AutoBindings <KeyPress-v> {lbv_run -1}
bind AutoBindings <$modifier-t> {popupToolOffset}

proc spindleDecrDone {} {
    lbv_spindle constant
...
bind . <Escape> {tklbv_abort}

# force explicit updates, so calls to lbv_estop, for example, don't
...
set lbv_teleop_enable_command_given 0
...
proc updateStatus {} {
    ...
    global lbv_teleop_enable_command_given
    ...
    if {![winfo exists .tooloffset] || ![winfo ismapped .tooloffset]} {
      set ::tentry(toolno) [lbv_tool]
      set tooloffsetsetting [format "X%.4f Y%.4f Z%.4f" [lbv_tool_offset X] [lbv_tool_offset Y] [lbv_tool_offset Z]]
      # note: currently no lbv_tool_offset options for diam,front,back,orient
      foreach item "$::coordnames" {
        set ::tentry($item) [format %.4f [lbv_tool_offset $item]]
      }
    }

    # set the offset information
    set offsetsetting [lsearch -inline [lbv_program_codes] {G5[4-9]*}]
    for {set i 0} {$i < $::numjoints} {incr i} {
    if { [lsearch $::coordnames [lindex $::worldlabellist $i]] != -1 } {
      set fstr [lindex $::worldlabellist $i]
      set spec " $fstr%.4f"
      set args [lbv_pos_offset $fstr]
      append offsetsetting [format $spec $args]
      }
    }

    # set the unit information
    set unitsetting [lbv_display_linear_units]
    ...
    set teleop_mode [lbv_teleop_enable]
    ...
    if {$::jointworld == "joint" } {
        ...
        for {set idx 0} {$idx < $::numjoints} {incr idx} {
          set posdigit$idx [format "%9.4f" [lbv_joint_pos $idx] ]
        }
    } else {
        # TELEOP
        if { $::lastjointworld != "world" } {
            if {   [lbv_teleop_enable] == 0
                && [lbv_kinematics_type] != $::KINEMATICS_IDENTITY
                && $modeInDisplay == "manual"
               } {
                if { $lbv_teleop_enable_command_given == 0 } {
                    lbv_teleop_enable 1
                    set lbv_teleop_enable_command_given 1
                } else {
                    set ::jointworld "joint"
                    set lbv_teleop_enable_command_given 0
                }
        ...
        if { $::lastjointworld == "world" } {
          for {set idx 0} {$idx < $::numjoints} {incr idx} {
            ...
            if {$coords == "relative" && $actcmd == "commanded"} {
                set posdigit$idx [format "%9.4f" [lbv_rel_cmd_pos $letter ] ]
            } elseif {$coords == "relative" && $actcmd == "actual"} {
                set posdigit$idx [format "%9.4f" [lbv_rel_act_pos $letter  ] ]
            } elseif {$coords == "machine" && $actcmd == "commanded"} {
                set posdigit$idx [format "%9.4f" [lbv_abs_cmd_pos $letter ] ]
            } else {
                # $coords == "machine" && $actcmd == "actual"
                set posdigit$idx [format "%9.4f" [lbv_abs_act_pos $letter ] ]
            }
        }
      }
    }
    ...
    set programcodestring [lbv_program_codes]

    # fill in the program status
    set oldstatusstring $programstatusstring
    set programstatusstring [lbv_program_status]
    ...
    if {[winfo exists .diagnostics]} {
        set taskhb [lbv_task_heartbeat]
        set taskcmd [lbv_task_command]
        set tasknum [lbv_task_command_number]
        set taskstatus [lbv_task_command_status]
        set iohb [lbv_io_heartbeat]
        set iocmd [lbv_io_command]
        set ionum [lbv_io_command_number]
        set iostatus [lbv_io_command_status]
        set motionhb [lbv_motion_heartbeat]
        set motioncmd [lbv_motion_command]
        set motionnum [lbv_motion_command_number]
        set motionstatus [lbv_motion_command_status]
    }
...
# LBV_TRAJ_SET_ORIGIN message so the stat buffer (and our gui) gets updated
proc tklbv_abort {} {
    lbv_abort
    set oldmode [lbv_mode]
    lbv_mode mdi
    lbv_mdi "G55"
    lbv_mdi "G54"
    lbv_mode $oldmode
...
set temp [lbv_ini "USER_COMMAND_FILE" "DISPLAY"]
if [file exists $temp] {
  source $temp
}
