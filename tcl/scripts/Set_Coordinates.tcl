...
wm title . [msgcat::mc "LabvCNC Set Coordinate"]
...
proc getLocation {axletter} {
    if {$axletter == "all"} {
        foreach letter $::coordnames {
            set ::val($letter) [expr [lbv_abs_act_pos $i] - $::forceval($letter)]]
        }
    } else {
        set ::val($axletter) [expr [lbv_abs_act_pos $axletter] - $::forceval($axletter)]
    }
}
