...
# window signal otherwise you can't close down labvcnc 
...
ALERT_ICON = "/home/jim/labvcnc/configs/silverdragon/images/applet-critical.png"
INFO_ICON = "/home/jim/labvcnc/configs/silverdragon/images/std_info.gif"
...
class HandlerClass:
    ...
    # command is for control and status of labvcnc
    # data is important data from gscreen and labvcnc
...
# global variables
    ...
    # Since this custom screen uses gladeVCP magic for its interaction with labvcnc
    ...
    def periodic(self):
        # put the poll comand in a try, so if the labvcnc pid is killed
        # from an external command, also quit the GUI
        try:
            self.stat.poll()
        except:
            raise SystemExit, "SilverDragon cannot poll labvcnc status any more"
        ...
        if self.stat.task_mode == labvcnc.MODE_MANUAL \
            and self.stat.interp_state == labvcnc.INTERP_IDLE \
...
# ================
    ...
    def check_mode(self):
        string=[]
        if self.stat.task_mode == labvcnc.MODE_MANUAL and self.widgets.ntb_main.get_current_page() == _NB_PREVIEW:
...
# ==========================
    ...
    def update_toolinfo(self, tool):
        ...
        if self.load_tool:
            ...
            return
        if "G43" in self.data.active_gcodes and self.stat.task_mode != labvcnc.MODE_AUTO:
            self.command.mode(labvcnc.MODE_MDI)
            self.command.wait_complete()
