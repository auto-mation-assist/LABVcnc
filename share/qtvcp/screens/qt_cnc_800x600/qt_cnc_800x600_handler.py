...
class HandlerClass:
    ...
    def estop_toggled(self,pressed):
        ...
        if pressed:
            self.cmnd.state(labvcnc.STATE_ESTOP_RESET)
        else:
            self.cmnd.state(labvcnc.STATE_ESTOP)
    ...
    def machineon_toggled(self,pressed):
        ...
        if pressed:
            self.cmnd.state(labvcnc.STATE_ON)
            self.w.button_frame.setEnabled(True)
        else:
            self.cmnd.state(labvcnc.STATE_OFF)
            self.w.button_frame.setEnabled(False)
    ...
    def home_clicked(self):
        print 'home click'
        self.cmnd.mode(labvcnc.MODE_MANUAL)
        self.cmnd.home(-1)
    ...
    def loadfile_clicked(self):
        ...
        fname = QtWidgets.QFileDialog.getOpenFileName(None, 'Open file',
                os.path.join(os.path.expanduser('~'), 'labvcnc/nc_files/examples'))
        fname =fname[0]
        print fname
        if fname:
            NOTE.notify('Loaded',str(fname),None,10)
            f = open(str(fname), 'r')
            self.cmnd.mode(labvcnc.MODE_AUTO)
    ...
    def runfile_clicked(self):
        print 'run file'
        self.cmnd.mode(labvcnc.MODE_AUTO)
        self.cmnd.auto(labvcnc.AUTO_RUN,0)
    ...
    def pausefile_clicked(self):
        ...
        if not STATUS.stat.paused:
            #self.cmnd.mode(labvcnc.MODE_AUTO)
            self.cmnd.auto(labvcnc.AUTO_PAUSE)
        else:
            print 'resume'
            self.cmnd.auto(labvcnc.AUTO_RESUME)
