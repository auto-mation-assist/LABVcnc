...
class LbvInterface(object):
    ...
    def __init__(self):
        try:
            ...
            print "cant initialize LbvInterface: %s - LabvCNC not running?" %(msg)

    def running(self,do_poll=True):
        if do_poll: self.s.poll()
        return self.s.task_mode == labvcnc.MODE_AUTO and self.s.interp_state != labvcnc.INTERP_IDLE

    def manual_ok(self,do_poll=True):
        if do_poll: self.s.poll()
        if self.s.task_state != labvcnc.STATE_ON: return False
        return self.s.interp_state == labvcnc.INTERP_IDLE


    def ensure_mode(self,m, *p):
        '''
        If LabvCNC is not already in one of the modes given, switch it to the first mode
        example:
...
class HandlerClass:
    ...
    def on_manual_mode(self,widget,data=None):
        if self.e.ensure_mode(labvcnc.MODE_MANUAL):
            print "switched to manual mode"
        else:
            print "cant switch to manual in this state"

    def on_mdi_mode(self,widget,data=None):
        if self.e.ensure_mode(labvcnc.MODE_MDI):
            print "switched to MDI mode"
        else:
            print "cant switch to MDI in this state"

    def _query_lbv_status(self,data=None):
        (task_mode, task_state, exec_state, interp_state) = self.e.lbv_status()
    ...
    def on_probe(self,widget,data=None):
        ...
        if self.e.s.task_state != labvcnc.STATE_ON:
        ...
        self.e.ensure_mode(labvcnc.MODE_MDI)
...
#        self.e.ensure_mode(self.previous_mode)
    ...
    def __init__(self, halcomp,builder,useropts):
        ...
        glib.timeout_add_seconds(1, self._query_lbv_status)
