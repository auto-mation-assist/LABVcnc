...
def prepare_epilog(self, userdata, **words):
	...
	if retval >= 0:
		...
		lbvcanon.SELECT_POCKET( self._pocket, self._tool)
		return INTERP_OK
	else:
...
def change_epilog(self, userdata,**words):
	...
	if retval > 0:
		...
		lbvcanon.CHANGE_TOOL(self.selected_tool)
...
def gen_backtrace(self, userdata,**words):
	if  'lbvtask' in sys.builtin_module_names:
