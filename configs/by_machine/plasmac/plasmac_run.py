...
#                self.builder.get_object('material').set_active(self.materialFileDict[material][12])
    ...
    def on_single_cut_pressed(self, widget):
        ...
        if x <> 0 or y <> 0:
            self.s.poll()
            if not self.s.estop and self.s.enabled and self.s.homed.count(1) == self.s.joints and self.s.interp_state == labvcnc.INTERP_IDLE:
    ...
    def periodic(self):
        ...
        if homed and self.s.interp_state == labvcnc.INTERP_IDLE:
