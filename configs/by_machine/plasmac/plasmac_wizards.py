...
class wizards:
    ...
    def on_button_pressed(self, button):
        ...
        if 'change-consumables' in commands.lower():
            ...
            if self.gui == 'axis':
                Popen('axis-remote {}/{}'.format(os.environ['LABVCNC_NCFILES_DIR'], lFile), stdout = PIPE, shell = True)
            elif self.gui == 'gmoccapy':
                self.c.program_open('{}/{}'.format(os.environ['LABVCNC_NCFILES_DIR'], lFile))
        else:
            for command in commands.split('\\'):
                if command.strip()[0] == '%':
                    ...
                    if '{' in command:
                        ...
                        command = newCommand
                    self.s.poll()
                    if not self.s.estop and self.s.enabled and self.s.homed and (self.s.interp_state == labvcnc.INTERP_IDLE):
                        mode = self.s.task_mode
                        if mode != labvcnc.MODE_MDI:
                            mode = self.s.task_mode
                            self.c.mode(labvcnc.MODE_MDI)
                            self.c.wait_complete()
                        self.c.mdi(command)
                        self.s.poll()
                        while self.s.interp_state != labvcnc.INTERP_IDLE:
                            self.s.poll()
                        self.c.mode(mode)
                        self.c.wait_complete()
