...
w(ftabs,'itlbvonfigure','manual','-text',' Manual - F3 ')
w(ftabs,'itlbvonfigure','mdi','-text',' MDI - F5 ')
w(ftabs,'configure','-bd','1')
w(fright,'configure','-arcradius','2','-tabbevelsize','8')
w(fright,'itlbvonfigure','preview','-text',' Preview ')
w(fright,'itlbvonfigure','numbers','-text',' DRO ')
...
for number in range(0,labvcnc.MAX_JOINTS):
...
# only show paused motion if LabvCNC version 2.9 or later
if int(labvcnc.version.split('.')[0] + labvcnc.version.split('.')[1]) >= 29:
...
def joint_mode_switch(a,b,c):
    if vars.motion_mode.get() == labvcnc.TRAJ_MODE_FREE and s.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
        ...
        for command in commands.split('\\'):
            if command.strip()[0] == '%':
                ...
                if '{' in command:
                    ...
                    command = newCommand
                s.poll()
                if not s.estop and s.enabled and s.homed and (s.interp_state == labvcnc.INTERP_IDLE):
                    mode = s.task_mode
                    if mode != labvcnc.MODE_MDI:
                        mode = s.task_mode
                        c.mode(labvcnc.MODE_MDI)
                        c.wait_complete()
                    c.mdi(command)
                    s.poll()
                    while s.interp_state != labvcnc.INTERP_IDLE:
                        s.poll()
                    c.mode(mode)
                    c.wait_complete()
