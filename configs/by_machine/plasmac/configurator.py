...
class configurator:
    ...
    def __init__(self):
        ...
        if 'configs/by_machine/plasmac' in os.path.realpath(os.path.dirname(sys.argv[0])):
            ...
            self.configPath = os.path.expanduser('~') + '/labvcnc/configs'
...
# ******************************************************************************
        ...
        elif 'labvcnc/configs' in os.path.realpath(os.path.dirname(sys.argv[0])):
            ...
            print('Configurator started from unknown directory\n'\
                  'It must be located in a LabvCNC configuration directory\n'\
    ...
    def on_selection(self, button, event, selection):
        ...
        if len(sys.argv) == 3 and self.configureType == 'upgrade':
            ...
            self.machineName = self.i.find('LBV', 'MACHINE')
    ...
    def on_inifile_press_event(self,button,event):
        ...
        filter.set_name('LabvCNC INI File (*.ini)')
        ...
        if self.configureType == 'upgrade' or self.configureType == 'reconfigure':
            ...
            while 1:
                ...
                if line.startswith('[LBV]'):
                    break
                elif not line:
                    print('[LBV] missing from {}'.format(self.orgIniFile))
                    return
            while 1:
                line = inFile.readline()
    ...
    def on_halfile_press_event(self,button,event):
        self.dlg = gtk.FileChooserDialog('Save..', None, gtk.FILE_CHOOSER_ACTION_OPEN,
          (gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        filter = gtk.FileFilter()
        filter.set_name('LabvCNC HAL File (*.hal *.tcl)')
    ...
    def add_spindle_to_halfile(self,halfile):
        ...
        if os.path.exists(halFile):
            ...
            for line in inFile:
                if line.replace(' ','').startswith('loadrtmotmod') or line.replace(' ','').startswith('loadrt[LBVMOT]LBVMOT'):
        ...
        while 1:
            ...
            if line.startswith('['):
                ...
                if line.strip() in done:
                    ...
                    editSection = True
                if line.strip() == '[LBV]' and editSection:
                    newName = True
...
#            return
    ...
    def success_dialog(self):
        if '/usr/share/doc' in self.gitPath:
            ...
            cmd = '{}/scripts/labvcnc'.format(self.gitPath)
        self.dialog_ok(\
            'SUCCESS',\
    ...
    def create_widgets(self):
        ...
        if self.configureType == 'new':
            ...
            self.nameFile.set_tooltip_markup(\
                ...
                'If not existing, this creates a directory ~/labvcnc/configs/<b>name</b>.\n'\
