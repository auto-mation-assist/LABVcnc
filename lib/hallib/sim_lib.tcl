...
proc indices_for_trivkins {axes} {
  # ref: src/lbv/kinematics/trivkins.c
...
proc check_ini_items {} {
  ...
  set n_extrajoints 0
  if [info exists ::LBVMOT(LBVMOT)] {
    set mot [split [lindex $::LBVMOT(LBVMOT) 0]]
...
proc core_sim {axes
  ...
  # note: with default lbvmot==motmot,
  #       thread will not be added for (default) base_pariod == 0

  setup_kins $axes

  if {"$lbvmot" == "motmod"} {
    if [catch {loadrt $lbvmot \
       ...
       puts stderr "\n"
       puts stderr "core_sim: loadrt $lbvmot FAIL"
       puts stderr "     msg=$msg\n"
       exit 1
    }
  } else {
     ...
     #       num_extrajoints=n
     set module  [split [lindex $lbvmot 0]]
     ...
     if [catch {eval loadrt $modname $modparm \
        ...
        puts stderr "\n"
        puts stderr "core_sim:unhandled lbvmot<$lbvmot>"
  ...
  net estop:loop => iocontrol.0.lbv-enable-in
