#    This is a component of AXIS, a front-end for lbv
...
class pyvcp_dial(Canvas):
    ...
    def __init__(self,root,pycomp,halpin=None,halparam=None,param_pin=0,size=200,cpr=40,dialcolor="", \
        ...
        self.itlbvonfig(self.circle2,fill=edgecolor,activefill=edgecolor)

        self.circle=self.create_oval(pad,pad,size-pad,size-pad)             # dial circle
        self.itlbvonfig(self.circle,fill=dialcolor,activefill=dialcolor)
        
        self.itlbvonfig(self.circle)
        self.mid=size/2
        self.r=(size-2*pad)/2
        self.alfa=0
        ...
        self.itlbvonfig(self.dot,fill=dotcolor,activefill="black")
        ...
        self.itlbvonfig(self.line,arrow="last",arrowshape=(10,10,10))
        self.itlbvonfig(self.line,width=10)
    ...
    def bdown(self,event):
        ...
        self.itlbvonfig(self.dot,fill="black",activefill="black")

    def bup(self,event):
        self.itlbvonfig(self.dot,fill=self.dotcolor)
    ...
    def update_dro(self):
        ...
        self.itlbvonfig(self.dro,text=valtext)

    def update_scale(self):
        valtext = str(self.funit)
        valtext = 'x ' + valtext
        self.itlbvonfig(self.delta,text=valtext)
...
class pyvcp_meter(Canvas):
    ...
    def __init__(self,root,pycomp,halpin=None, size=200,text=None,subtext=None,min_=0,max_=100,majorscale=None, minorscale=None,region1=None,region2=None,region3=None,**kw):
        ...
        self.itlbvonfig(self.circle,fill="white")
        self.mid=size/2
        ...
        self.itlbvonfig(self.line,width=3)
...
class pyvcp_jogwheel(Canvas):
    ...
    def __init__(self,root,pycomp,halpin=None,size=200,cpr=40,**kw):
        ...
        self.itlbvonfig(self.circle,fill="lightgrey",activefill="lightgrey")
        self.mid=size/2
        self.r=(size-2*pad)/2
        self.alfa=0
        self.d_alfa=2*math.pi/cpr
        ...
        self.dot = self.create_oval(self.dot_coords())
        self.itlbvonfig(self.dot,fill="black")
        self.line = self.create_line( self.mid+(self.r*1)*math.cos(self.alfa), \
                            self.mid+(self.r*1)*math.sin(self.alfa), \
                            self.mid+(self.r*1.1)*math.cos(self.alfa), \
                            self.mid+(self.r*1.1)*math.sin(self.alfa))
        self.itlbvonfig(self.line,arrow="last",arrowshape=(10,10,10))
        self.itlbvonfig(self.line,width=8)
...
## allow value ranges in different colours
    ...
    def __init__(self,master,pycomp,fillcolor="green",bgcolor="grey",
        ...
        self.itlbvonfig(border,fill=bgcolor)
        ...
        self.itlbvonfig(self.bar,fill=fillcolor)
    ...
    def set_fill(self, (start1, end1, color1),(start2, end2, color2), (start3, end3, color3)):
        if self.value:
    		...
    		self.itlbvonfig(self.bar,fill=color1)	
    	    else:
    		if (self.value > start2) and (self.value <= end2):
    		    self.itlbvonfig(self.bar,fill=color2)	
		else:
		    if (self.value > start3) and (self.value <= end3):
    			self.itlbvonfig(self.bar,fill=color3)	
    ...
    def update(self,pycomp):
        ...
        if newvalue != self.value:
            ...
            self.itlbvonfig(self.val_text,text=valtext)
            # set bar colour
            if self.ranges:
...
class pyvcp_led(Canvas):
    ...
    def __init__(self,master,pycomp, halpin=None,disable_pin=False,     
        ...
        self.state = 0
        self.itlbvonfig(self.oh,fill=off_color)
        if halpin == None:
    ...
    def update(self,pycomp):
        ...
        if newstate == 1:
            self.itlbvonfig(self.oh,fill=self.on_color)
            self.state=1
        else:
            self.itlbvonfig(self.oh,fill=self.off_color) 
            self.state=0
        if self.disable_pin:
            is_disabled = pycomp[self.halpin_disable]
            if is_disabled == 1:
                self.itlbvonfig(self.oh,fill=self.disabled_color)
