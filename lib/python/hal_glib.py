...
class _GStat(gobject.GObject):
    '''Emits signals based on labvcnc status '''
    __gsignals__ = {
    ...
    STATES = { labvcnc.STATE_ESTOP:       'state-estop'
             , labvcnc.STATE_ESTOP_RESET: 'state-estop-reset'
             , labvcnc.STATE_ON:          'state-on'
             , labvcnc.STATE_OFF:         'state-off'
             }

    MODES  = { labvcnc.MODE_MANUAL: 'mode-manual'
             , labvcnc.MODE_AUTO:   'mode-auto'
             , labvcnc.MODE_MDI:    'mode-mdi'
             }

    INTERP = { labvcnc.INTERP_WAITING: 'interp-waiting'
             , labvcnc.INTERP_READING: 'interp-reading'
             , labvcnc.INTERP_PAUSED: 'interp-paused'
             , labvcnc.INTERP_IDLE: 'interp-idle'
             }
    ...
    def update(self):
        try:
            ...
            self._status_active = False
            # some things might not need labvcnc status but do need periodic
            self.emit('periodic')
            # Reschedule
            return True
        ...
        if state_new != state_old:
            if state_new > labvcnc.STATE_ESTOP:
        ...
        if state_new != state_old:
            if state_old == labvcnc.STATE_ON and state_new < labvcnc.STATE_ON:
                self.emit('state-off')
            self.emit(self.STATES[state_new])
            if state_new == labvcnc.STATE_ON:
                old['mode'] = 0
                old['interp'] = 0
        ...
        if interp_new != interp_old:
            if not interp_old or interp_old == labvcnc.INTERP_IDLE:
    ...
    def machine_is_on(self):
        return self.old['state']  > labvcnc.STATE_OFF

    def estop_is_clear(self):
        return self.old['state'] > labvcnc.STATE_ESTOP

    def is_man_mode(self):
        self.stat.poll()
        return self.stat.task_mode  == labvcnc.MODE_MANUAL

    def is_mdi_mode(self):
        self.stat.poll()
        return self.stat.task_mode  == labvcnc.MODE_MDI
    ...
    def is_on_and_idle(self):
        self.stat.poll()
        return self.stat.task_state > labvcnc.STATE_OFF and self.stat.interp_state == labvcnc.INTERP_IDLE

    def is_auto_running(self):
        self.stat.poll()
        return self.stat.task_mode == labvcnc.MODE_AUTO and self.stat.interp_state != labvcnc.INTERP_IDLE
    ...
    def is_interp_running(self):
        self.stat.poll()
        return self.stat.interp_state != labvcnc.INTERP_IDLE

    def is_interp_paused(self):
        self.stat.poll()
        return self.stat.interp_state == labvcnc.INTERP_PAUSED

    def is_interp_reading(self):
        self.stat.poll()
        return self.stat.interp_state == labvcnc.INTERP_READING

    def is_interp_waiting(self):
        self.stat.poll()
        return self.stat.interp_state == labvcnc.INTERP_WAITING
    ...
    def is_joint_mode(self):
        try:
            ...
            return None
        return bool(self.stat.motion_mode == labvcnc.TRAJ_MODE_FREE)
    ...
    def set_tool_touchoff(self,tool,axis,value):
        ...
        if self.stat.task_mode != labvcnc.MODE_MDI:
            premode = self.stat.task_mode
            self.cmd.mode(labvcnc.MODE_MDI)
    ...
    def do_jog(self, axisnum, direction, distance=0):
        ...
        if direction == 0:
            self.cmd.jog(labvcnc.JOG_STOP, jjogmode, j_or_a)
        else:
            if axisnum in (3,4,5):
            ...
            if distance == 0:
                self.cmd.jog(labvcnc.JOG_CONTINUOUS, jjogmode, j_or_a, direction * rate)
            else:
                self.cmd.jog(labvcnc.JOG_INCREMENT, jjogmode, j_or_a, direction * rate, distance)

    def get_jjogmode(self):
        self.stat.poll()
        if self.stat.motion_mode == labvcnc.TRAJ_MODE_FREE:
            return JOGJOINT
