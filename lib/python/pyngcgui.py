...
g_ui_dir = labvcnc.SHARE + "/labvcnc"
...
import gettext
LOCALEDIR = labvcnc.SHARE + "/locale"
gettext.install("labvcnc", localedir=LOCALEDIR, unicode=True)
...
g_stat              = None # labvcnc.stat  object
...
# src/lbv/rs274ngc/interp_internal.hh:#define INTERP_SUB_PARAMS 30
...
def get_labvcnc_ini_file():
    ps   = subprocess.Popen('ps -C labvcncsvr --no-header -o args'.split(),
                             stdout=subprocess.PIPE
                           )
    p,e = ps.communicate()

    if ps.returncode:
        print(_('get_labvcnc_ini_file: stdout= %s') % p)
        print(_('get_labvcnc_ini_file: stderr= %s') % e)
        return None
...
def default_send(filename):
    ...
    try:
        ...
        user_message(mtype=gtk.MESSAGE_ERROR
            ,title=_('labvcnc not running')
            ,msg = _('cannot send, labvcnc not running'))
        return False
    try:
        fchooser = gladevcp.hal_filechooser.LBV_Action_Open()
        fchooser._hal_init()
...
def spath_from_inifile(fname):
    if not fname:
        return []
    ini = labvcnc.ini(fname)
    homedir = os.path.dirname(os.path.realpath(fname))
    # http://www.labvcnc.org/docs/devel/html/config/ini_config.html
    l = []
...
def coord_value(char):
    # offset calc from lbv_interface.py (touchy et al)
    # char = 'x' | 'y' | ...
    # 'd' is for diameter
    c = char.lower()
    g_stat.poll()
...
class LabvcncInterface():
    """LabvcncInterface: ini file and running labvcnc data"""
    def __init__(self,cmdline_ini_file=''):
        ...
        try:
            global g_stat
            g_stat = labvcnc.stat()
            g_stat.poll() # poll faults if labvcnc not running
            self.lrunning = True
            l_ini_file = get_labvcnc_ini_file()
        except labvcnc.error,msg:
            g_stat = None
            print('INTFC:err:',msg)
            print('INTFC:' + _('Warning: labvcnc not running'))

        print('%s:INTFC:labvcnc running=%d' % (g_progname,self.lrunning))
        ...
        if ini_file:
            ...
            self.ini_data = labvcnc.ini(self.ini_file)
        ...
        for line in self.inputlines:
            ...
            if endsubname is not None:
                if lineiscomment or (l.strip() == ''):
                    ...
                    # labvcnc ignores m2 after endsub in
...
class TestButtons():
    ...
    def btest(self,widget,v):
        ...
        elif v == 'lcnc':  show_dir(labvcnc,tag='lcnc')
    ...
    def savesection_ngc(self):
        ...
        if stat == 'NOTFOUND':
            ...
            user_message(mtype=gtk.MESSAGE_INFO
                ...
                + '%s\n\n'
                + _('is not in the labvcnc path\n'
                    'Expanding in place.\n\n'
                    'Note: labvcnc will fail if it calls\n'
...
class NgcGui():
    ...
    def __init__(self,w=None
        ...
        self.intfc = LabvcncInterface(ini_file)
            ...
            if len(self.intfc.subroutine_path) != 0:
                user_message(mtype=gtk.MESSAGE_WARNING
                    ...
                    ,msg=_('No subroutine path available.\n'
                      ...
                      + '\n'
                      + _('Generated results may not be usable with labvcnc')
                    )
        if len(self.intfc.subroutine_path) == 0:
            if g_alive:
        ...
        if auto_file:
            ...
            try:
                ...
                #        ,msg=_('autofile==%s is not in labvcnc\n'
        ...
        for idx in sub_info.ndict:
            ...
            try:
                ...
                emsg = emsg + (
                     ...
                     % (int(idx),value))
            #note: e formats not accepted by labvcnc (like 1e2)
        ...
        if (not mypg.expandsub) and (not force_expand):
            ...
            for line in sub_info.inputlines:
                ...
                if idx in sub_info.ldict:
                    ...
                    # hack: try to reduce long line length so labvcnc wont
...
Notes:
      ...
      If --ini is NOT specified:
         search for a running labvcnc and use its inifile
    """ % g_progname)
#-----------------------------------------------------------------------------
