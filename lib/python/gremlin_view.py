...
Standalone functionality if labvcnc running
...
def ini_check ():
    ...
    if ini_filename is not None:
        ...
        return True # success
    print(_('%s:labvcnc ini file  not available') % g_progname)
    return False # exit here crashes glade-gtk2

def get_labvcnc_ini_file():
    """find labvcnc ini file with pgrep"""
...
class GremlinView():
    ...
    def __init__(self
        ...
        self.alive = alive
        labvcnc_running = False
        if ini_check():
            labvcnc_running = True
...
Note: labvcnc must be running on same machine
""") % g_progname
        if msg:
            print('\n%s' % msg)
    #---------------------------------------
