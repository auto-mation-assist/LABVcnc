...
import sys, os, pango, labvcnc, hashlib, glib
...
class ToolEdit(gtk.VBox):
    ...
    def periodic_check(self):
        try:
            self.lbvstat.poll()
            on = self.lbvstat.task_state > labvcnc.STATE_OFF
            idle = self.lbvstat.interp_state == labvcnc.INTERP_IDLE
    ...
    def get_toolinfo(self,toolnum=None):
        if toolnum == None:
            self.toolinfo_num = self.lbvstat.tool_in_spindle
        else:
...
def main(filename=None):
    ...
    tooledit.set_filename("/home/jim/labvcnc/configs/sim.gmoccapy/tool.tbl")
    #tooledit.set_filename("/home/chris/lbv2-dev/configs/sim/lathe.tbl")
