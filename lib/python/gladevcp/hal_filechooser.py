...
from hal_actions import _LBV_ActionBase, _LBV_Action
...
class _LBV_FileChooser(_LBV_ActionBase):
    def _hal_init(self):
    ...
    def _load_file(self, filename):
        if filename:
            self.labvcnc.mode(labvcnc.MODE_AUTO)
            old = self.gstat.stat.file
            self.labvcnc.program_open(filename)
            if old == filename:
...
class LBV_FileChooserDialog(gtk.FileChooserDialog, _LBV_FileChooser):
    __gtype_name__ = 'LBV_FileChooserDialog'
    def __init__(self, *a, **kw):
        gtk.FileChooserDialog.__init__(self, *a, **kw)
        _LBV_FileChooser._hal_init(self)
...
class LBV_FileChooserButton(gtk.FileChooserButton, _LBV_FileChooser):
    __gtype_name__ = 'LBV_FileChooserButton'
    def __init__(self, *a, **kw):
...
class LBV_Action_Open(_LBV_Action, _LBV_FileChooser):
    __gtype_name__ = 'LBV_Action_Open'
    fixed_file = gobject.property(type=str, default='', nick='Fixed file name')

    def _hal_init(self):
        _LBV_FileChooser._hal_init(self)
        _LBV_Action._hal_init(self)
        self.currentfolder = os.path.expanduser("~/labvcnc/nc_files")
    ...
    def on_activate(self, w):
        if self.fixed_file:
            ...
            return
        dialog = LBV_FileChooserDialog(title="Open File",action=gtk.FILE_CHOOSER_ACTION_OPEN, 
...
class LBV_Action_Reload(_LBV_Action, _LBV_FileChooser):
    __gtype_name__ = 'LBV_Action_Reload'
    def _hal_init(self):
        _LBV_Action._hal_init(self)
