...
# This widgets displays labvcnc axis position information.
#
...
#    INIPATH = '/home/lbvmesa/labvcnc-dev/configs/sim/gmoccapy/gmoccapy.ini'
    pass

# This is the main class
class Combi_DRO(gtk.VBox):
    '''
    Combi_DRO will display an labvcnc DRO with all three types at ones
    ...
    def _periodic(self):
        # we do not want to throw errors if labvcnc has been killed
        ...
        if self.status.kinematics_type != labvcnc.KINEMATICS_IDENTITY and not self.homed:
    ...
    def _not_all_homed(self, widget, data = None):
        if self.status.kinematics_type == labvcnc.KINEMATICS_IDENTITY:
            self.status.poll()
