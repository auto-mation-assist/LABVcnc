...
from hal_actions import _LBV_ActionBase, ensure_mode
...
class LBV_MDIHistory(gtk.VBox, _LBV_ActionBase):
    '''
    LBV_MDIHistory will store each MDI command to a file on your hard drive
    and display the grabbed commands in a treeview so they can be used again
    without typing the complete comand again
    '''

    __gtype_name__ = 'LBV_MDIHistory'
    __gproperties__ = {
    ...
    def submit(self, *a):
        ...
        if not cmd:
            return
        ensure_mode(self.gstat.stat, self.labvcnc, labvcnc.MODE_MDI)
...
# for testing without glade editor or LabvCNC not running:
def main():
