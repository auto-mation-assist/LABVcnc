...
# This widget reads the offsets for current tool, current G5x, G92 using python library labvcnc.
# all other offsets are read directly from the Var file, if available, as labvcnc does not give
...
import sys, os, pango, labvcnc
from hal_glib import GStat
datadir = os.path.abspath(os.path.dirname(__file__))
AXISLIST = ['offset', 'X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W', 'name']
# we need to know if labvcnc isn't running when using the GLADE editor
...
# labvcnc is probably not running then
try:
...
class OffsetPage(gtk.VBox):
    ...
    def __init__(self, filename = None, *a, **kw):
        ...
        # check labvcnc status every half second
    ...
    # TODO the edited column does not end up showing the editted number even though labvcnc
    ...
    def col_editted(self, widget, filtered_path, new_text, col):
        ...
        try:
            ...
            print 'error'
        # now update labvcnc to the change
        try:
            global lncnc_runnning
            if lncnc_running:
                if self.status.task_mode != self.labvcnc.MODE_MDI:
                    self.cmd.mode(self.labvcnc.MODE_MDI)
                ...
                self.cmd.mode(self.labvcnc.MODE_MANUAL)
                self.cmd.wait_complete()
                self.cmd.mode(self.labvcnc.MODE_MDI)
                self.cmd.wait_complete()
    ...
    def periodic_check(self):
        ...
        try:
            self.status.poll()
            on = self.status.task_state > labvcnc.STATE_OFF
            idle = self.status.interp_state == labvcnc.INTERP_IDLE
...
# Must labvcnc running to see anything
