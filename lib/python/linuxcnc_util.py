"""
LabvCNC User Interface helper functions
"""
...
class LabvCNC_Exception(Exception):
    pass


class LabvCNC:
    """
    This class implements the main interface to LabvCNC.
    """
    ...
    def wait_for_labvcnc_startup(self, timeout=10.0):
        ...
        while time.time() - start_time < timeout:
            ...
            if (self.status.angular_units == 0.0) \
                ...
                or (self.status.exec_state != labvcnc.EXEC_DONE) \
                or (self.status.interp_state != labvcnc.INTERP_IDLE) \
                ...
                or (self.status.state != labvcnc.STATE_ESTOP) \
                or (self.status.task_state != labvcnc.STATE_ESTOP):
                time.sleep(0.1)
            else:
                # looks good
                return
    ...
    def wait_for_home(self, joints, timeout=10.0):
        ...
        raises LabvCNC_Exception if the timeout expired first."""
	...
	"""Arguments:
        ...
        expires.  Raises LabvCNC_Exception if the timeout expires before
        the axis stops.
        """
    ...
    def jog_axis(self, axis_letter, target, vel=5.0, timeout=10.0):
        ...
        This function uses the labvcnc.command.jog() function to do
        ...
        any other axis moved, raises LabvCNC_Exeption.
        """
	...
	self.command.jog(labvcnc.JOG_CONTINUOUS, 0, axis_index, vel)
	...
	for i in range(0, 9):
		...
		raise LabvCNC_Exception("axis %s moved from %.3f to %.3f but shouldnt have!" % ('xyzabcuvw'[i], start_pos[i], self.status.position[i]))
	...
	while ((time.time() - start) < timeout):
		...
		return
	    time.sleep(0.1)
	raise LabvCNC_Exception("timeout waiting for axis %s to stop at %.3f (pos=%.3f, vel=%.3f)" % (axis_letter, target, self.status.position[axis_index], vel))
	...
	Arguments:
            ...
            one of labvcnc.INTERP_IDLE, labvcnc.INTERP_PAUSED,
            labvcnc.INTERP_READING, or labvcnc.INTERP_WAITING.

            'timeout' is a float, the number of seconds to wait for the
            Interpreter to reach the target state.

        This function polls the LabvCNC Status buffer waiting for the
	...
	if self.status.interp_state != target_state:
	    raise LabvCNC_Exception("interpreter state %d did not reach target state %d" % (self.state.interp_state, target_state))
	...
	"""
        ...
        before the tool appears, it raises LabvCNC_Exception.
        """
	...
	while (time.time() - start_time) < timeout:
		...
		return
	raise LabvCNC_Exception("the Stat buffer's toolInSpindle value is %d, expected %d" % (self.status.tool_in_spindle, expected_tool))
