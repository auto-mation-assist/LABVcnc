...
# lib/tcltk/lbv2 for installed lbv
# tcl            for run-in-place lbv
for candidate in 'lib/tcltk/labvcnc', 'tcl':
    LABVCNC_TCL = os.path.join(nf.PREFIX, candidate, 'labvcnc.tcl')
    if os.path.exists(LABVCNC_TCL): break

options = '''
. configure -bg #d9d9d9

set BASE_FONT [labvcnc::standard_font]
set FIXED_FONT [labvcnc::standard_fixed_font]
