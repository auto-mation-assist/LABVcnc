...
class _Lcnc_Action(object):
    ...
    def SET_ESTOP_STATE(self, state):
        if state:
            self.cmd.state(labvcnc.STATE_ESTOP)
        else:
            self.cmd.state(labvcnc.STATE_ESTOP_RESET)

    def SET_MACHINE_STATE(self, state):
        if state:
            self.cmd.state(labvcnc.STATE_ON)
        else:
            self.cmd.state(labvcnc.STATE_OFF)

    def SET_MACHINE_HOMING(self, joint):
        log.info('Homing Joint: {}'.format(joint))
        self.ensure_mode(labvcnc.MODE_MANUAL)
        self.cmd.teleop_enable(False)
        if not INFO.HOME_ALL_FLAG and joint == -1:
            STATUS.emit('error',labvcnc.NML_ERROR,'Home-all not available according to INI Joint Home sequence')
        else:
            self.cmd.home(joint)
    ...
    def SET_MACHINE_UNHOMED(self, joint):
        self.ensure_mode(labvcnc.MODE_MANUAL)
        self.cmd.teleop_enable(False)
        #self.cmd.traj_mode(labvcnc.TRAJ_MODE_FREE)
        self.cmd.unhome(joint)

    def SET_AUTO_MODE(self):
        self.ensure_mode(labvcnc.MODE_AUTO)
    ...
    def TOGGLE_LIMITS_OVERRIDE(self):
        if STATUS.is_limits_override_set() and STATUS.is_hard_limits_tripped():
            STATUS.emit('error',labvcnc.OPERATOR_ERROR,'''Can Not Reset Limits Override - Still On Hard Limits''')
        elif not STATUS.is_limits_override_set() and STATUS.is_hard_limits_tripped():
            STATUS.emit('error',labvcnc.OPERATOR_ERROR,'Hard Limits Are Overridden!')
            self.cmd.override_limits()
        else:
            STATUS.emit('error',labvcnc.OPERATOR_TEXT,'Hard Limits Are Reset To Active!')
            self.cmd.override_limits()
    ...
    def SET_AXIS_ORIGIN(self,axis,value):
        ...
        fail, premode = self.ensure_mode(labvcnc.MODE_MDI)
        self.cmd.mdi(m)
    ...
    def RUN(self, line=0):
        ...
        if STATUS.is_auto_paused() and line == 0:
            self.cmd.auto(labvcnc.AUTO_STEP)
            return
        elif not STATUS.is_auto_running():
            self.cmd.auto(labvcnc.AUTO_RUN,line)

    def STEP(self):
        if STATUS.is_auto_running() and not STATUS.is_auto_paused():
            self.cmd.auto(labvcnc.AUTO_PAUSE)
            return
    ...
    def PAUSE(self):
        if not STATUS.stat.paused:
            ...
            log.debug('resume')
            self.cmd.auto(labvcnc.AUTO_RESUME)
    ...
    def SET_SPINDLE_FASTER(self, number = 0):
        self.cmd.spindle(labvcnc.SPINDLE_INCREASE, number)
    def SET_SPINDLE_SLOWER(self, number = 0):
        self.cmd.spindle(labvcnc.SPINDLE_DECREASE, number)
    def SET_SPINDLE_STOP(self, number = 0):
        self.cmd.spindle(labvcnc.SPINDLE_OFF, number)
    ...
    def DO_JOG(self, joint_axis, direction):
        ...
        if isinstance(joint_axis, (int, long)):
            if STATUS.stat.joint[joint_axis]['jointType'] == labvcnc.ANGULAR:
                angular =  True
            jointnum = joint_axis
        else:
        ...
        try:
            try:
                ...
                try:
                    ...
                    try:
                        subprocess.call('systlbvtl poweroff', shell=True)
                    except:
                        raise
        except Exception as e:
    ...
    def get_jog_info (self,num):
        if STATUS.stat.motion_mode == labvcnc.TRAJ_MODE_FREE:
            return True, self.jnum_check(num)
        return False, num

    def jnum_check(self,num):
        if STATUS.stat.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
    ...
    def _mktemp(self):
        if self.tmp:
            return
        self.tmp = tempfile.mkdtemp(prefix='lbvflt-', suffix='.d')
