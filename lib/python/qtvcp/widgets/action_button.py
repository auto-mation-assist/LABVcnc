...
# Action buttons are used to change labvcnc behaivor do to button pushing.
...
# STATUS gives us status messages from labvcnc
# ACTION gives commands to labvcnc
...
class ActionButton(Indicated_PushButton, _HalWidgetBase):
    ...
    # other entities change labvcnc's state
    # also some buttons are disabled/enabled based on
    # labvcnc state / possible actions
    #
    ...
    # the buttons can be synced with labvcnc
    ...
    def action(self, state=None):
        ...
        if self.estop:
            ...
            ACTION.SET_SPINDLE_ROTATION(labvcnc.SPINDLE_FORWARD, INFO.DEFAULT_SPINDLE_SPEED)
        elif self.spindle_rev:
            ACTION.SET_SPINDLE_ROTATION(labvcnc.SPINDLE_REVERSE, INFO.DEFAULT_SPINDLE_SPEED)
        elif self.spindle_stop:
    ...
    def jog_selected_action(self, direction):
        ...
        if direction:
            ACTION.ensure_mode(labvcnc.MODE_MANUAL)
