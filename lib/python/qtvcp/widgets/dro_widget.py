...
class DROLabel(ScaledLabel, _HalWidgetBase):
    ...
    def _hal_init(self):
        ...
        if self._joint_type == labvcnc.ANGULAR:
    ...
    def motion_mode(self, w, mode):
        if mode == labvcnc.TRAJ_MODE_COORD:
            pass
        # Joint mode
        elif mode == labvcnc.TRAJ_MODE_FREE:
            self._mode = False
        # axis 
        elif mode == labvcnc.TRAJ_MODE_TELEOP:
            self._mode = True
    ...
    def update(self, widget, absolute, relative, dtg, joint):
        ...
        try:
            if self.reference_type == 0:
                if not self._mode and STATUS.stat.kinematics_type != labvcnc.KINEMATICS_IDENTITY:
